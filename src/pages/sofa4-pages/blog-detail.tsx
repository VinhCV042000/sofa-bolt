...
import { SOFA4_PAGE_BLOG_POSTS } from 'src/sections/sofa4-pages/sofa4-pages-data';
import { Sofa4Section, Sofa4PageHero } from 'src/sections/sofa4-pages/sofa4-page-hero';

const metadata = { title: 'Chi tiết bài viết - Sofa Pop' };

export default function Page() {
  const { slug } = useParams();
  const post = SOFA4_PAGE_BLOG_POSTS.find((p) => p.slug === slug) || SOFA4_PAGE_BLOG_POSTS[0];
  const related = SOFA4_PAGE_BLOG_POSTS.filter((p) => p.id !== post.id).slice(0, 3);
  ...
  return (
    <>
      ...
      <Sofa4PageHero overline={post.categoryLabel} title={post.title} subtitle={post.excerpt} image={post.image} />
          ...
          <Grid xs={12} md={8}>
            <Stack spacing={3}>
              <Stack direction="row" spacing={2} alignItems="center">
                <Avatar sx={{ bgcolor: 'warning.main' }}>{post.author.charAt(0)}</Avatar>
                <Stack>
              ...
              <Typography sx={{ color: 'text.secondary', lineHeight: 1.8 }}>
                Sofa Pop cam kết sử dụng 100% vật liệu tự nhiên, từ linen hữu cơ đến gỗ tre bền vững.
              ...
              <Box>
                ...
                <Stack direction="row" spacing={2}>
                  {['solar:facebook-bold-duotone', 'solar:share-bold-duotone', 'solar:letter-bold-duotone'].map((icon) => (
                    <Box key={icon} component="button" sx={{ width: 44, height: 44, borderRadius: '50%', border: 0, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: (t) => t.palette.mode === 'dark' ? 'background.paper' : 'grey.100', color: 'warning.main', '&:hover': { bgcolor: (t) => t.palette.mode === 'dark' ? 'warning.dark' : 'warning.lighter' } }}>
              ...
              <Box>
                ...
                <Stack spacing={2}>
                  <Stack direction="row" spacing={2}>
                    <Avatar sx={{ bgcolor: 'warning.main' }}>M</Avatar>
              ...
              {related.map((p, i) => (
                <Box key={p.id} component={m.div} variants={varFade({ distance: 24 }).inUp} transition={{ delay: i * 0.08 }}>
                  <Stack direction="row" spacing={2} sx={{ textDecoration: 'none' }}>
                    ...
                    <Stack spacing={0.5}>
                      <Typography variant="overline" sx={{ color: 'warning.main' }}>{p.categoryLabel}</Typography>
