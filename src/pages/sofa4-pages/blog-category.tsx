...
import { SOFA4_PAGE_BLOG_POSTS, SOFA4_PAGE_BLOG_CATEGORIES } from 'src/sections/sofa4-pages/sofa4-pages-data';

const metadata = { title: 'Chuyên mục blog - Sofa Pop' };

export default function Page() {
  const { slug } = useParams();
  const category = SOFA4_PAGE_BLOG_CATEGORIES.find((c) => c.slug === slug);
  const posts = slug ? SOFA4_PAGE_BLOG_POSTS.filter((p) => p.category === slug) : SOFA4_PAGE_BLOG_POSTS;
  ...
  return (
    <>
      ...
      <Sofa4PageHero overline="Blog" title={category ? category.label : 'Tất cả bài viết'} subtitle={`${posts.length} bài viết`} image={SOFA4_PAGE_BLOG_POSTS[0].image} />
