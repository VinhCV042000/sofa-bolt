...
const metadata = { title: 'Theo dõi tuyển dụng - Sofa Pop' };

const APPLICATIONS = [
  { id: 'APP001', position: 'Thợ mộc chế tác sofa eco', date: '15/01/2025', status: 'Đang xét duyệt' },
  { id: 'APP002', position: 'Nhân viên tư vấn Pop', date: '10/01/2025', status: 'Phê duyệt' },
];
...
export default function Page() {
  return (
    <>
      ...
      <Sofa4PageHero overline="Tuyển dụng" title="Theo dõi hồ sơ ứng tuyển" subtitle="Kiểm tra trạng thái hồ sơ ứng tuyển của bạn." image={SOFA4_PAGE_IMAGES.careers} />
        ...
        <Stack spacing={3} component={MotionViewport} sx={{ maxWidth: 720, mx: 'auto' }}>
          {APPLICATIONS.map((app, index) => (
            <Stack key={app.id} component={m.div} variants={varFade({ distance: 24 }).inUp} transition={{ delay: index * 0.08 }} direction="row" spacing={3} sx={{ p: 3, borderRadius: 2, bgcolor: 'background.paper', border: (t) => `1px solid ${varAlpha(t.vars.palette.grey['500Channel'], 0.12)}` }} alignItems="center">
              <Box sx={{ width: 48, height: 48, borderRadius: 1.5, display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: (t) => varAlpha(t.vars.palette.warning.mainChannel, 0.08), color: 'warning.main' }}>
              ...
              <Box sx={{ px: 2, py: 1, borderRadius: 1, bgcolor: app.status === 'Phê duyệt' ? 'warning.main' : 'warning.main', color: 'common.white' }}>
