...
const metadata = { title: 'Showroom - Sofa Pop' };
...
export default function Page() {
  return (
    <>
      ...
      <Sofa4PageHero overline="Showroom" title="Hệ thống showroom eco" subtitle="Trải nghiệm sofa Pop tại 4 showroom trên toàn quốc." image={SOFA4_PAGE_IMAGES.contact} />

      <Box component="section" sx={{ py: { xs: 8, md: 12 } }}>
        <Container component={MotionViewport}>
          <Grid container spacing={3}>
            {SOFA4_PAGE_SHOWROOMS.map((showroom, index) => (
              <Grid key={showroom.id} xs={12} sm={6} md={3}>
                <Stack component={m.div} variants={varFade({ distance: 24 }).inUp} transition={{ delay: index * 0.08 }} sx={{ height: 1, borderRadius: 2, overflow: 'hidden', bgcolor: 'background.paper', boxShadow: (t) => t.customShadows.card }}>
                  ...
                  <Stack spacing={1.5} sx={{ p: 2.5 }}>
                    ...
                    <Stack direction="row" spacing={1} alignItems="center"><Iconify icon="solar:map-point-bold-duotone" width={18} sx={{ color: 'warning.main' }} /><Typography variant="body2" sx={{ color: 'text.secondary' }}>{showroom.address}</Typography></Stack>
                    <Stack direction="row" spacing={1} alignItems="center"><Iconify icon="solar:phone-bold-duotone" width={18} sx={{ color: 'warning.main' }} /><Typography variant="body2" sx={{ color: 'text.secondary' }}>{showroom.phone}</Typography></Stack>
                    <Stack direction="row" spacing={1} alignItems="center"><Iconify icon="solar:clock-circle-bold-duotone" width={18} sx={{ color: 'warning.main' }} /><Typography variant="body2" sx={{ color: 'text.secondary' }}>{showroom.hours}</Typography></Stack>
                    <Box component={RouterLink} href={`/sofa4/showrooms/${showroom.id}`} sx={{ textDecoration: 'none' }}>
                      <Typography variant="button" sx={{ color: 'warning.main' }}>Xem chi tiết →</Typography>
                    </Box>
                  </Stack>
                </Stack>
              </Grid>
