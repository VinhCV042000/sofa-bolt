...
const metadata = { title: 'Theo dõi đơn hàng - Sofa Pop' };
...
export default function Page() {
  return (
    <>
      ...
      <Sofa4PageHero overline="Giỏ hàng" title="Theo dõi đơn hàng" subtitle="Kiểm tra trạng thái đơn hàng của bạn." image={SOFA4_PAGE_IMAGES.cta} />
          ...
          {ORDERS.map((order, index) => (
            <Stack key={order.id} component={m.div} variants={varFade({ distance: 24 }).inUp} transition={{ delay: index * 0.08 }} spacing={2} sx={{ p: 3, borderRadius: 2, bgcolor: 'background.paper', border: (t) => `1px solid ${varAlpha(t.vars.palette.grey['500Channel'], 0.12)}` }}>
              <Stack direction="row" justifyContent="space-between" alignItems="center">
                ...
                <Box sx={{ px: 2, py: 1, borderRadius: 1, bgcolor: order.status === 'Đã giao' ? 'warning.main' : 'warning.main', color: 'common.white' }}>
                  <Typography variant="caption" sx={{ fontWeight: 'fontWeightBold' }}>{order.status}</Typography>
                </Box>
              </Stack>
              <Typography variant="subtitle2" sx={{ color: 'warning.main' }}>{order.total}</Typography>
              <Stack direction="row" spacing={1} alignItems="center">
                <Iconify icon="solar:tracking-bold-duotone" width={20} sx={{ color: 'warning.main' }} />
