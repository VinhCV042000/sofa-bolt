...
import { SOFA4_PAGE_PRODUCTS, formatSofa4PagePrice } from 'src/sections/sofa4-pages/sofa4-pages-data';

const metadata = { title: 'Chi tiết sản phẩm - Sofa Pop' };
...
export default function Page() {
  ...
  const product = SOFA4_PAGE_PRODUCTS.find((p) => p.id === slug) || SOFA4_PAGE_PRODUCTS[0];
  const related = SOFA4_PAGE_PRODUCTS.filter((p) => p.id !== product.id).slice(0, 3);
  ...
  return (
    <>
      ...
      <Sofa4PageHero overline="Sản phẩm" title={product.name} subtitle={product.description} image={product.image} />
          ...
          <Grid xs={12} md={6}>
            <Stack spacing={3} component={m.div} variants={varFade({ distance: 24 }).inRight}>
              ...
              <Stack direction="row" alignItems="center" spacing={2}>
                <Typography variant="h3" sx={{ color: 'warning.main', fontWeight: 'fontWeightBold' }}>{formatSofa4PagePrice(product.price)}</Typography>
                {product.oldPrice && <Typography variant="h5" sx={{ color: 'text.disabled', textDecoration: 'line-through' }}>{formatSofa4PagePrice(product.oldPrice)}</Typography>}
              </Stack>

              <Stack spacing={1.5}>
                <Stack direction="row" spacing={1} alignItems="center">
                  <Iconify icon="solar:leaf-bold-duotone" width={20} sx={{ color: 'warning.main' }} />
                ...
                <Stack direction="row" spacing={1} alignItems="center">
                  <Iconify icon="solar:palette-bold-duotone" width={20} sx={{ color: 'warning.main' }} />
                  <Typography variant="body2" sx={{ color: 'text.secondary' }}>Màu sắc: {product.colors.join(', ')}</Typography>
                </Stack>
              </Stack>

              <Stack direction="row" spacing={2}>
                <Button variant="contained" color="warning" size="large" startIcon={<Iconify icon="solar:cart-bold-duotone" />} component={RouterLink} href="/sofa4/cart">
                  Thêm vào giỏ
                </Button>
                <Button variant="outlined" color="warning" size="large" startIcon={<Iconify icon="solar:heart-bold-duotone" />}>
                  Yêu thích
                </Button>
              </Stack>
              ...
              <Box sx={{ py: 2 }}>
                ...
                {tab === 3 && (
                  <Stack spacing={2}>
                    ...
                    <Button variant="outlined" color="warning" size="small">Đặt câu hỏi</Button>
                  </Stack>
                )}
              </Box>
            </Stack>
              ...
              <Grid key={p.id} xs={12} sm={6} md={4}>
                <Stack component={m.div} variants={varFade({ distance: 24 }).inUp} sx={{ height: 1, borderRadius: 2, overflow: 'hidden', bgcolor: 'background.paper', boxShadow: (t) => t.customShadows.card }}>
                  ...
                  <Stack spacing={1} sx={{ p: 2.5 }}>
                    ...
                    <Typography variant="subtitle2" sx={{ color: 'warning.main', fontWeight: 'fontWeightBold' }}>{formatSofa4PagePrice(p.price)}</Typography>
                    <Button component={RouterLink} href={`/sofa4/products/${p.id}`} variant="outlined" color="warning" size="small">Xem chi tiết</Button>
                  </Stack>
                </Stack>
              </Grid>
            ))}
