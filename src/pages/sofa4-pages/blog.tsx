...
import { SOFA4_PAGE_IMAGES, SOFA4_PAGE_BLOG_POSTS, SOFA4_PAGE_BLOG_CATEGORIES } from 'src/sections/sofa4-pages/sofa4-pages-data';

const metadata = { title: 'Blog - Sofa Pop' };

export default function Page() {
  const featured = SOFA4_PAGE_BLOG_POSTS[0];
  const rest = SOFA4_PAGE_BLOG_POSTS.slice(1);
  ...
  return (
    <>
      ...
      <Sofa4PageHero overline="Blog" title="Tin tức & kiến thức eco" subtitle="Kinh nghiệm chọn sofa, xu hướng nội thất, kiến thức vật liệu xanh." image={SOFA4_PAGE_IMAGES.blog1} />

      <Box component="section" sx={{ py: { xs: 8, md: 12 } }}>
        <Container component={MotionViewport}>
          <Stack spacing={4}>
            <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap', gap: 1 }}>
              {SOFA4_PAGE_BLOG_CATEGORIES.map((cat) => (
                <Box key={cat.slug} component={RouterLink} href={`/sofa4/blog/category/${cat.slug}`} sx={{ px: 2, py: 1, borderRadius: 2, textDecoration: 'none', bgcolor: (t) => t.palette.mode === 'dark' ? 'background.paper' : 'grey.100', color: 'text.secondary', fontSize: 14, fontWeight: 'fontWeightMedium', '&:hover': { color: 'warning.main' } }}>
                  {cat.label}
                </Box>
              ))}
            </Stack>

            <Box component={RouterLink} href={`/sofa4/blog/${featured.slug}`} sx={{ position: 'relative', overflow: 'hidden', borderRadius: 3, textDecoration: 'none', display: 'block' }}>
              ...
              <Stack spacing={1} sx={{ position: 'absolute', bottom: 0, left: 0, right: 0, p: { xs: 3, md: 5 }, color: 'common.white' }}>
                <Typography variant="overline" sx={{ color: 'warning.light' }}>{featured.categoryLabel} · {featured.date}</Typography>
              ...
              {rest.map((post, index) => (
                <Grid key={post.id} xs={12} sm={6} md={4}>
                  <Stack component={m.div} variants={varFade({ distance: 24 }).inUp} transition={{ delay: index * 0.06 }} sx={{ height: 1, borderRadius: 2, overflow: 'hidden', bgcolor: 'background.paper', boxShadow: (t) => t.customShadows.card }}>
                    <Box component={RouterLink} href={`/sofa4/blog/${post.slug}`} sx={{ textDecoration: 'none' }}>
