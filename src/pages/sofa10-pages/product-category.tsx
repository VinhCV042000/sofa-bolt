...
export default function Page() {
  ...
  const allCategories = [
    ...SOFA10_PAGE_PRODUCT_CATEGORIES.types,
    ...SOFA10_PAGE_PRODUCT_CATEGORIES.styles,
    ...SOFA10_PAGE_PRODUCT_CATEGORIES.spaces,
    ...SOFA10_PAGE_PRODUCT_CATEGORIES.sizes,
    ...SOFA10_PAGE_PRODUCT_CATEGORIES.prices,
  ];
  const category = allCategories.find((c) => c.slug === slug);
  const label = category?.label || 'Tất cả sản phẩm';
  const filtered = SOFA10_PAGE_PRODUCTS.filter((p) => p.category === slug || p.style === slug);
      ...
      <Sofa10Section>
        ...
        {filtered.length === 0 ? (
          <Stack spacing={2} alignItems="center" sx={{ py: 10, textAlign: 'center' }}>
            ...
            <Typography variant="h5" sx={{ color: 'text.secondary', fontWeight: 'fontWeightLight' }}>Chưa có sản phẩm trong danh mục này</Typography>
          </Stack>
        ) : (
