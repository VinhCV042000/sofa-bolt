...
import { SOFA20_PAGE_BLOG_POSTS } from 'src/sections/sofa20-pages/sofa20-pages-data';
...
export default function Page() {
  ...
  const post = SOFA20_PAGE_BLOG_POSTS.find((p) => p.slug === slug) || SOFA20_PAGE_BLOG_POSTS[0];
  const [comment, setComment] = useState('');
  const [comments, setComments] = useState<string[]>([]);

  return (
    <>
      <Helmet><title>{post.title} — Sofa20 Blog</title></Helmet>
      <Sofa20PageHero overline="BLOG" title={<span>{post.title.toUpperCase()}</span>} subtitle={post.excerpt} image={post.image} />
        ...
        <Grid container spacing={5}>
          <Grid xs={12} md={8}>
            <Stack spacing={4} component={m.div} variants={varFade({ distance: 24 }).inUp}>
              ...
              <Stack direction="row" spacing={2} alignItems="center">
                <Avatar sx={{ bgcolor: SOFA20_COLORS.gold, color: SOFA20_COLORS.black, fontWeight: 'fontWeightMedium' }}>{post.author.charAt(0)}</Avatar>
                <Stack>
              ...
              <Stack spacing={3}>
                ...
                {comments.map((c, i) => (
                  <Sofa20Card key={i} accent={[SOFA20_COLORS.gold, SOFA20_COLORS.emerald, SOFA20_COLORS.burgundy][i % 3]}>
                    <Stack direction="row" spacing={2}>
                      <Avatar sx={{ bgcolor: [SOFA20_COLORS.gold, SOFA20_COLORS.emerald, SOFA20_COLORS.burgundy][i % 3], color: SOFA20_COLORS.black, fontWeight: 'fontWeightMedium' }}>{i + 1}</Avatar>
                ...
                <Stack spacing={2}>
                  ...
                  <Button onClick={() => { if (comment.trim()) { setComments([...comments, comment]); setComment(''); } }} variant="contained" sx={{ alignSelf: 'flex-start', borderRadius: 0, fontWeight: 'fontWeightMedium', color: SOFA20_COLORS.black, bgcolor: SOFA20_COLORS.gold, border: `1px solid ${SOFA20_COLORS.gold}`, '&:hover': { bgcolor: SOFA20_COLORS.goldLight } }}>Gửi bình luận</Button>
                </Stack>
              </Stack>
            </Stack>
          </Grid>
          <Grid xs={12} md={4}>
            <Stack spacing={3} component={m.div} variants={varFade({ distance: 24 }).inRight} sx={{ position: 'sticky', top: 80 }}>
              <Typography variant="h6" sx={{ fontWeight: 'fontWeightMedium', color: SOFA20_COLORS.gold }}>Chia sẻ</Typography>
              <Stack direction="row" spacing={2}>
                ...
                <Iconify icon="solar:letter-bold-duotone" width={32} sx={{ color: SOFA20_COLORS.gold, cursor: 'pointer' }} />
              </Stack>
            </Stack>
          </Grid>
        </Grid>
