...
export default function Page() {
  ...
  return (
    <>
      <Helmet><title>Khuyến mãi — Sofa20</title></Helmet>
      <Sofa20PageHero overline="KHUYẾN MÃI" title={<>KHUYẾN <span>MÃI</span></>} subtitle="Bảo hành, bảo trì, sửa chữa, vệ sinh, đổi trả — dịch vụ trọn đời." image={SOFA20_PAGE_IMAGES.prod1} />

      <Sofa20Section bg="black">
        <Grid container spacing={3}>
          {SOFA20_PAGE_PROMOTION_SERVICES.map((s) => (
            <Grid key={s.slug} xs={12} sm={6} md={4}>
              <Stack component={RouterLink} href={`/sofa20/promotions/${s.slug}`} spacing={2} sx={{ p: 4, height: 1, textDecoration: 'none', border: `1px solid ${varAlpha(SOFA20_COLORS.gold, 0.3)}`, bgcolor: SOFA20_COLORS.charcoal, '&:hover': { border: `1px solid ${SOFA20_COLORS.gold}` }, transition: 'all 0.25s' }}>
