...
export default function Page() {
  const total = SOFA20_CART_ITEMS.reduce((sum, i) => sum + i.price * i.qty, 0);

  return (
    <>
      <Helmet><title>Thanh toán — Sofa20</title></Helmet>

      <Sofa20PageHero overline="THANH TOÁN" title={<>THANH <span>TOÁN</span></>} subtitle="Hoàn tất thông tin giao hàng và phương thức thanh toán." image={SOFA20_PAGE_IMAGES.product8} />

      <Sofa20Section bg="black">
        <Grid container spacing={4}>
          <Grid xs={12} md={7}>
            <Stack spacing={4}>
              <Sofa20Panel title="Thông tin giao hàng">
                <Stack spacing={2.5}>
                  ...
                  <TextField fullWidth label="Địa chỉ giao hàng" sx={sofa20FieldSx} />
                  <TextField fullWidth multiline rows={3} label="Ghi chú" sx={sofa20FieldSx} />
                </Stack>
              </Sofa20Panel>

              <Sofa20Panel title="Phương thức thanh toán">
                <Stack spacing={1.5}>
                  {PAYMENTS.map((p) => (
                    <Stack key={p} sx={{ p: 2, border: `1px solid ${varAlpha(SOFA20_COLORS.gold, 0.25)}` }}>
              ...
              <Stack spacing={1.5}>
                {SOFA20_CART_ITEMS.map((item) => (
                  <Stack key={item.id} direction="row" justifyContent="space-between">
                    ...
                    <Typography variant="body2" sx={{ color: 'common.white' }}>{sofa20Price(item.price * item.qty)}</Typography>
                  </Stack>
                ))}
                <Divider sx={{ borderColor: varAlpha(SOFA20_COLORS.gold, 0.2) }} />
                <Stack direction="row" justifyContent="space-between">
                  <Typography sx={{ color: 'common.white' }}>Tổng cộng</Typography>
                  <Typography sx={{ color: SOFA20_COLORS.gold, fontWeight: 'fontWeightBold' }}>{sofa20Price(total)}</Typography>
                </Stack>
                <Button component={RouterLink} href="/sofa20/checkout/success" size="large" variant="contained" sx={sofa20GoldButtonSx}>Đặt hàng</Button>
              </Stack>
            </Sofa20Panel>
          </Grid>
        </Grid>
