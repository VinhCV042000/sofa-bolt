...
export default function Page() {
  ...
  return (
    <>
      <Helmet><title>Sản phẩm — Sofa20</title></Helmet>
      <Sofa20PageHero overline="SẢN PHẨM" title={<>SOFA20 <span>TÁC PHẨM</span></>} subtitle="Da thật, dệt tay souk, burgundy. Mỗi tác phẩm là một tuyệt tác Riad." image={SOFA20_PAGE_IMAGES.prod1} />

      <Sofa20Section bg="black">
        <Grid container spacing={4}>
          <Grid xs={12} md={3}>
            <Stack spacing={4} component={m.div} variants={varFade({ distance: 24 }).inLeft} sx={{ position: 'sticky', top: 80 }}>
              {Object.entries(SOFA20_PAGE_PRODUCT_CATEGORIES).map(([key, items]) => (
                <Stack key={key} spacing={1.5}>
                  <Typography variant="overline" sx={{ color: varAlpha(SOFA20_COLORS.gold, 0.6), letterSpacing: 2, fontWeight: 'fontWeightMedium' }}>
                    {key === 'types' ? 'LOẠI SOFA' : key === 'styles' ? 'PHONG CÁCH' : key === 'spaces' ? 'KHÔNG GIAN' : key === 'sizes' ? 'KÍCH THƯỚC' : 'GIÁ'}
                  </Typography>
                  {items.map((item: any) => (
                    <Link component={RouterLink} key={item.slug} href={`/sofa20/products/category/${item.slug}`} sx={{ textDecoration: 'none', '&:hover .MuiTypography-root': { color: SOFA20_COLORS.gold } }}>
                  ...
                  <Stack component={RouterLink} href={`/sofa20/products/${product.slug}`} spacing={2} sx={{ textDecoration: 'none', '&:hover .product-img': { transform: 'scale(1.05)' }, '&:hover .product-name': { color: SOFA20_COLORS.gold } }}>
                    ...
                    <Stack spacing={0.5}>
                      ...
                      <Stack direction="row" spacing={1} alignItems="center">
                        ...
                        {product.oldPrice && <Typography variant="body2" sx={{ color: varAlpha('#FFFFFF', 0.2), textDecoration: 'line-through' }}>{sofa20FormatPrice(product.oldPrice)}</Typography>}
                      </Stack>
                    </Stack>
                  </Stack>
                </Grid>
