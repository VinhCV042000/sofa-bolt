...
export default function Page() {
  ...
  const allCategories = [
    ...SOFA8_PAGE_PRODUCT_CATEGORIES.types,
    ...SOFA8_PAGE_PRODUCT_CATEGORIES.styles,
    ...SOFA8_PAGE_PRODUCT_CATEGORIES.spaces,
    ...SOFA8_PAGE_PRODUCT_CATEGORIES.sizes,
    ...SOFA8_PAGE_PRODUCT_CATEGORIES.prices,
  ];
  const category = allCategories.find((c) => c.slug === slug);
  const label = category?.label || 'Tất cả sản phẩm';

  const filtered = SOFA8_PAGE_PRODUCTS.filter((p) => p.category === slug || p.style === slug);

  return (
    <>
      <Helmet>
        <title>{label} — Sofa8</title>
      </Helmet>
      ...
      <Sofa8Section>
        ...
        {filtered.length === 0 ? (
          <Stack spacing={2} alignItems="center" sx={{ py: 10, textAlign: 'center' }}>
            ...
            <Typography variant="body2" sx={{ color: 'text.disabled' }}>Hãy thử danh mục khác hoặc xem tất cả sản phẩm</Typography>
          </Stack>
        ) : (
