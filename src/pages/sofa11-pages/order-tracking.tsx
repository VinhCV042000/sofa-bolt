...
export default function Page() {
  ...
  return (
    <>
      <Helmet>
        <title>Theo dõi đơn hàng — Sofa11</title>
      </Helmet>

      <Sofa11PageHero overline="THEO DÕI" title={<>THEO DÕI <span>ĐƠN HÀNG</span></>} subtitle="Tra cứu trạng thái đơn hàng của bạn." image={SOFA11_PAGE_IMAGES.cta} />
      ...
      <Sofa11Section>
        <Stack spacing={4} sx={{ maxWidth: 600, mx: 'auto' }} component={m.div} variants={varFade({ distance: 24 }).inUp}>
          ...
          {tracking && (
            <Stack spacing={3} sx={{ p: 3, borderRadius: 3, bgcolor: 'background.paper', border: `1px solid ${SOFA11_COLORS.cyan}` }}>
              ...
              <Stack spacing={2}>
                {['Đặt hàng', 'Xác nhận', 'Đang sản xuất', 'Đang giao', 'Giao thành công'].map((step, i) => (
                  <Stack key={i} direction="row" spacing={2} alignItems="center">
                    <Box sx={{ width: 32, height: 32, borderRadius: '50%', bgcolor: i < 3 ? SOFA11_COLORS.cyan : 'grey.300', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
