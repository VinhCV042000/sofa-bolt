...
export default function Page() {
  ...
  return (
    <>
      <Helmet>
        <title>Tài khoản — Sofa11</title>
      </Helmet>

      <Sofa11PageHero overline="TÀI KHOẢN" title={<>TÀI <span>KHOẢN</span></>} subtitle="Quản lý thông tin cá nhân, đơn hàng, bảo hành." image={SOFA11_PAGE_IMAGES.contact} />
          ...
          <Grid xs={12} md={3}>
            <Stack spacing={1} component={m.div} variants={varFade({ distance: 24 }).inLeft} sx={{ position: 'sticky', top: 80 }}>
              {ACCOUNT_SECTIONS.map((s) => (
                <Stack key={s.slug} component={RouterLink} href={`/sofa11/account/${s.slug === 'profile' ? '' : s.slug}`} direction="row" spacing={2} alignItems="center" sx={{ p: 2, borderRadius: 30, textDecoration: 'none', bgcolor: section === s.slug ? varAlpha(SOFA11_COLORS.magenta, 0.08) : 'transparent', '&:hover': { bgcolor: varAlpha(SOFA11_COLORS.magenta, 0.04) } }}>
            ...
            <Stack spacing={3} component={m.div} variants={varFade({ distance: 24 }).inRight}>
              {section === 'profile' || section === undefined ? (
                <>
                  ...
                  <Button variant="contained" sx={{ borderRadius: 30, alignSelf: 'flex-start', bgcolor: SOFA11_COLORS.magenta, color: 'common.white', '&:hover': { bgcolor: SOFA11_COLORS.magentaSoft } }}>Cập nhật</Button>
                </>
              ) : section === 'orders' ? (
                <>
                  ...
                  {[1, 2, 3].map((i) => (
                    <Stack key={i} spacing={2} sx={{ p: 3, borderRadius: 3, bgcolor: 'background.paper', border: `1px solid ${varAlpha(theme.vars.palette.grey['500Channel'], 0.12)}` }}>
                      <Stack direction="row" justifyContent="space-between">
                        ...
                        <Typography variant="caption" sx={{ color: SOFA11_COLORS.cyan, fontWeight: 'fontWeightBold' }}>Đã giao</Typography>
                      </Stack>
                      <Typography variant="body2" sx={{ color: 'text.secondary' }}>{formatSofa11Price(12000000 + i * 1000000)}</Typography>
                    </Stack>
                  ))}
                </>
              ) : section === 'wishlist' ? (
                <>
                  ...
                  <Grid container spacing={2}>
                    {SOFA11_PAGE_PRODUCTS.slice(0, 4).map((p) => (
                      <Grid key={p.id} xs={6} md={3}>
                        <Stack component={RouterLink} href={`/sofa11/products/${p.slug}`} spacing={1} sx={{ textDecoration: 'none' }}>
                          ...
                          <Typography variant="caption" sx={{ color: SOFA11_COLORS.magenta }}>{formatSofa11Price(p.price)}</Typography>
                        </Stack>
                      </Grid>
                    ))}
                  </Grid>
