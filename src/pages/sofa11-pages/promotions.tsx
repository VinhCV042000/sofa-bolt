...
export default function Page() {
  ...
  return (
    <>
      <Helmet>
        <title>Khuyến mãi — Sofa11</title>
      </Helmet>

      <Sofa11PageHero overline="KHUYẾN MÃI" title={<>KHUYẾN <span>MÃI</span></>} subtitle="Bảo hành, bảo trì, sửa chữa, vệ sinh, đổi trả — dịch vụ trọn đời." image={SOFA11_PAGE_IMAGES.cta} />

      <Sofa11Section>
        <Grid container spacing={3}>
          {SOFA11_PAGE_PROMOTION_SERVICES.map((s) => (
            <Grid key={s.slug} xs={12} sm={6} md={4}>
              <Stack component={RouterLink} href={`/sofa11/promotions/${s.slug}`} spacing={2} sx={{ p: 4, height: 1, borderRadius: 3, textDecoration: 'none', bgcolor: 'background.paper', border: `2px solid ${varAlpha(theme.vars.palette.grey['500Channel'], 0.12)}`, '&:hover': { borderColor: SOFA11_COLORS.magenta, transition: 'border-color 0.3s' } }}>
