...
const metadata = { title: 'Dịch vụ sản xuất - LUXE Sofa' };

const SERVICES = [
  { key: 'custom', icon: 'solar:hammer-bold-duotone', title: 'Gia công', description: 'Gia công sofa theo yêu cầu với độ chính xác cao và nguyên liệu chọn lọc.' },
  { key: 'oem', icon: 'solar:factory-bold-duotone', title: 'OEM', description: 'Sản xuất sofa theo thiết kế và thương hiệu của đối tác.' },
  { key: 'odm', icon: 'solar:lightbulb-bold-duotone', title: 'ODM', description: 'Thiết kế và sản xuất sản phẩm độc quyền cho đối tác.' },
  { key: 'project', icon: 'solar:buildings-bold-duotone', title: 'Đặt hàng dự án', description: 'Cung cấp sofa số lượng lớn cho dự án căn hộ, khách sạn, resort.' },
];

const CAPABILITIES = [
  { value: '200m²', label: 'Diện tích xưởng', icon: 'solar:factory-bold-duotone' },
  { value: '50+', label: 'Nghệ nhân lành nghề', icon: 'solar:users-group-rounded-bold-duotone' },
  { value: '10 năm', label: 'Bảo hành chính hãng', icon: 'solar:shield-check-bold-duotone' },
];

const QUALITY_STEPS = [
  { title: 'Chọn nguyên liệu', desc: 'Gỗ sồi FSC, da bò thật, vải linen nhập khẩu — kiểm định chất lượng đầu vào.' },
  ...
  return (
    <>
      ...
      <Sofa2PageHero
        ...
        subtitle="Xưởng sản xuất quy mô lớn với nghệ nhân lành nghề — biến ý tưởng thành sản phẩm chất lượng."
      ...
      <Sofa2Section bg="grey">
        <Stack spacing={2} sx={{ textAlign: 'center', maxWidth: 640, mx: 'auto', mb: { xs: 5, md: 8 } }}>
          ...
          <Typography variant="h2">Cơ sở sản xuất</Typography>
        </Stack>
        <Grid container spacing={3}>
      ...
      <Sofa2Section>
        <Stack spacing={2} sx={{ mb: { xs: 5, md: 8 } }}>
          ...
          <Typography variant="h2">Quy trình sản xuất</Typography>
        </Stack>
        <Grid container spacing={3}>
      ...
      <Sofa2Section bg="grey">
        <Stack component={MotionViewport} spacing={3} alignItems="center" sx={{ textAlign: 'center', maxWidth: 640, mx: 'auto' }}>
          ...
          <Typography sx={{ color: 'text.secondary' }}>Liên hệ để nhận báo giá dự án và hợp tác OEM/ODM từ LUXE Sofa.</Typography>
