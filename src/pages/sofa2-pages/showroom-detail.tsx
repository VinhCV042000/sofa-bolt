...
export default function Page() {
  ...
  const showroom = SOFA2_SHOWROOMS.find((s) => s.id === id);

  if (!showroom) {
    return (
      <>
        <Helmet><title>Không tìm thấy showroom - LUXE Sofa</title></Helmet>
        <Stack spacing={3} alignItems="center" sx={{ textAlign: 'center', py: 20 }}>
          ...
          <Button component={RouterLink} href="/sofa2/showrooms" variant="contained">Quay lại danh sách</Button>
        </Stack>
      </>
    );
  }
  ...
  return (
    <>
      <Helmet><title>{showroom.name} - LUXE Sofa</title></Helmet>

      <Sofa2PageHero overline="Showroom" title={showroom.name} subtitle={showroom.address} image={showroom.image} />
      ...
      <Box component="section" sx={{ py: { xs: 8, md: 12 } }}>
        <Container component={MotionViewport}>
          <Grid container spacing={{ xs: 5, md: 8 }}>
            <Grid xs={12} md={6}>
              <Box component={m.img} variants={varFade({ distance: 24 }).inLeft} src={showroom.image} alt={showroom.name} sx={{ width: 1, borderRadius: 2, aspectRatio: '4/3', objectFit: 'cover' }} />
            </Grid>
            <Grid xs={12} md={6}>
              <Stack spacing={3} component={m.div} variants={varFade({ distance: 24 }).inRight}>
                <Typography variant="overline" sx={{ color: 'text.disabled' }}>Thông tin showroom</Typography>
                ...
                <Stack spacing={2}>
                  ...
                  <Stack direction="row" spacing={1.5} alignItems="center">
                    ...
                    <Box>
                      <Typography variant="caption" sx={{ color: 'text.disabled' }}>Giờ mở cửa</Typography>
                      <Typography>{showroom.hours}</Typography>
                    </Box>
                  </Stack>
                </Stack>
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ mt: 2 }}>
                  <Button component={RouterLink} href="/sofa2/showrooms/visit" variant="contained" size="large" startIcon={<Iconify icon="solar:calendar-bold-duotone" />}>
                    Đặt lịch tham quan
                  </Button>
                  <Button component={RouterLink} href="/sofa2/showrooms/consult" variant="outlined" size="large" startIcon={<Iconify icon="solar:chat-square-bold-duotone" />}>
                    Đặt lịch tư vấn
                  </Button>
                </Stack>
              </Stack>
      ...
      <Box component="section" sx={{ py: { xs: 8, md: 12 }, bgcolor: (t) => varAlpha(t.vars.palette.grey['500Channel'], 0.04) }}>
        <Container component={MotionViewport}>
          <Stack spacing={2} sx={{ mb: 4 }}>
            <Typography variant="overline" sx={{ color: 'text.disabled' }}>Vị trí</Typography>
            <Typography variant="h3">Bản đồ đến showroom</Typography>
          </Stack>
          <Box component={m.div} variants={varFade({ distance: 24 }).inUp} sx={{ height: 360, borderRadius: 3, bgcolor: (t) => varAlpha(t.vars.palette.grey['500Channel'], 0.16), display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <Box component="section" sx={{ py: { xs: 8, md: 12 } }}>
        <Container component={MotionViewport}>
          <Stack spacing={2} sx={{ mb: 4 }}>
            <Typography variant="overline" sx={{ color: 'text.disabled' }}>Không gian</Typography>
            <Typography variant="h3">Gallery showroom</Typography>
          </Stack>
          <Grid container spacing={2}>
            {GALLERY.map((src, idx) => (
              <Grid key={idx} xs={6} sm={3}>
                <Box component={m.img} variants={varFade({ distance: 24 }).inUp} src={src} alt={`${showroom.name} ${idx + 1}`} sx={{ width: 1, borderRadius: 2, aspectRatio: '1/1', objectFit: 'cover' }} />
              </Grid>
            ))}
          </Grid>
        </Container>
