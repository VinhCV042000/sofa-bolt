...
export default function Page() {
  ...
  const matchLabel = FILTER_GROUPS.reduce<string | null>((result, group) => {
    if (result) return result;
    const found = SOFA2_PRODUCT_CATEGORIES[group.key].find((c) => c.slug === slug);
    return found ? found.label : null;
  }, null);

  // filter products by category or style matching slug
  const filtered = slug
    ? SOFA2_PRODUCTS.filter((p) => p.category === slug || p.style === slug)
    : SOFA2_PRODUCTS;

  const products = filtered.length > 0 ? filtered : SOFA2_PRODUCTS;
  const label = matchLabel ?? 'Tất cả sản phẩm';

  return (
    <>
      <Helmet>
        <title>{label} - LUXE Sofa</title>
      </Helmet>
      ...
      <Sofa2PageHero
        ...
        image={products[0]?.image ?? 'https://images.pexels.com/photos/276624/pexels-photo-276624.jpeg?auto=compress&cs=tinysrgb&w=1920'}
      />
              ...
              <Stack spacing={3} sx={{ position: { md: 'sticky' }, top: { md: 88 }, p: 3, borderRadius: 2, bgcolor: 'background.paper', boxShadow: (t) => t.customShadows.card }}>
                ...
                {FILTER_GROUPS.map((group) => (
                  <Stack key={group.key} spacing={1}>
                    ...
                    <Stack spacing={0.5}>
                      {SOFA2_PRODUCT_CATEGORIES[group.key].map((item) => (
                        <Button
                          key={item.slug}
                          component={RouterLink}
                          href={`/sofa2/products/category/${item.slug}`}
                          size="small"
                    ...
                    >
                      ...
                      <Stack spacing={1.5} sx={{ p: 2.5 }}>
                        ...
                        <Stack direction="row" alignItems="center" spacing={1}>
                          ...
                          {product.oldPrice && (
                            <Typography variant="body2" sx={{ color: 'text.disabled', textDecoration: 'line-through' }}>{formatSofa2Price(product.oldPrice)}</Typography>
                          )}
                        </Stack>
                        <Button component={RouterLink} href={`/sofa2/products/${product.id}`} variant="outlined" size="small" startIcon={<Iconify icon="solar:eye-bold-duotone" />}>
                          Xem chi tiết
                        </Button>
                      </Stack>
                    </Stack>
