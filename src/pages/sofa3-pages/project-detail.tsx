...
import { SOFA3_PROJECTS, SOFA3_PRODUCTS, SOFA3_PAGE_IMAGES } from 'src/sections/sofa3-pages/sofa3-pages-data';

const metadata = { title: 'Chi tiết dự án - Sofa Terra' };

export default function Page() {
  const { id } = useParams();
  const project = SOFA3_PROJECTS.find((p) => p.id === id) || SOFA3_PROJECTS[0];
  const usedProducts = SOFA3_PRODUCTS.slice(0, 3);
  ...
  return (
    <>
      ...
      <Sofa3PageHero overline="Dự án" title={project.name} subtitle={project.description} image={project.image} />
      ...
      <Sofa3Section>
        <Stack spacing={4} component={MotionViewport}>
          <Grid container spacing={{ xs: 4, md: 6 }}>
            <Grid xs={12} md={8}>
              ...
              <Grid container spacing={2} sx={{ mt: 2 }}>
                {[SOFA3_PAGE_IMAGES.project2, SOFA3_PAGE_IMAGES.project3, SOFA3_PAGE_IMAGES.project4].map((img, i) => (
                  <Grid key={i} xs={4}>
            ...
            <Grid xs={12} md={4}>
              <Stack spacing={3} component={m.div} variants={varFade({ distance: 24 }).inRight}>
                ...
                <Stack spacing={1.5}>
                  <Stack direction="row" spacing={1} alignItems="center"><Iconify icon="solar:map-point-bold-duotone" width={20} sx={{ color: 'success.main' }} /><Typography variant="body2" sx={{ color: 'text.secondary' }}>{project.location}</Typography></Stack>
                  <Stack direction="row" spacing={1} alignItems="center"><Iconify icon="solar:calendar-bold-duotone" width={20} sx={{ color: 'success.main' }} /><Typography variant="body2" sx={{ color: 'text.secondary' }}>{project.year}</Typography></Stack>
                  <Stack direction="row" spacing={1} alignItems="center"><Iconify icon="solar:buildings-bold-duotone" width={20} sx={{ color: 'success.main' }} /><Typography variant="body2" sx={{ color: 'text.secondary' }}>{project.typeLabel}</Typography></Stack>
                </Stack>
          ...
          <Box>
            ...
            <Stack spacing={2}>
              {[{ name: 'Mai Thị Hồng', rating: 5, content: '&ldquo;Sofa Terra hoàn toàn phù hợp với dự án xanh của chúng tôi. Khách hàng khen không gian tự nhiên, ấm áp.&rdquo;' }].map((r) => (
