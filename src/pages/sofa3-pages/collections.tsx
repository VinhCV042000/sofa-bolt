...
import { SOFA3_COLLECTIONS, SOFA3_PAGE_IMAGES } from 'src/sections/sofa3-pages/sofa3-pages-data';

const metadata = { title: 'Bộ sưu tập - Sofa Terra' };
...
export default function Page() {
  return (
    <>
      ...
      <Sofa3PageHero overline="Bộ sưu tập" title="Các bộ sưu tập sofa eco" subtitle="Khám phá 5 bộ sưu tập sofa eco-living — từ mới nhất đến bestseller." image={SOFA3_PAGE_IMAGES.aboutHero} />

      <Box component="section" sx={{ py: { xs: 8, md: 12 } }}>
        <Container component={MotionViewport}>
          <Grid container spacing={4}>
            {SOFA3_COLLECTIONS.map((col, index) => (
              <Grid key={col.id} xs={12} sm={6}>
                <Box
                  component={RouterLink}
                  href={`/sofa3/collections/${col.slug}`}
                  sx={{
                    position: 'relative',
                    overflow: 'hidden',
                    borderRadius: 3,
                ...
                >
                  ...
                  <Stack spacing={1} sx={{ position: 'absolute', bottom: 0, left: 0, right: 0, p: 4, color: 'common.white' }}>
                    <Typography variant="overline" sx={{ color: 'success.light' }}>{col.count}</Typography>
