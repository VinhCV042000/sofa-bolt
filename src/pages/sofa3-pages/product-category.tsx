...
const metadata = { title: 'Danh mục sản phẩm - Sofa Terra' };

export default function Page() {
  const { slug } = useParams();

  let categoryLabel = 'Tất cả sản phẩm';
  Object.values(SOFA3_PRODUCT_CATEGORIES).forEach((group) => {
    const found = group.find((item) => item.slug === slug);
    if (found) categoryLabel = found.label;
  });

  const filtered = slug ? SOFA3_PRODUCTS.filter((p) => p.category === slug || p.style === slug) : SOFA3_PRODUCTS;
  ...
  return (
    <>
      ...
      <Sofa3PageHero overline="Danh mục" title={categoryLabel} subtitle={`${filtered.length} mẫu sofa trong danh mục`} image={SOFA3_PAGE_IMAGES.aboutHero} />
        ...
        <Container component={MotionViewport}>
          {filtered.length === 0 ? (
            <Stack spacing={2} alignItems="center" sx={{ py: 8, textAlign: 'center' }}>
              ...
              <Button component={RouterLink} href="/sofa3/products" variant="outlined" color="success">Xem tất cả sản phẩm</Button>
            </Stack>
          ) : (
