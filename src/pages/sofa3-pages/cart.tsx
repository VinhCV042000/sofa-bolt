...
import { SOFA3_PRODUCTS, SOFA3_PAGE_IMAGES, formatSofa3Price } from 'src/sections/sofa3-pages/sofa3-pages-data';

const metadata = { title: 'Giỏ hàng - Sofa Terra' };

const CART_ITEMS = SOFA3_PRODUCTS.slice(0, 2);
...
export default function Page() {
  ...
  return (
    <>
      ...
      <Sofa3PageHero overline="Giỏ hàng" title="Giỏ hàng của bạn" subtitle={`${CART_ITEMS.length} sản phẩm trong giỏ`} image={SOFA3_PAGE_IMAGES.cta} />
        ...
        <Container component={MotionViewport}>
          <Stack spacing={3}>
            ...
            <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ p: 3, borderRadius: 2, bgcolor: 'background.paper', border: (t) => `1px solid ${varAlpha(t.vars.palette.grey['500Channel'], 0.12)}` }}>
              <Typography variant="h5">Tổng cộng: <Typography component="span" variant="h4" sx={{ color: 'success.main', fontWeight: 'fontWeightBold' }}>{formatSofa3Price(total)}</Typography></Typography>
              <Button variant="contained" color="success" size="large" component={RouterLink} href="/sofa3/checkout" startIcon={<Iconify icon="solar:cart-bold-duotone" />}>Thanh toán</Button>
            </Stack>
          </Stack>
        </Container>
      </Box>
