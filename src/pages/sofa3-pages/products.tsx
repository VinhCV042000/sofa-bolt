...
import { SOFA3_PRODUCTS, formatSofa3Price, SOFA3_PAGE_IMAGES, SOFA3_PRODUCT_CATEGORIES } from 'src/sections/sofa3-pages/sofa3-pages-data';

const metadata = { title: 'Sản phẩm - Sofa Terra' };
...
export default function Page() {
  return (
    <>
      ...
      <Sofa3PageHero overline="Sản phẩm" title="Bộ sưu tập sofa eco-living" subtitle="Khám phá hơn 60 mẫu sofa thiết kế eco — từ linen hữu cơ đến gỗ tre bền vững." image={SOFA3_PAGE_IMAGES.aboutHero} />
              ...
              <Stack spacing={3} sx={{ position: { md: 'sticky' }, top: { md: 88 }, p: 3, borderRadius: 2, bgcolor: 'background.paper', boxShadow: (t) => t.customShadows.card }}>
                ...
                {FILTER_GROUPS.map((group) => (
                  <Stack key={group.key} spacing={1}>
                    ...
                    <Stack spacing={0.5}>
                      {SOFA3_PRODUCT_CATEGORIES[group.key].map((item) => (
                        <Button key={item.slug} component={RouterLink} href={`/sofa3/products/category/${item.slug}`} size="small" sx={{ justifyContent: 'flex-start', color: 'text.secondary', textTransform: 'none', fontWeight: 'fontWeightRegular', '&:hover': { color: 'success.main', bgcolor: (t) => varAlpha(t.vars.palette.success.mainChannel, 0.08) } }}>
                          {item.label}
                        </Button>
                      ))}
                    </Stack>
                    ...
                    <Stack component={m.div} variants={varFade({ distance: 24 }).inUp} sx={{ height: 1, borderRadius: 2, overflow: 'hidden', bgcolor: 'background.paper', boxShadow: (t) => t.customShadows.card }}>
                      ...
                      <Stack spacing={1.5} sx={{ p: 2.5 }}>
                        ...
                        <Stack direction="row" alignItems="center" spacing={1}>
                          ...
                          {product.oldPrice && <Typography variant="body2" sx={{ color: 'text.disabled', textDecoration: 'line-through' }}>{formatSofa3Price(product.oldPrice)}</Typography>}
                        </Stack>
                        <Button component={RouterLink} href={`/sofa3/products/${product.id}`} variant="outlined" size="small" color="success" startIcon={<Iconify icon="solar:eye-bold-duotone" />}>
                          Xem chi tiết
                        </Button>
                      </Stack>
                    </Stack>
