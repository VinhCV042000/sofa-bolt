...
export default function Page() {
  ...
  const cartItems = SOFA13_PAGE_PRODUCTS.slice(0, 2);
  ...
  return (
    <>
      ...
      <Sofa13PageHero overline="THANH TOÁN" title={<>THANH <span>TOÁN</span></>} subtitle="Hoàn tất đơn hàng retro của bạn." image={SOFA13_PAGE_IMAGES.product7} />
                ...
                <Stack spacing={2}>
                  {['Chuyển khoản ngân hàng', 'Thanh toán khi nhận hàng (COD)', 'Thanh toán online (VNPay/Momo)'].map((method, i) => (
                    <Stack key={i} direction="row" spacing={2} alignItems="center" sx={{ p: 2, borderRadius: 3, border: `1px solid ${varAlpha(theme.vars.palette.grey['500Channel'], 0.12)}` }}>
                      <Iconify icon={i === 0 ? 'solar:card-bold-duotone' : i === 1 ? 'solar:delivery-bold-duotone' : 'solar:wallet-bold-duotone'} width={24} sx={{ color: SOFA13_COLORS.gold }} />
            ...
            <Stack spacing={3} sx={{ p: 3, borderRadius: 3, bgcolor: 'background.paper', border: `1px solid ${varAlpha(theme.vars.palette.grey['500Channel'], 0.12)}` }}>
              ...
              {cartItems.map((item) => (
                <Stack key={item.id} direction="row" spacing={2} alignItems="center">
                  ...
                  <Typography variant="body2" sx={{ color: SOFA13_COLORS.gold, fontWeight: 'fontWeightBold' }}>{formatSofa13Price(item.price)}</Typography>
                </Stack>
              ))}
              <Divider />
              ...
              <Button component={RouterLink} href="/sofa13/checkout/success" size="large" variant="contained" fullWidth startIcon={<Iconify icon="solar:check-circle-bold-duotone" />} sx={{ borderRadius: 30, bgcolor: SOFA13_COLORS.gold, color: 'common.white', fontWeight: 'fontWeightBold', boxShadow: `0 4px 0 ${SOFA13_COLORS.noir}`, '&:hover': { bgcolor: SOFA13_COLORS.goldPale, boxShadow: `0 6px 0 ${SOFA13_COLORS.noir}` } }}>Đặt hàng</Button>
            </Stack>
          </Grid>
        </Grid>
      </Sofa13Section>
