...
export default function Page() {
  ...
  return (
    <>
      ...
      <Sofa9PageHero overline="CHI TIẾT DỰ ÁN" title={<><span>{project.name.toUpperCase()}</span></>} subtitle={`${project.typeLabel} · ${project.location} · ${project.year}`} image={project.image} />
          ...
          <Grid xs={12} md={8}>
            <Stack spacing={4} component={m.div} variants={varFade({ distance: 24 }).inUp}>
              ...
              <Stack spacing={2}>
                ...
                <Typography sx={{ color: 'text.secondary', lineHeight: 1.7 }}>Dự án {project.name} là minh chứng cho chất lượng và thẩm mỹ retro của Sofa9. Với velvet neon, họa tiết bold, mỗi không gian đều mang lại cảm giác vui vẻ tuyệt đối.</Typography>
              </Stack>
              <Stack spacing={2}>
                <Typography variant="h5" sx={{ fontWeight: 'fontWeightBold' }}>Album ảnh</Typography>
                <Grid container spacing={2}>
                  {[SOFA9_PAGE_IMAGES.project1, SOFA9_PAGE_IMAGES.project2, SOFA9_PAGE_IMAGES.project3, SOFA9_PAGE_IMAGES.project4].map((img, i) => (
              ...
              <Stack spacing={2}>
                ...
                <Stack spacing={2} sx={{ p: 3, borderRadius: 3, bgcolor: 'background.paper', border: `1px solid ${varAlpha(theme.vars.palette.grey['500Channel'], 0.12)}` }}>
                  ...
                  <Typography sx={{ color: 'text.secondary', lineHeight: 1.7 }}>"Sofa9 đã biến không gian của chúng tôi thành một sân khấu retro thực sự. Velvet neon bùng nổ, họa tiết bold. Khách hàng đều chụp ảnh không ngừng!"</Typography>
            ...
            <Stack spacing={3} component={m.div} variants={varFade({ distance: 24 }).inRight}>
              ...
              <Stack spacing={2}>
                ...
                {SOFA9_PAGE_PRODUCTS.slice(0, 3).map((p) => (
                  <Stack key={p.id} component={RouterLink} href={`/sofa9/products/${p.slug}`} direction="row" spacing={2} sx={{ textDecoration: 'none', '&:hover .p-name': { color: SOFA9_COLORS.pink } }}>
                    ...
                    <Stack spacing={0.5} justifyContent="center">
                      ...
                      <Typography variant="caption" sx={{ color: SOFA9_COLORS.pink, fontWeight: 'fontWeightBold' }}>{formatSofa9Price(p.price)}</Typography>
                    </Stack>
                  </Stack>
                ))}
              </Stack>
              <Button component={RouterLink} href="/sofa9/showrooms/visit" size="large" variant="contained" fullWidth startIcon={<Iconify icon="solar:phone-bold-duotone" />} sx={{ borderRadius: 30, bgcolor: SOFA9_COLORS.pink, color: 'common.white', fontWeight: 'fontWeightBold', boxShadow: `0 4px 0 ${SOFA9_COLORS.black}`, '&:hover': { bgcolor: SOFA9_COLORS.pinkLight, boxShadow: `0 6px 0 ${SOFA9_COLORS.black}` } }}>Tư vấn dự án tương tự</Button>
            </Stack>
          </Grid>
        </Grid>
      </Sofa9Section>
