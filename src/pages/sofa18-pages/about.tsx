...
import { SOFA18_PAGE_IMAGES, SOFA18_PAGE_COMPANY_INFO } from 'src/sections/sofa18-pages/sofa18-pages-data';
...
export default function Page() {
  ...
  return (
    <>
      ...
      <Sofa18PageHero overline="GIỚI THIỆU" title={<>GIỚI <span>THIỆU</span></>} subtitle="Muji-An — xưởng sofa wabi-sabi Kyoto từ 1972. Tôn vinh sự không hoàn hảo, thủ công bào tay." image={SOFA18_PAGE_IMAGES.aboutHero} />

      <Sofa18Section>
        <Grid container spacing={4}>
          <Grid xs={12} md={6}>
            <Box sx={{ overflow: 'hidden', borderRadius: 3, aspectRatio: '4/3' }} component={m.div} variants={varFade({ distance: 24 }).inLeft}>
              <Box component="img" src={SOFA18_PAGE_IMAGES.workshop} alt="Xưởng Kyoto" sx={{ width: 1, height: 1, objectFit: 'cover' }} />
            </Box>
          </Grid>
          <Grid xs={12} md={6}>
            <Stack spacing={3} component={m.div} variants={varFade({ distance: 24 }).inRight}>
              <Typography variant="h3" sx={{ fontWeight: 'fontWeightBold', color: SOFA18_COLORS.charcoal }}>{SOFA18_PAGE_COMPANY_INFO.name}</Typography>
              <Typography sx={{ color: 'text.secondary', lineHeight: 1.8 }}>{SOFA18_PAGE_COMPANY_INFO.tagline}</Typography>
              <Grid container spacing={2}>
                {SOFA18_PAGE_COMPANY_INFO.stats.map((stat) => (
                  <Grid key={stat.label} xs={6} sm={3}>
                    <Stack spacing={1} sx={{ p: 2, borderRadius: 2, bgcolor: 'background.paper', border: `1px solid ${varAlpha(theme.vars.palette.grey['500Channel'], 0.12)}` }}>
                      <Typography variant="h5" sx={{ fontWeight: 'fontWeightBold', color: SOFA18_COLORS.sageDeep }}>{stat.value}</Typography>
          ...
          {ABOUT_ITEMS.map((item, i) => (
            <Grid key={i} xs={12} sm={6} md={4}>
              <Stack spacing={2} sx={{ p: 3, height: 1, borderRadius: 3, bgcolor: 'background.default', border: `1px solid ${varAlpha(theme.vars.palette.grey['500Channel'], 0.12)}` }} component={m.div} variants={varFade({ distance: 24 }).inUp}>
                <Typography variant="h6" sx={{ fontWeight: 'fontWeightBold', color: SOFA18_COLORS.charcoal }}>{item.heading}</Typography>
