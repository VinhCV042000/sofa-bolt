...
export default function Page() {
  ...
  return (
    <>
      <Helmet><title>Blog — Sofa18</title></Helmet>
      <Sofa18PageHero overline="BLOG" title={<>B<span>LOG</span></>} subtitle="Tin tức, kiến thức, triết lý Mediterranean và nội thất Riviera." image={SOFA18_PAGE_IMAGES.blog1} />

      <Sofa18Section bg="cream">
        <Grid container spacing={4}>
          <Grid xs={12} md={3}>
            <Stack spacing={2} component={m.div} variants={varFade({ distance: 24 }).inLeft} sx={{ position: 'sticky', top: 80 }}>
              <Typography variant="overline" sx={{ color: SOFA18_COLORS.clayDeep, letterSpacing: 2, fontWeight: 'fontWeightMedium' }}>CHUYÊN MỤC</Typography>
              {SOFA18_PAGE_BLOG_CATEGORIES.map((cat) => (
                <Link component={RouterLink} key={cat.slug} href={`/sofa18/blog/category/${cat.slug}`} sx={{ textDecoration: 'none', '&:hover .MuiTypography-root': { color: SOFA18_COLORS.clay } }}>
                  <Typography variant="body2" sx={{ color: SOFA18_COLORS.inkSoft, transition: 'color 0.2s', fontWeight: 'fontWeightLight' }}>{cat.label}</Typography>
                </Link>
              ))}
            </Stack>
          </Grid>
          <Grid xs={12} md={9}>
            <Grid container spacing={4}>
              {SOFA18_PAGE_BLOG_POSTS.map((post) => (
                <Grid key={post.id} xs={12} sm={6}>
                  <Stack component={RouterLink} href={`/sofa18/blog/${post.slug}`} spacing={2} sx={{ textDecoration: 'none', '&:hover .blog-img': { transform: 'scale(1.05)' }, '&:hover .blog-title': { color: SOFA18_COLORS.clay } }}>
                    ...
                    <Stack direction="row" spacing={2} alignItems="center">
                      <Typography variant="caption" sx={{ color: SOFA18_COLORS.inkSoft }}>{post.author}</Typography>
                      <Typography variant="caption" sx={{ color: SOFA18_COLORS.inkSoft, opacity: 0.5 }}>·</Typography>
                      <Typography variant="caption" sx={{ color: SOFA18_COLORS.inkSoft }}>{post.date}</Typography>
                      <Typography variant="caption" sx={{ color: SOFA18_COLORS.inkSoft, opacity: 0.5 }}>·</Typography>
                      <Typography variant="caption" sx={{ color: SOFA18_COLORS.inkSoft }}>{post.readTime}</Typography>
                    </Stack>
                  </Stack>
                </Grid>
              ))}
