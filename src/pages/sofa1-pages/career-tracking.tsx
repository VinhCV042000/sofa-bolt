...
const metadata = { title: 'Theo dõi hồ sơ - Casa Sofa' };
...
export default function Page() {
  ...
  const handleSearch = () => {
    if (email.trim() || code.trim()) setSearched(true);
  };
  ...
  return (
    <>
      ...
      <Sofa1PageHero
        ...
        subtitle="Kiểm tra tình trạng ứng tuyển của bạn bằng email và mã ứng tuyển."
          ...
          <Grid xs={12} md={8}>
            ...
            >
              <Stack spacing={1}>
                ...
                <Typography sx={{ color: 'text.secondary' }}>Nhập email và mã ứng tuyển bạn đã nhận khi nộp hồ sơ.</Typography>
              </Stack>
              <Stack spacing={1.5}>
                <TextField fullWidth label="Email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} />
                <TextField fullWidth label="Mã ứng tuyển" value={code} onChange={(e) => setCode(e.target.value)} />
                <Button onClick={handleSearch} variant="contained" size="large" sx={{ width: 'fit-content' }} startIcon={<Iconify icon="solar:search-bold-duotone" />}>
                  Tra cứu
                </Button>
              </Stack>
                ...
                <Stack spacing={3} sx={{ mt: 2 }}>
                  <Stack direction="row" spacing={1.5} alignItems="center" sx={{ p: 2, borderRadius: 1.5, bgcolor: (t) => varAlpha(t.vars.palette.primary.mainChannel, 0.04) }}>
                    ...
                    <Box>
                      ...
                      <Typography variant="caption" sx={{ color: 'text.secondary' }}>Mã hồ sơ: CASA-2025-0142</Typography>
                    </Box>
                  </Stack>
            ...
            >
              ...
              <Stack spacing={2}>
                <Stack direction="row" spacing={1.5} alignItems="center">
                  ...
                  <Typography variant="body2">{SOFA1_COMPANY_INFO.phone}</Typography>
                </Stack>
                <Stack direction="row" spacing={1.5} alignItems="center">
                  <Iconify icon="solar:letter-bold-duotone" width={20} sx={{ color: 'primary.main' }} />
                  <Typography variant="body2">{SOFA1_COMPANY_INFO.email}</Typography>
                </Stack>
                <Stack direction="row" spacing={1.5} alignItems="flex-start">
                  <Iconify icon="solar:map-point-bold-duotone" width={20} sx={{ color: 'primary.main', mt: 0.25 }} />
                  <Typography variant="body2">{SOFA1_COMPANY_INFO.address}</Typography>
                </Stack>
              </Stack>
              <Box sx={{ flex: 1 }} />
