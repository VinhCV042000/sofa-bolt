...
export default function Page() {
  ...
  const product = SOFA1_PRODUCTS.find((p) => p.id === id);

  if (!product) {
    return (
      <>
        <Helmet><title>Không tìm thấy sản phẩm - Casa Sofa</title></Helmet>
        <Stack spacing={3} alignItems="center" sx={{ textAlign: 'center', py: 20 }}>
          ...
          <Button component={RouterLink} href="/sofa1/products" variant="contained">Quay lại danh sách</Button>
        </Stack>
      </>
    );
  }

  const related = SOFA1_PRODUCTS.filter((p) => p.id !== product.id).slice(0, 4);
  const images = product.images?.length ? product.images : [product.image];

  return (
    <>
      <Helmet><title>{product.name} - Casa Sofa</title></Helmet>

      <Sofa1PageHero overline="Chi tiết sản phẩm" title={product.name} subtitle={product.description} image={product.image} />
            ...
            <Grid xs={12} md={6}>
              <Stack spacing={3} component={m.div} variants={varFade({ distance: 24 }).inRight}>
                ...
                <Stack direction="row" alignItems="center" spacing={2}>
                  <Typography variant="h4" sx={{ color: 'primary.main', fontWeight: 'fontWeightBold' }}>{formatSofa1Price(product.price)}</Typography>
                  {product.oldPrice && (
                    <Typography variant="h6" sx={{ color: 'text.disabled', textDecoration: 'line-through' }}>{formatSofa1Price(product.oldPrice)}</Typography>
                  )}
                </Stack>
                ...
                <Stack direction="row" spacing={2}>
                  ...
                  <Button component={RouterLink} href="/sofa1/showrooms" size="large" variant="outlined" startIcon={<Iconify icon="solar:phone-bold-duotone" />}>
                    Tư vấn miễn phí
                  </Button>
                </Stack>
              </Stack>
          ...
          <Stack spacing={4}>
            ...
            {tab === 'info' && (
              <Stack spacing={2} component={m.div} variants={varFade({ distance: 24 }).inUp}>
                ...
                <Typography sx={{ color: 'text.secondary' }}>{product.description}</Typography>
                <Typography sx={{ color: 'text.secondary' }}>Sofa {product.name} được chế tác thủ công bởi nghệ nhân Casa Sofa, đảm bảo chất lượng cao và độ bền vượt thời gian. Sản phẩm đi kèm bảo hành chính hãng 7 năm.</Typography>
              </Stack>
            )}
            ...
            {tab === 'care' && (
              <Stack spacing={2} component={m.div} variants={varFade({ distance: 24 }).inUp}>
                ...
                <Typography sx={{ color: 'text.secondary' }}>• Liên hệ dịch vụ bảo trì của Casa Sofa trong năm đầu.</Typography>
              </Stack>
            )}
          </Stack>
        </Container>
              ...
              <Grid key={p.id} xs={12} sm={6} md={3}>
                <Stack component={m.div} variants={varFade({ distance: 24 }).inUp} sx={{ height: 1, borderRadius: 2, overflow: 'hidden', bgcolor: 'background.paper', boxShadow: (t) => t.customShadows.card }}>
                  ...
                  <Stack spacing={1} sx={{ p: 2 }}>
                    ...
                    <Typography variant="subtitle1" sx={{ color: 'primary.main', fontWeight: 'fontWeightBold' }}>{formatSofa1Price(p.price)}</Typography>
                    <Button component={RouterLink} href={`/sofa1/products/${p.id}`} size="small" variant="outlined">Xem chi tiết</Button>
                  </Stack>
                </Stack>
              </Grid>
            ))}
