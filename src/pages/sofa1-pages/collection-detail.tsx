...
export default function Page() {
  ...
  const collection = SOFA1_COLLECTIONS.find((c) => c.slug === slug);
  // products for this collection — representative set filtered by collection theme
  const products = collection
    ? SOFA1_PRODUCTS.filter((p) =>
        collection.slug === 'luxury'
          ? p.style === 'luxury' || p.price >= 30000000
          : collection.slug === 'new'
            ? Boolean(p.badge === 'Mới')
            : collection.slug === 'bestseller'
              ? Boolean(p.badge === 'Bestseller')
              : collection.slug === 'featured'
                ? p.rating >= 4.8
                : true
      ).slice(0, collection.slug === 'new' ? 6 : 8)
    : [];
  ...
  if (!collection) {
    return (
      <>
        <Helmet>
          <title>Không tìm thấy bộ sưu tập - Casa Sofa</title>
        </Helmet>
        <Stack spacing={3} alignItems="center" sx={{ textAlign: 'center', py: 20 }}>
          ...
          <Button component={RouterLink} href="/sofa1/collections" variant="contained">
            Quay lại bộ sưu tập
          </Button>
        </Stack>
      </>
  ...
  return (
    <>
      <Helmet>
        <title>{collection.name} - Casa Sofa</title>
      </Helmet>
            ...
            <Grid xs={12} md={6}>
              <Stack spacing={3} component={m.div} variants={varFade({ distance: 24 }).inRight}>
                ...
                <Typography sx={{ color: 'text.secondary' }}>
                  Mỗi mẫu trong bộ sưu tập được chế tác thủ công bởi nghệ nhân Casa Sofa với da bò
                  Ý và gỗ sồi cao cấp, đảm bảo chất lượng và độ bền vượt thời gian.
                </Typography>
                <Button
                  ...
                  variant="contained"
                  size="large"
                  startIcon={<Iconify icon="solar:bag-bold-duotone" />}
                >
                  Xem tất cả sản phẩm
                </Button>
              </Stack>
              ...
              <Grid key={product.id} xs={12} sm={6} md={3}>
                ...
                >
                  ...
                  <Stack spacing={1} sx={{ p: 2 }}>
                    ...
                    >
                      {formatSofa1Price(product.price)}
                    </Typography>
                    <Button
                      component={RouterLink}
                      href={`/sofa1/products/${product.id}`}
                      size="small"
                      variant="outlined"
                    >
                      Xem chi tiết
