...
export default function Page() {
  ...
  const category = SOFA1_BLOG_CATEGORIES.find((c) => c.slug === slug);
  const posts = SOFA1_BLOG_POSTS.filter((p) => p.category === slug);
  const label = category?.label ?? slug;

  return (
    <>
      <Helmet>
        <title>{`${label} - Blog Casa Sofa`}</title>
      </Helmet>

      <Sofa1PageHero
        overline="Danh mục blog"
        title={label}
        subtitle="Tổng hợp các bài viết thuộc danh mục này — chia sẻ từ nghệ nhân Casa Sofa."
      ...
      {/* Posts grid */}
      <Sofa1Section bg="grey" py={{ xs: 4, md: 8 }}>
        {posts.length > 0 ? (
