...
import { SOFA1_BLOG_POSTS, SOFA1_BLOG_CATEGORIES, SOFA1_PAGE_IMAGES } from 'src/sections/sofa1-pages/sofa1-pages-data';

// ----------------------------------------------------------------------

const metadata = { title: 'Blog nội thất - Casa Sofa' };

export default function Page() {
  const theme = useTheme();
  const featured = SOFA1_BLOG_POSTS[0];
  const rest = SOFA1_BLOG_POSTS.slice(1);
  ...
  return (
    <>
      ...
      <Sofa1PageHero
        overline="Blog"
        title="Blog Casa Sofa"
        subtitle="Cẩm nang nội thất, mẹo bảo dưỡng sofa và xu hướng thiết kế từ đội ngũ nghệ nhân."
        image={SOFA1_PAGE_IMAGES.blog1}
      />

      {/* Featured */}
      <Sofa1Section py={{ xs: 8, md: 10 }}>
          ...
          <Grid container spacing={{ xs: 4, md: 6 }} alignItems="center">
            ...
            <Grid xs={12} md={5}>
              <Stack spacing={2} component={m.div} variants={varFade({ distance: 24 }).inRight}>
                ...
                <Button component={RouterLink} href={`/sofa1/blog/${featured.id}`} variant="contained" sx={{ width: 'fit-content' }} endIcon={<Iconify icon="eva:arrow-ios-forward-fill" />}>
                  Đọc tiếp
                </Button>
              </Stack>
            </Grid>
        ...
        <Grid container spacing={{ xs: 4, md: 6 }}>
          ...
          <Grid xs={12} md={3}>
            <Stack spacing={2} component={m.div} variants={varFade({ distance: 24 }).inUp}>
              <Typography variant="overline" sx={{ color: 'text.disabled' }}>Danh mục</Typography>
              <Stack spacing={1}>
                <Chip
                  label="Tất cả"
                  ...
                  color="primary"
                />
                {SOFA1_BLOG_CATEGORIES.map((cat) => (
                  <Chip
                    key={cat.slug}
                    label={cat.label}
                    component={RouterLink}
                    href={`/sofa1/blog/category/${cat.slug}`}
                    clickable
                    variant="soft"
                  />
                ))}
                ...
                <Grid key={post.id} xs={12} sm={6} md={4}>
                  ...
                  >
                    ...
                    <Stack spacing={1.5} sx={{ p: 3 }}>
                      ...
                      <Button component={RouterLink} href={`/sofa1/blog/${post.id}`} size="small" sx={{ width: 'fit-content' }} endIcon={<Iconify icon="eva:arrow-ios-forward-fill" />}>Đọc tiếp</Button>
                    </Stack>
                  </Stack>
                </Grid>
              ))}
