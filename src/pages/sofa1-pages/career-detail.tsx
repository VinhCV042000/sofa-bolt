...
export default function Page() {
  ...
  const { id } = useParams();
  const job = SOFA1_CAREERS.find((item) => item.id === id);

  if (!job) {
    return (
      <>
        <Helmet><title>Không tìm thấy vị trí - Casa Sofa</title></Helmet>
        <Container sx={{ py: { xs: 10, md: 16 }, textAlign: 'center' }}>
          <Stack spacing={3} alignItems="center">
            ...
            <Button component={RouterLink} href="/sofa1/careers" variant="contained" startIcon={<Iconify icon="eva:arrow-back-fill" />}>
              Quay lại danh sách
            </Button>
          </Stack>
        </Container>
  ...
  return (
    <>
      <Helmet>
        <title>{`${job.title} - Casa Sofa`}</title>
      </Helmet>
          ...
          <Stack spacing={4}>
            ...
            <Stack component={m.div} variants={varFade({ distance: 24 }).inUp} spacing={2}>
              ...
              <Typography sx={{ color: 'text.secondary', lineHeight: 1.8 }}>
                Tại Casa Sofa, chúng tôi tìm kiếm những người yêu nghề, tỉ mỉ và tận tâm với sản phẩm.
                Bạn sẽ làm việc trong môi trường chuyên nghiệp cùng nghệ nhân lành nghề với hơn 12 năm
                kinh nghiệm chế tác sofa da bò & gỗ sồi.
              </Typography>
            </Stack>
            ...
            {/* Summary */}
            <Stack component={m.div} variants={varFade({ distance: 24 }).inUp} direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ p: 3, borderRadius: 2, bgcolor: 'background.paper', boxShadow: (t) => t.customShadows.card }}>
              <Box sx={{ flex: 1 }}>
                <Typography variant="overline" sx={{ color: 'text.disabled' }}>Tổng quan</Typography>
              </Box>
              <Stack spacing={1} sx={{ flex: 2 }}>
                <Stack direction="row" spacing={1} alignItems="center">
                  <Iconify icon="solar:map-point-bold-duotone" width={18} sx={{ color: 'primary.main' }} />
                  <Typography variant="body2"><strong>Địa điểm:</strong> {job.location}</Typography>
                </Stack>
                <Stack direction="row" spacing={1} alignItems="center">
                  <Iconify icon="solar:clock-circle-bold-duotone" width={18} sx={{ color: 'warning.main' }} />
                  <Typography variant="body2"><strong>Hình thức:</strong> {job.type}</Typography>
                </Stack>
                <Stack direction="row" spacing={1} alignItems="center">
                  <Iconify icon="solar:wallet-bold-duotone" width={18} sx={{ color: 'success.main' }} />
                  <Typography variant="body2"><strong>Mức lương:</strong> {job.salary}</Typography>
                </Stack>
              </Stack>
            </Stack>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ mt: 2 }}>
              <Button component={RouterLink} href="/sofa1/careers/apply" variant="contained" size="large" color="primary" startIcon={<Iconify icon="solar:letter-bold-duotone" />}>
                Nộp CV ngay
              </Button>
              <Button component={RouterLink} href="/sofa1/careers" variant="outlined" size="large" startIcon={<Iconify icon="eva:arrow-back-fill" />}>
                Vị trí khác
              </Button>
            </Stack>
          </Stack>
