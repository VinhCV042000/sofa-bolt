...
import { formatSofa1Price, SOFA1_PAGE_IMAGES } from 'src/sections/sofa1-pages/sofa1-pages-data';

// ----------------------------------------------------------------------

const metadata = { title: 'Theo dõi đơn hàng — Casa Sofa' };
...
export default function Page() {
  ...
  return (
    <>
      ...
      <Sofa1PageHero title="Theo dõi đơn hàng" subtitle="Nhập mã đơn hàng và email để xem trạng thái giao hàng." image={SOFA1_PAGE_IMAGES.service2} overline="Theo dõi" />
        ...
        {!searched ? (
            ...
            <Grid xs={12} md={7}>
              <Stack component={m.div} variants={varFade({ distance: 24 }).inUp} spacing={3} sx={{ p: { xs: 3, md: 5 }, borderRadius: 3, bgcolor: 'background.paper', boxShadow: (t) => t.customShadows.card }}>
                ...
                <Grid container spacing={2}>
                  <Grid xs={12} sm={6}><TextField fullWidth label="Mã đơn hàng" placeholder="VD: #CS20250218001" /></Grid>
          ...
          <Stack spacing={5}>
            ...
            <Stack component={m.div} variants={varFade({ distance: 24 }).inUp} spacing={2} sx={{ mb: 2 }}>
              <Typography variant="h5">Trạng thái đơn hàng #CS20250218001</Typography>
              ...
              {STEPS.map((s, index) => {
                ...
                return (
                  <Grid key={s.label} xs={12} sm={6} md={3}>
                    <Stack spacing={2} sx={{ p: 3, height: 1, borderRadius: 3, bgcolor: 'background.paper', boxShadow: (t) => t.customShadows.card, opacity: active ? 1 : 0.5, border: active ? (t) => `1px solid ${varAlpha(t.vars.palette.primary.mainChannel, 0.24)}` : 'none' }}>
            ...
            <Grid container spacing={{ xs: 4, md: 5 }}>
              <Grid xs={12} md={8}>
                <Stack component={m.div} variants={varFade({ distance: 24 }).inUp} spacing={2} sx={{ p: { xs: 3, md: 4 }, borderRadius: 3, bgcolor: 'background.paper', boxShadow: (t) => t.customShadows.card }}>
                  ...
                  {[
                    { name: 'Sofa Roma Da Bò Nâu', qty: 1, price: 28500000 },
                    { name: 'Sofa Milano Linen Be', qty: 2, price: 12500000 },
                  ].map((it) => (
                  ...
                  <Divider />
                  <Stack direction="row" justifyContent="space-between"><Typography variant="subtitle2">Tổng cộng</Typography><Typography variant="h6" sx={{ color: 'primary.main' }}>{formatSofa1Price(28500000 + 12500000 * 2 + 300000)}</Typography></Stack>
                </Stack>
              </Grid>
              <Grid xs={12} md={4}>
