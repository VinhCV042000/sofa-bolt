...
const metadata = { title: 'Sản phẩm - TERRA ARCH' };
...
export default function Page() {
  return (
    <>
      ...
      <Sofa12PageHero overline="Sản phẩm" title="Bộ sưu tập sofa TERRA" subtitle="Khám phá hơn 40 mẫu sofa đất nung thủ công — từ vải lanh thô đến bouclé Dune." image={SOFA12_PAGE_IMAGES.aboutHero} />
              ...
              <Stack spacing={3} sx={{ position: { md: 'sticky' }, top: { md: 88 }, p: 3, borderRadius: 2, bgcolor: 'background.paper', boxShadow: (t) => t.customShadows.card }}>
                ...
                {FILTER_GROUPS.map((group) => (
                  <Stack key={group.key} spacing={1}>
                    ...
                    <Stack spacing={0.5}>
                      {SOFA12_PAGE_PRODUCT_CATEGORIES[group.key].map((item) => (
                        <Button key={item.slug} component={RouterLink} href={`/sofa12/products/category/${item.slug}`} size="small" sx={{ justifyContent: 'flex-start', color: 'text.secondary', textTransform: 'none', fontWeight: 'fontWeightRegular', '&:hover': { color: 'warning.main', bgcolor: (t) => varAlpha(t.vars.palette.warning.mainChannel, 0.08) } }}>
                          {item.label}
                        </Button>
                      ))}
                    </Stack>
                    ...
                    <Stack component={m.div} variants={varFade({ distance: 24 }).inUp} sx={{ height: 1, borderRadius: 2, overflow: 'hidden', bgcolor: 'background.paper', boxShadow: (t) => t.customShadows.card }}>
                      ...
                      <Stack spacing={1.5} sx={{ p: 2.5 }}>
                        ...
                        <Stack direction="row" alignItems="center" spacing={1}>
                          ...
                          {product.oldPrice && <Typography variant="body2" sx={{ color: 'text.disabled', textDecoration: 'line-through' }}>{formatSofa12PagePrice(product.oldPrice)}</Typography>}
                        </Stack>
                        <Button component={RouterLink} href={`/sofa12/products/${product.id}`} variant="outlined" size="small" color="warning" startIcon={<Iconify icon="solar:eye-bold-duotone" />}>
                          Xem chi tiết
                        </Button>
                      </Stack>
                    </Stack>
