...
import { sofa19FormatPrice, SOFA19_PAGE_IMAGES, SOFA19_PAGE_PRODUCTS } from 'src/sections/sofa19-pages/sofa19-pages-data';
...
export default function Page() {
  ...
  return (
    <>
      <Helmet>
        <title>Tài khoản — Sofa19</title>
      </Helmet>

      <Sofa19PageHero overline="TÀI KHOẢN" title={<>TÀI <span>KHOẢN</span></>} subtitle="Quản lý thông tin cá nhân, đơn hàng, bảo hành." image={SOFA19_PAGE_IMAGES.contact} />
          ...
          <Grid xs={12} md={3}>
            <Stack spacing={1} component={m.div} variants={varFade({ distance: 24 }).inLeft} sx={{ position: 'sticky', top: 80 }}>
              {ACCOUNT_SECTIONS.map((s) => (
                <Stack key={s.slug} component={RouterLink} href={`/sofa19/account/${s.slug === 'profile' ? '' : s.slug}`} direction="row" spacing={2} alignItems="center" sx={{ p: 2, borderRadius: 30, textDecoration: 'none', bgcolor: section === s.slug ? varAlpha(SOFA19_COLORS.coral, 0.08) : 'transparent', '&:hover': { bgcolor: varAlpha(SOFA19_COLORS.coral, 0.04) } }}>
                  <Iconify icon={s.icon} width={20} sx={{ color: SOFA19_COLORS.coral }} />
                  <Typography variant="body2" sx={{ fontWeight: section === s.slug ? 'fontWeightBold' : 'fontWeightMedium', color: section === s.slug ? SOFA19_COLORS.coral : 'text.secondary' }}>{s.label}</Typography>
                </Stack>
              ))}
            </Stack>
          </Grid>
            ...
            <Stack spacing={3} component={m.div} variants={varFade({ distance: 24 }).inRight}>
              {section === 'profile' || section === undefined ? (
                <>
                  ...
                  <Button variant="contained" sx={{ borderRadius: 30, alignSelf: 'flex-start', bgcolor: SOFA19_COLORS.coral, color: 'common.white', '&:hover': { bgcolor: SOFA19_COLORS.golden } }}>Cập nhật</Button>
                </>
              ) : section === 'orders' ? (
                <>
                  ...
                  {[1, 2, 3].map((i) => (
                    <Stack key={i} spacing={2} sx={{ p: 3, borderRadius: 3, bgcolor: 'background.paper', border: `1px solid ${varAlpha(theme.vars.palette.grey['500Channel'], 0.12)}` }}>
                      <Stack direction="row" justifyContent="space-between">
                        ...
                        <Typography variant="caption" sx={{ color: SOFA19_COLORS.jungleLight, fontWeight: 'fontWeightBold' }}>Đã giao</Typography>
                      </Stack>
                      <Typography variant="body2" sx={{ color: 'text.secondary' }}>{sofa19FormatPrice(12000000 + i * 1000000)}</Typography>
                    </Stack>
                  ))}
                </>
              ) : section === 'wishlist' ? (
                <>
                  ...
                  <Grid container spacing={2}>
                    {SOFA19_PAGE_PRODUCTS.slice(0, 4).map((p) => (
                      <Grid key={p.id} xs={6} md={3}>
                        <Stack component={RouterLink} href={`/sofa19/products/${p.slug}`} spacing={1} sx={{ textDecoration: 'none' }}>
                          ...
                          <Typography variant="caption" sx={{ color: SOFA19_COLORS.coral }}>{sofa19FormatPrice(p.price)}</Typography>
                        </Stack>
                      </Grid>
                    ))}
                  </Grid>
