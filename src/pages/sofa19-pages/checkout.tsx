...
export default function Page() {
  ...
  const cartItems = SOFA19_PAGE_PRODUCTS.slice(0, 2);
  const total = cartItems.reduce((sum, p) => sum + p.price, 0);

  return (
    <>
      <Helmet>
        <title>Thanh toán — Sofa19</title>
      </Helmet>

      <Sofa19PageHero overline="THANH TOÁN" title={<>THANH <span>TOÁN</span></>} subtitle="Hoàn tất đơn hàng retro của bạn." image={SOFA19_PAGE_IMAGES.product7} />
                ...
                <Stack spacing={2}>
                  {['Chuyển khoản ngân hàng', 'Thanh toán khi nhận hàng (COD)', 'Thanh toán online (VNPay/Momo)'].map((method, i) => (
                    <Stack key={i} direction="row" spacing={2} alignItems="center" sx={{ p: 2, borderRadius: 3, border: `1px solid ${varAlpha(theme.vars.palette.grey['500Channel'], 0.12)}` }}>
                      <Iconify icon={i === 0 ? 'solar:card-bold-duotone' : i === 1 ? 'solar:delivery-bold-duotone' : 'solar:wallet-bold-duotone'} width={24} sx={{ color: SOFA19_COLORS.coral }} />
            ...
            <Stack spacing={3} sx={{ p: 3, borderRadius: 3, bgcolor: 'background.paper', border: `1px solid ${varAlpha(theme.vars.palette.grey['500Channel'], 0.12)}` }}>
              ...
              {cartItems.map((item) => (
                <Stack key={item.id} direction="row" spacing={2} alignItems="center">
                  ...
                  <Typography variant="body2" sx={{ color: SOFA19_COLORS.coral, fontWeight: 'fontWeightBold' }}>{sofa19FormatPrice(item.price)}</Typography>
                </Stack>
              ))}
              <Divider />
              <Stack direction="row" justifyContent="space-between"><Typography sx={{ color: 'text.secondary' }}>Tạm tính</Typography><Typography sx={{ fontWeight: 'fontWeightBold' }}>{sofa19FormatPrice(total)}</Typography></Stack>
              ...
              <Divider />
              <Stack direction="row" justifyContent="space-between"><Typography variant="h6" sx={{ fontWeight: 'fontWeightBold' }}>Tổng</Typography><Typography variant="h6" sx={{ fontWeight: 'fontWeightBold', color: SOFA19_COLORS.coral }}>{sofa19FormatPrice(total)}</Typography></Stack>
              <Button component={RouterLink} href="/sofa19/checkout/success" size="large" variant="contained" fullWidth startIcon={<Iconify icon="solar:check-circle-bold-duotone" />} sx={{ borderRadius: 30, bgcolor: SOFA19_COLORS.coral, color: 'common.white', fontWeight: 'fontWeightBold', boxShadow: `0 4px 0 ${SOFA19_COLORS.ink}`, '&:hover': { bgcolor: SOFA19_COLORS.golden, boxShadow: `0 6px 0 ${SOFA19_COLORS.ink}` } }}>Đặt hàng</Button>
            </Stack>
          </Grid>
        </Grid>
      </Sofa19Section>
