...
export default function Page() {
  return (
    <>
      <Helmet><title>Blog — Sofa19</title></Helmet>
      <Sofa19PageHero overline="BLOG" title={<>B<span>LOG</span></>} subtitle="Tin tức, kiến thức, triết lý pastel pop và nội thất claymorphism." image={SOFA19_PAGE_IMAGES.blog1} />

      <Sofa19Section>
        <Grid container spacing={4}>
          <Grid xs={12} md={3}>
            <Stack spacing={2} component={m.div} variants={varFade({ distance: 24 }).inLeft} sx={{ position: 'sticky', top: 80 }}>
              <Typography variant="overline" sx={{ color: sofa19Alpha(SOFA19_COLORS.inkSoft, 0.6), letterSpacing: 2, fontWeight: 800 }}>CHUYÊN MỤC</Typography>
              {SOFA19_PAGE_BLOG_CATEGORIES.map((cat) => (
                <Link component={RouterLink} key={cat.slug} href={`/sofa19/blog/category/${cat.slug}`} sx={{ textDecoration: 'none', '&:hover .MuiTypography-root': { color: SOFA19_COLORS.coralDeep } }}>
                  <Typography variant="body2" sx={{ color: sofa19Alpha(SOFA19_COLORS.inkSoft, 0.8), transition: 'color 0.2s', fontWeight: 700 }}>{cat.label}</Typography>
                </Link>
              ))}
            </Stack>
          </Grid>
          <Grid xs={12} md={9}>
            <Grid container spacing={4}>
              {SOFA19_PAGE_BLOG_POSTS.map((post) => (
                <Grid key={post.id} xs={12} sm={6}>
                  <Stack component={RouterLink} href={`/sofa19/blog/${post.slug}`} spacing={2} sx={{ textDecoration: 'none', '&:hover .blog-img': { transform: 'scale(1.05)' }, '&:hover .blog-title': { color: SOFA19_COLORS.coralDeep } }}>
                    ...
                    <Stack direction="row" spacing={2} alignItems="center">
                      <Typography variant="caption" sx={{ color: sofa19Alpha(SOFA19_COLORS.inkSoft, 0.5), fontWeight: 700 }}>{post.author}</Typography>
                      <Typography variant="caption" sx={{ color: sofa19Alpha(SOFA19_COLORS.inkSoft, 0.5) }}>·</Typography>
                      <Typography variant="caption" sx={{ color: sofa19Alpha(SOFA19_COLORS.inkSoft, 0.5), fontWeight: 700 }}>{post.date}</Typography>
                      <Typography variant="caption" sx={{ color: sofa19Alpha(SOFA19_COLORS.inkSoft, 0.5) }}>·</Typography>
                      <Typography variant="caption" sx={{ color: sofa19Alpha(SOFA19_COLORS.inkSoft, 0.5), fontWeight: 700 }}>{post.readTime}</Typography>
                    </Stack>
                  </Stack>
                </Grid>
              ))}
