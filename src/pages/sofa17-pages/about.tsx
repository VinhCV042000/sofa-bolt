...
const VALUES = [
  { icon: 'solar:crown-bold-duotone', title: 'Thủ công 10+ năm', desc: '10+ năm đúc kết thủ công Riad Ma-rốc.', accent: SOFA17_COLORS.gold },
  { icon: 'solar:shield-check-bold-duotone', title: 'Bảo hành 10 năm', desc: 'Khung & đệm bảo hành 10 năm.', accent: SOFA17_COLORS.emerald },
  { icon: 'solar:hand-stars-bold-duotone', title: '99% hài lòng', desc: 'Hơn 3,200 khách hàng tin tưởng.', accent: SOFA17_COLORS.gold },
];
    ...
    <>
        ...
        <Grid container spacing={{ xs: 5, md: 8 }} alignItems="center">
          <Grid xs={12} md={6}>
            <Box component={m.div} variants={varFade({ distance: 24 }).inLeft}>
              <Box component="img" src={SOFA17_PAGE_IMAGES.workshop} alt="Xưởng Sofa17" sx={{ width: 1, aspectRatio: '4/5', objectFit: 'cover', border: `1px solid ${varAlpha(SOFA17_COLORS.gold, 0.3)}` }} />
            </Box>
          </Grid>
          <Grid xs={12} md={6}>
      ...
      <Sofa17Section bg="charcoal">
        <Stack spacing={2} sx={{ textAlign: 'center', maxWidth: 640, mx: 'auto', mb: { xs: 5, md: 8 } }}>
          <Typography variant="overline" sx={{ color: varAlpha(SOFA17_COLORS.gold, 0.6), letterSpacing: 4 }}>GIÁ TRỊ CỐT LÕI</Typography>
          <Typography variant="h2" sx={{ fontWeight: 'fontWeightLight', color: 'common.white', fontFamily: theme.typography.fontSecondaryFamily, '& span': { ...textGradient(`90deg, ${SOFA17_COLORS.goldLight} 0%, ${SOFA17_COLORS.gold} 50%, ${SOFA17_COLORS.goldDark} 100%`), fontStyle: 'italic' } }}>
            3 LÝ DO <span>CHỌN SOFA17</span>
          </Typography>
        </Stack>
        <Grid container spacing={3}>
          {VALUES.map((item) => (
            <Grid key={item.title} xs={12} md={4}>
              <Sofa17Card accent={item.accent} component={m.div} variants={varFade({ distance: 24 }).inUp}>
                <Stack spacing={2}>
      ...
      <Sofa17Section bg="black">
        <Grid container spacing={3}>
          {SOFA17_PAGE_COMPANY_INFO.stats.map((s) => (
            <Grid key={s.label} xs={6} md={3}>
              <Stack spacing={0.5} alignItems="center" component={m.div} variants={varFade({ distance: 24 }).inUp}>
                <Typography variant="h2" sx={{ fontWeight: 'fontWeightLight', ...textGradient(`90deg, ${SOFA17_COLORS.goldLight} 0%, ${SOFA17_COLORS.gold} 50%, ${SOFA17_COLORS.goldDark} 100%`) }}>{s.value}</Typography>
      ...
      <Sofa17Section bg="charcoal">
        <Stack spacing={2} sx={{ textAlign: 'center', maxWidth: 640, mx: 'auto', mb: { xs: 5, md: 8 } }}>
          <Typography variant="overline" sx={{ color: varAlpha(SOFA17_COLORS.gold, 0.6), letterSpacing: 4 }}>KHÁM PHÁ THÊM</Typography>
          <Typography variant="h2" sx={{ fontWeight: 'fontWeightLight', color: 'common.white', fontFamily: theme.typography.fontSecondaryFamily, '& span': { ...textGradient(`90deg, ${SOFA17_COLORS.goldLight} 0%, ${SOFA17_COLORS.gold} 50%, ${SOFA17_COLORS.goldDark} 100%`), fontStyle: 'italic' } }}>
            11 GÓC NHÌN <span>VỀ SOFA17</span>
          </Typography>
        </Stack>
        <Grid container spacing={2}>
          {SUB_PAGES.map((item) => (
            <Grid key={item.slug} xs={6} sm={4} md={3}>
              <Stack component={RouterLink} href={`/sofa17/about/${item.slug}`} spacing={2} sx={{ p: 3, height: 1, textDecoration: 'none', border: `1px solid ${varAlpha(SOFA17_COLORS.gold, 0.2)}`, bgcolor: SOFA17_COLORS.black, '&:hover': { border: `1px solid ${SOFA17_COLORS.gold}` }, transition: 'all 0.25s' }}>
                <Box sx={{ width: 40, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', border: `1px solid ${varAlpha(SOFA17_COLORS.gold, 0.3)}`, color: SOFA17_COLORS.gold }}>
        ...
        <Sofa17Card accent={SOFA17_COLORS.gold} sx={{ maxWidth: 720, mx: 'auto', textAlign: 'center', p: { xs: 5, md: 8 } }}>
          <Stack spacing={4} alignItems="center">
            ...
            <Stack direction="row" spacing={2} flexWrap="wrap" justifyContent="center">
              <Button component={RouterLink} href="/sofa17/showrooms" size="large" startIcon={<Iconify icon="solar:map-point-bold-duotone" />} sx={{ px: 4, py: 1.75, fontWeight: 'fontWeightMedium', borderRadius: 0, color: SOFA17_COLORS.black, bgcolor: SOFA17_COLORS.gold, border: `1px solid ${SOFA17_COLORS.gold}`, '&:hover': { bgcolor: SOFA17_COLORS.goldLight } }}>XEM SHOWROOM</Button>
              <Button component={RouterLink} href="/sofa17/products" size="large" sx={{ px: 4, py: 1.75, fontWeight: 'fontWeightMedium', borderRadius: 0, color: SOFA17_COLORS.gold, bgcolor: 'transparent', border: `1px solid ${varAlpha(SOFA17_COLORS.gold, 0.4)}`, '&:hover': { border: `1px solid ${SOFA17_COLORS.gold}`, bgcolor: varAlpha(SOFA17_COLORS.gold, 0.08) } }}>XEM SẢN PHẨM</Button>
            </Stack>
          </Stack>
        </Sofa17Card>
      </Sofa17Section>
