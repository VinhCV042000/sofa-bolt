...
export default function Page() {
  ...
  return (
    <>
      <Helmet><title>Khuyến mãi — Sofa17</title></Helmet>
      <Sofa17PageHero overline="KHUYẾN MÃI" title={<>KHUYẾN <span>MÃI</span></>} subtitle="Bảo hành, bảo trì, sửa chữa, vệ sinh, đổi trả — dịch vụ trọn đời." image={SOFA17_PAGE_IMAGES.prod1} />

      <Sofa17Section bg="black">
        <Grid container spacing={3}>
          {SOFA17_PAGE_PROMOTION_SERVICES.map((s) => (
            <Grid key={s.slug} xs={12} sm={6} md={4}>
              <Stack component={RouterLink} href={`/sofa17/promotions/${s.slug}`} spacing={2} sx={{ p: 4, height: 1, textDecoration: 'none', border: `1px solid ${varAlpha(SOFA17_COLORS.gold, 0.3)}`, bgcolor: SOFA17_COLORS.charcoal, '&:hover': { border: `1px solid ${SOFA17_COLORS.gold}` }, transition: 'all 0.25s' }}>
