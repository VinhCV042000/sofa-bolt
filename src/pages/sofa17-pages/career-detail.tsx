...
export default function Page() {
  ...
  const id = params.id || '';
  const job = SOFA17_PAGE_CAREERS.find((j) => j.id === id) || SOFA17_PAGE_CAREERS[0];

  return (
    <>
      <Helmet><title>{job.title} — Sofa17 Tuyển dụng</title></Helmet>
      <Sofa17PageHero overline="TUYỂN DỤNG" title={<span>{job.title.toUpperCase()}</span>} subtitle={`${job.location} · ${job.type} · ${job.salary}`} image={SOFA17_PAGE_IMAGES.careers} />
        ...
        <Grid container spacing={5}>
          <Grid xs={12} md={8}>
            <Stack spacing={4} component={m.div} variants={varFade({ distance: 24 }).inUp}>
              ...
              <Typography variant="h5" sx={{ fontWeight: 'fontWeightMedium', color: SOFA17_COLORS.gold }}>Yêu cầu</Typography>
              <Stack spacing={2}>
                {job.requirements.map((req, i) => (
                  <Stack key={i} direction="row" spacing={2} alignItems="flex-start">
                    <Box sx={{ width: 8, height: 8, mt: 1.5, bgcolor: SOFA17_COLORS.gold, flexShrink: 0 }} />
                    <Typography sx={{ color: varAlpha('#FFFFFF', 0.5), lineHeight: 1.9, fontWeight: 'fontWeightLight' }}>{req}</Typography>
                  </Stack>
                ))}
              </Stack>
              <Button component={RouterLink} href={`/sofa17/careers/apply/${job.id}`} size="large" variant="contained" startIcon={<Iconify icon="solar:upload-bold-duotone" />} sx={{ alignSelf: 'flex-start', borderRadius: 0, px: 4, py: 1.75, fontWeight: 'fontWeightMedium', color: SOFA17_COLORS.black, bgcolor: SOFA17_COLORS.gold, border: `1px solid ${SOFA17_COLORS.gold}`, '&:hover': { bgcolor: SOFA17_COLORS.goldLight } }}>Nộp CV</Button>
            </Stack>
          </Grid>
          <Grid xs={12} md={4}>
            <Stack spacing={3} component={m.div} variants={varFade({ distance: 24 }).inRight} sx={{ position: 'sticky', top: 80 }}>
              <Sofa17Card accent={SOFA17_COLORS.gold}>
                <Stack spacing={2}>
                  <Typography variant="h6" sx={{ fontWeight: 'fontWeightMedium', color: SOFA17_COLORS.gold }}>Thông tin</Typography>
              ...
              <Button component={RouterLink} href="/sofa17/careers/tracking" size="large" variant="outlined" fullWidth startIcon={<Iconify icon="solar:clipboard-list-bold-duotone" />} sx={{ borderRadius: 0, fontWeight: 'fontWeightMedium', color: SOFA17_COLORS.gold, bgcolor: 'transparent', border: `1px solid ${varAlpha(SOFA17_COLORS.gold, 0.4)}`, '&:hover': { border: `1px solid ${SOFA17_COLORS.gold}`, bgcolor: varAlpha(SOFA17_COLORS.gold, 0.08) } }}>Theo dõi hồ sơ</Button>
            </Stack>
          </Grid>
        </Grid>
      </Sofa17Section>
