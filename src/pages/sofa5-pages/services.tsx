        ...
        <Grid container spacing={3}>
          {SERVICES.map((s) => (
            <Grid key={s.title} xs={12} sm={6} md={4}>
              <Stack component={RouterLink} href={s.path} spacing={2} sx={{ p: 4, height: 1, textDecoration: 'none', border: `1px solid ${varAlpha(s.accent, 0.25)}`, borderRadius: 3, bgcolor: SOFA5_COLORS.creamLight, '&:hover': { border: `1px solid ${s.accent}` }, transition: 'all 0.25s' }}>
                ...
                <Typography variant="h6" sx={{ fontWeight: 'fontWeightMedium', color: SOFA5_COLORS.ink, fontFamily: theme.typography.fontSecondaryFamily }}>{s.title}</Typography>
                <Typography variant="body2" sx={{ color: SOFA5_COLORS.inkSoft, fontWeight: 'fontWeightLight', lineHeight: 1.8 }}>{s.desc}</Typography>
              </Stack>
            </Grid>
          ))}
        </Grid>
