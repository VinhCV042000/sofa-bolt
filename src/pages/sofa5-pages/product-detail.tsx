...
import { formatSofa5Price, SOFA5_PAGE_PRODUCTS } from 'src/sections/sofa5-pages/sofa5-pages-data';
...
export default function Page() {
  ...
  const product = SOFA5_PAGE_PRODUCTS.find((p) => p.slug === slug) || SOFA5_PAGE_PRODUCTS[0];
  const [tab, setTab] = useState('info');
  const related = SOFA5_PAGE_PRODUCTS.filter((p) => p.id !== product.id).slice(0, 4);
  ...
  return (
    <>
        ...
        <Grid container spacing={5}>
          <Grid xs={12} md={6}>
            <Stack spacing={2} component={m.div} variants={varFade({ distance: 24 }).inLeft}>
              ...
              <Stack direction="row" spacing={2}>
                <Button variant="outlined" startIcon={<Iconify icon="solar:camera-bold-duotone" />} sx={{ borderColor: SOFA5_COLORS.clay, color: SOFA5_COLORS.charcoal }}>Hình 360°</Button>
                <Button variant="outlined" startIcon={<Iconify icon="solar:video-bold-duotone" />} sx={{ borderColor: SOFA5_COLORS.clay, color: SOFA5_COLORS.charcoal }}>Video</Button>
              </Stack>
            </Stack>
          </Grid>
          <Grid xs={12} md={6}>
      ...
      <Sofa5Section>
        <Typography variant="h5" sx={{ fontWeight: 'fontWeightBold', color: SOFA5_COLORS.charcoal, mb: 4 }}>Sản phẩm liên quan</Typography>
        <Grid container spacing={3}>
          {related.map((p) => (
            <Grid key={p.id} xs={6} md={3}>
              <Stack component={RouterLink} href={`/sofa5/products/${p.slug}`} spacing={1} sx={{ textDecoration: 'none' }}>
