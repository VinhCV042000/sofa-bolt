...
import { SOFA14_PAGE_FAQS, SOFA14_PAGE_IMAGES } from 'src/sections/sofa14-pages/sofa14-pages-data';
...
export default function Page() {
  ...
  return (
    <>
      <Helmet>
        <title>FAQ — Sofa14</title>
      </Helmet>

      <Sofa14PageHero overline="FAQ" title={<>CÂU HỎI <span>THƯỜNG GẶP</span></>} subtitle="Tất cả câu hỏi về sofa retro được giải đáp." image={SOFA14_PAGE_IMAGES.contact} />

      <Sofa14Section>
        <Stack spacing={2} sx={{ maxWidth: 800, mx: 'auto' }} component={m.div} variants={varFade({ distance: 24 }).inUp}>
          {SOFA14_PAGE_FAQS.map((faq, i) => (
            <Accordion key={i} expanded={expanded === `panel-${i}`} onChange={(e, isExpanded) => setExpanded(isExpanded ? `panel-${i}` : false)} sx={{ borderRadius: '16px !important', border: `1px solid rgba(145,158,171,0.12)`, '&:before': { display: 'none' } }}>
              <AccordionSummary expandIcon={<Iconify icon="solar:alt-arrow-down-linear" width={20} sx={{ color: SOFA14_COLORS.coral }} />}>
