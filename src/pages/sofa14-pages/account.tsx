...
export default function Page() {
  ...
  return (
    <>
      <Helmet>
        <title>Tài khoản — Sofa14</title>
      </Helmet>

      <Sofa14PageHero overline="TÀI KHOẢN" title={<>TÀI <span>KHOẢN</span></>} subtitle="Quản lý thông tin cá nhân, đơn hàng, bảo hành." image={SOFA14_PAGE_IMAGES.contact} />
          ...
          <Grid xs={12} md={3}>
            <Stack spacing={1} component={m.div} variants={varFade({ distance: 24 }).inLeft} sx={{ position: 'sticky', top: 80 }}>
              {ACCOUNT_SECTIONS.map((s) => (
                <Stack key={s.slug} component={RouterLink} href={`/sofa14/account/${s.slug === 'profile' ? '' : s.slug}`} direction="row" spacing={2} alignItems="center" sx={{ p: 2, borderRadius: 30, textDecoration: 'none', bgcolor: section === s.slug ? varAlpha(SOFA14_COLORS.coral, 0.08) : 'transparent', '&:hover': { bgcolor: varAlpha(SOFA14_COLORS.coral, 0.04) } }}>
                  <Iconify icon={s.icon} width={20} sx={{ color: SOFA14_COLORS.coral }} />
                  <Typography variant="body2" sx={{ fontWeight: section === s.slug ? 'fontWeightBold' : 'fontWeightMedium', color: section === s.slug ? SOFA14_COLORS.coral : 'text.secondary' }}>{s.label}</Typography>
                </Stack>
              ))}
            </Stack>
          </Grid>
            ...
            <Stack spacing={3} component={m.div} variants={varFade({ distance: 24 }).inRight}>
              {section === 'profile' || section === undefined ? (
                <>
                  ...
                  <Button variant="contained" sx={{ borderRadius: 30, alignSelf: 'flex-start', bgcolor: SOFA14_COLORS.coral, color: 'common.white', '&:hover': { bgcolor: SOFA14_COLORS.butter } }}>Cập nhật</Button>
                </>
              ) : section === 'orders' ? (
                <>
                  ...
                  {[1, 2, 3].map((i) => (
                    <Stack key={i} spacing={2} sx={{ p: 3, borderRadius: 3, bgcolor: 'background.paper', border: `1px solid ${varAlpha(theme.vars.palette.grey['500Channel'], 0.12)}` }}>
                      <Stack direction="row" justifyContent="space-between">
                        ...
                        <Typography variant="caption" sx={{ color: SOFA14_COLORS.mint, fontWeight: 'fontWeightBold' }}>Đã giao</Typography>
                      </Stack>
                      <Typography variant="body2" sx={{ color: 'text.secondary' }}>{sofa14FormatPrice(12000000 + i * 1000000)}</Typography>
                    </Stack>
                  ))}
                </>
              ) : section === 'wishlist' ? (
                <>
                  ...
                  <Grid container spacing={2}>
                    {SOFA14_PAGE_PRODUCTS.slice(0, 4).map((p) => (
                      <Grid key={p.id} xs={6} md={3}>
                        <Stack component={RouterLink} href={`/sofa14/products/${p.slug}`} spacing={1} sx={{ textDecoration: 'none' }}>
                          ...
                          <Typography variant="caption" sx={{ color: SOFA14_COLORS.coral }}>{sofa14FormatPrice(p.price)}</Typography>
                        </Stack>
                      </Grid>
                    ))}
                  </Grid>
