...
export default function Page() {
  ...
  return (
    <>
      <Helmet>
        <title>Đăng ký đại lý — Sofa14</title>
      </Helmet>

      <Sofa14PageHero overline="B2B" title={<>ĐĂNG KÝ <span>ĐẠI LÝ</span></>} subtitle="Trở thành đại lý phân phối sofa retro Sofa14." image={SOFA14_PAGE_IMAGES.b2b} />
          ...
          <Grid xs={12} md={6}>
            {submitted ? (
              <Stack spacing={3} alignItems="center" sx={{ textAlign: 'center', py: 8 }} component={m.div} variants={varFade({ distance: 24 }).inUp}>
                <Iconify icon="solar:check-circle-bold-duotone" width={80} sx={{ color: SOFA14_COLORS.mint }} />
              ...
              <Stack spacing={3} component={m.div} variants={varFade({ distance: 24 }).inUp}>
                ...
                <Button onClick={() => setSubmitted(true)} size="large" variant="contained" startIcon={<Iconify icon="solar:document-bold-duotone" />} sx={{ borderRadius: 30, bgcolor: SOFA14_COLORS.coral, color: 'common.white', fontWeight: 'fontWeightBold', boxShadow: `0 4px 0 ${SOFA14_COLORS.ink}`, '&:hover': { bgcolor: SOFA14_COLORS.butter, boxShadow: `0 6px 0 ${SOFA14_COLORS.ink}` } }}>Gửi đăng ký</Button>
              </Stack>
            )}
          </Grid>
        </Grid>
