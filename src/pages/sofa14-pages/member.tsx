...
export default function Page() {
  ...
  return (
    <>
      <Helmet>
        <title>Khu vực thành viên — Sofa14</title>
      </Helmet>

      <Sofa14PageHero overline="THÀNH VIÊN" title={<>KHU VỰC <span>THÀNH VIÊN</span></>} subtitle="Dashboard khách hàng và đại lý Sofa14." image={SOFA14_PAGE_IMAGES.contact} />
          ...
          <Grid xs={12} md={3}>
            <Stack spacing={1} component={m.div} variants={varFade({ distance: 24 }).inLeft} sx={{ position: 'sticky', top: 80 }}>
              ...
              {CUSTOMER_ITEMS.map((s) => (
                <Stack key={s.slug} component={RouterLink} href={`/sofa14/member/${s.slug === 'dashboard' ? '' : s.slug}`} direction="row" spacing={2} alignItems="center" sx={{ p: 2, borderRadius: 30, textDecoration: 'none', bgcolor: section === s.slug ? varAlpha(SOFA14_COLORS.coral, 0.08) : 'transparent', '&:hover': { bgcolor: varAlpha(SOFA14_COLORS.coral, 0.04) } }}>
              ...
              {DEALER_ITEMS.map((s) => (
                <Stack key={s.slug} component={RouterLink} href={`/sofa14/member/${s.slug}`} direction="row" spacing={2} alignItems="center" sx={{ p: 2, borderRadius: 30, textDecoration: 'none', bgcolor: section === s.slug ? varAlpha(SOFA14_COLORS.coral, 0.08) : 'transparent', '&:hover': { bgcolor: varAlpha(SOFA14_COLORS.coral, 0.04) } }}>
                  <Iconify icon={s.icon} width={20} sx={{ color: SOFA14_COLORS.coral }} />
                  <Typography variant="body2" sx={{ fontWeight: section === s.slug ? 'fontWeightBold' : 'fontWeightMedium', color: section === s.slug ? SOFA14_COLORS.coral : 'text.secondary' }}>{s.label}</Typography>
                </Stack>
              ))}
                    ...
                    {[
                      ...
                      <Grid key={stat.label} xs={6} md={3}>
                        <Stack spacing={2} sx={{ p: 3, borderRadius: 3, bgcolor: 'background.paper', border: `1px solid ${varAlpha(theme.vars.palette.grey['500Channel'], 0.12)}` }}>
                          <Iconify icon={stat.icon} width={32} sx={{ color: SOFA14_COLORS.coral }} />
                          <Typography variant="h4" sx={{ fontWeight: 'fontWeightBold', color: SOFA14_COLORS.coral }}>{stat.value}</Typography>
