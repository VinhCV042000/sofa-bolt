...
import { SOFA6_PAGE_IMAGES, SOFA6_PRODUCTS, SOFA6_PRODUCT_CATEGORIES, formatSofa6Price } from 'src/sections/sofa6-pages/sofa6-pages-data';
import { SOFA6_COLORS } from 'src/sections/sofa6/sofa6-data';

const metadata = { title: 'Sản phẩm - Sofa Earth' };
...
export default function Page() {
  return (
    <>
      ...
      <Sofa6PageHero overline="Sản phẩm" title="Bộ sưu tập sofa tự nhiên" subtitle="Khám phá các mẫu sofa thủ công từ linen, bông hữu cơ, wool — vật liệu thiên nhiên 100%." image={SOFA6_PAGE_IMAGES.aboutHero} />
              ...
              <Stack spacing={3} sx={{ position: { md: 'sticky' }, top: { md: 88 }, p: 3, borderRadius: 2, bgcolor: 'background.paper', boxShadow: (t) => t.customShadows.card }}>
                ...
                {FILTER_GROUPS.map((group) => (
                  <Stack key={group.key} spacing={1}>
                    ...
                    <Stack spacing={0.5}>
                      {SOFA6_PRODUCT_CATEGORIES[group.key].map((item) => (
                        <Button key={item.slug} component={RouterLink} href={`/sofa6/products/category/${item.slug}`} size="small" sx={{ justifyContent: 'flex-start', color: 'text.secondary', textTransform: 'none', fontWeight: 'fontWeightRegular', '&:hover': { color: SOFA6_COLORS.terracotta, bgcolor: varAlpha(SOFA6_COLORS.terracotta, 0.08) } }}>
                          {item.label}
                        </Button>
                      ))}
                    </Stack>
                    ...
                    <Stack component={m.div} variants={varFade({ distance: 24 }).inUp} sx={{ height: 1, borderRadius: 2, overflow: 'hidden', bgcolor: 'background.paper', boxShadow: (t) => t.customShadows.card }}>
                      ...
                      <Stack spacing={1.5} sx={{ p: 2.5 }}>
                        ...
                        <Stack direction="row" alignItems="center" spacing={1}>
                          ...
                          {product.oldPrice && <Typography variant="body2" sx={{ color: 'text.disabled', textDecoration: 'line-through' }}>{formatSofa6Price(product.oldPrice)}</Typography>}
                        </Stack>
                        <Button component={RouterLink} href={`/sofa6/products/${product.id}`} variant="outlined" size="small" startIcon={<Iconify icon="solar:eye-bold-duotone" />} sx={{ color: SOFA6_COLORS.terracotta, borderColor: SOFA6_COLORS.terracotta, '&:hover': { borderColor: SOFA6_COLORS.clay, bgcolor: varAlpha(SOFA6_COLORS.terracotta, 0.08) } }}>
                          Xem chi tiết
                        </Button>
                      </Stack>
                    </Stack>
