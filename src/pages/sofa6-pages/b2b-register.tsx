...
const metadata = { title: 'Đăng ký đại lý — Sofa Earth' };

const BUSINESS_TYPES = ['Nhà thiết kế', 'Nhà thầu nội thất', 'Showroom nội thất', 'Đại lý phân phối', 'Khách sạn / Resort', 'Café / Nhà hàng'];

const BENEFITS = [
  { icon: 'solar:hand-money-bold-duotone', title: 'Chiết khấu đến 35%', desc: 'Mức chiết khấu hấp dẫn theo doanh số.' },
  ...
  return (
    <>
      ...
      <Sofa6PageHero
        ...
        subtitle="Hoàn tất biểu mẫu để trở thành đối tác phân phối Sofa Earth và nhận chính sách ưu đãi cho sản phẩm thủ công từ vật liệu thiên nhiên."
      ...
      <Sofa6Section>
        <Grid container spacing={{ xs: 4, md: 6 }}>
          ...
          {/* Benefits sidebar */}
          <Grid xs={12} md={4}>
            <Stack component={m.div} variants={varFade({ distance: 24 }).inLeft} spacing={3} sx={{ p: { xs: 3, md: 4 }, borderRadius: 3, bgcolor: varAlpha(SOFA6_COLORS.terracotta, 0.06), height: 1 }}>
              <Typography variant="h6">Quyền lợi đại lý</Typography>
              {BENEFITS.map((b) => (
                <Stack key={b.title} direction="row" spacing={2} alignItems="flex-start">
                  <Box sx={{ width: 40, height: 40, display: 'flex', borderRadius: 1.5, alignItems: 'center', justifyContent: 'center', bgcolor: SOFA6_COLORS.terracotta, color: 'common.white', flexShrink: 0 }}>
              ...
              <Button component={RouterLink} href="/sofa6/b2b/policy" variant="outlined" fullWidth>Xem chính sách đại lý</Button>
            </Stack>
          </Grid>
        </Grid>
      </Sofa6Section>
