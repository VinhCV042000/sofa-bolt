...
import { type AdminRow, useSofa20AdminRows } from '../sofa20-admin-store';
import { Sofa20AdminLayout, SOFA20_ADMIN_THEME } from './sofa20-admin-layout';
import { SOFA20_ADMIN_ROOT, SOFA20_ADMIN_GROUPS, findSofa20AdminModule } from '../sofa20-admin-data';

// ----------------------------------------------------------------------

const { ACCENT, ACCENT_DEEP, SURFACE } = SOFA20_ADMIN_THEME;
...
export function Sofa20AdminModuleView() {
  const { group: groupSlug, module: moduleSlug } = useParams();

  const found = useMemo(() => findSofa20AdminModule(groupSlug, moduleSlug), [groupSlug, moduleSlug]);

  const group = found?.group;
  const module = found?.module;

  const { rows, createRow, updateRow, deleteRow, deleteRows, resetRows } = useSofa20AdminRows(
    group?.slug ?? '',
    module?.slug ?? ''
  );
  ...
  if (!found || !group || !module) {
    const first = SOFA20_ADMIN_GROUPS[0];
    return <Navigate to={`${SOFA20_ADMIN_ROOT}/${first.slug}/${first.modules[0].slug}`} replace />;
  }
  ...
  return (
    <>
      <Helmet>
        <title>{`${module.name} | ${group.name} - Quản trị Sofa20`}</title>
