...
import { SOFA20_ADMIN_TOOLS, SOFA20_ADMIN_FUNNEL } from '../sofa20-admin-tools';
...
export function Sofa20AdminRevenueChart() {
  const actual = SOFA20_ADMIN_REVENUE_SERIES[0].data;
  const target = SOFA20_ADMIN_REVENUE_SERIES[1].data;
...
export function Sofa20AdminFunnel() {
  const top = SOFA20_ADMIN_FUNNEL[0].value;
  ...
  return (
    <Card sx={{ height: 1 }}>
      ...
      <Stack spacing={2} sx={{ p: 3 }}>
        {SOFA20_ADMIN_FUNNEL.map((step, index) => {
          const ratio = (step.value / top) * 100;
          const previous = SOFA20_ADMIN_FUNNEL[index - 1];
...
export function Sofa20AdminQuickLinks() {
  return (
    <Card sx={{ p: 3 }}>
      ...
      <Grid container spacing={2}>
        {SOFA20_ADMIN_TOOLS.map((tool) => (
          <Grid key={tool.slug} xs={12} sm={6} md={3}>
            <ButtonBase
              component={RouterLink}
              href={`${SOFA20_ADMIN_ROOT}/tools/${tool.slug}`}
              sx={{
                width: 1,
                height: 1,
                p: 2,
