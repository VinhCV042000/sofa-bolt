...
import {
  SOFA20_PROFILE,
  SOFA20_ADMIN_TOOLS,
  SOFA20_NOTIFICATIONS,
  SOFA20_SETTINGS_FIELDS,
  SOFA20_SETTINGS_TOGGLES,
  findSofa20AdminTool,
} from '../sofa20-admin-tools';
import { Sofa20AdminCharts } from './sofa20-admin-charts';
import { Sofa20AdminFunnel, Sofa20AdminRevenueChart } from './sofa20-admin-insights';
import { Sofa20AdminLayout } from './sofa20-admin-layout';
import { SOFA20_ADMIN_ROOT } from '../sofa20-admin-data';
...
export function Sofa20AdminToolsView() {
  const { tool: toolSlug } = useParams();
  const tool = findSofa20AdminTool(toolSlug ?? 'reports');
  ...
  const [settings, setSettings] = useState<SettingsValues>(() =>
    Object.fromEntries(SOFA20_SETTINGS_FIELDS.map((field) => [field.key, field.value]))
  );
  const [toggles, setToggles] = useState<ToggleValues>(() =>
    Object.fromEntries(SOFA20_SETTINGS_TOGGLES.map((item) => [item.key, item.on]))
  );

  const notifications = useMemo(
    () =>
      SOFA20_NOTIFICATIONS.filter((item) => {
  ...
  if (!tool) {
    return <Navigate to={`${SOFA20_ADMIN_ROOT}/tools/reports`} replace />;
  }
    ...
    <Stack spacing={3}>
      <Card>
        ...
        <Grid container spacing={2.5} sx={{ p: 3 }}>
          {SOFA20_SETTINGS_FIELDS.map((field) => (
      ...
      <Card>
        ...
        <Grid container spacing={1} sx={{ p: 2.5 }}>
          {SOFA20_SETTINGS_TOGGLES.map((item) => (
      ...
      <Grid xs={12} md={5}>
        <Card sx={{ p: 3, height: 1 }}>
          <Stack alignItems="center" spacing={1.5} sx={{ textAlign: 'center' }}>
            ...
            <Typography variant="h6">{SOFA20_PROFILE.name}</Typography>
            <Chip label={SOFA20_PROFILE.role} color="warning" variant="soft" />
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>{SOFA20_PROFILE.email}</Typography>
          </Stack>
          <Divider sx={{ my: 3 }} />
          <Stack spacing={1.5}>
            {[
              ['Số điện thoại', SOFA20_PROFILE.phone],
              ['Bộ phận', SOFA20_PROFILE.team],
              ['Tham gia từ', SOFA20_PROFILE.joined],
            ].map(([label, value]) => (
        ...
        <Stack spacing={3}>
          <Card>
            ...
            <Stack direction="row" flexWrap="wrap" useFlexGap gap={1} sx={{ p: 3 }}>
              {SOFA20_PROFILE.permissions.map((permission) => (
          ...
          <Card>
            ...
            <Divider />
            <Stack divider={<Divider />}>
              {SOFA20_PROFILE.sessions.map((session) => (
  ...
  return (
    <>
      <Helmet>
        <title>{`${tool.name} | Quản trị Sofa20`}</title>
      ...
      >
        <Stack spacing={3}>
          <Stack direction="row" spacing={1} sx={{ overflowX: 'auto', pb: 0.5 }}>
            {SOFA20_ADMIN_TOOLS.map((item) => (
              <Button
                key={item.slug}
                href={`${SOFA20_ADMIN_ROOT}/tools/${item.slug}`}
