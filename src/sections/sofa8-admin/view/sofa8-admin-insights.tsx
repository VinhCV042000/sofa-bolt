...
import {
  SOFA8_ADMIN_ROOT,
  SOFA8_ADMIN_CHART_MONTHS,
  SOFA8_ADMIN_REVENUE_SERIES,
} from '../sofa8-admin-data';
import { SOFA8_ADMIN_TOOLS, SOFA8_ADMIN_FUNNEL } from '../sofa8-admin-tools';
...
export function Sofa8AdminRevenueChart() {
  const actual = SOFA8_ADMIN_REVENUE_SERIES[0].data;
  const target = SOFA8_ADMIN_REVENUE_SERIES[1].data;
  ...
  const options = useChart({
    ...
    xaxis: { categories: SOFA8_ADMIN_CHART_MONTHS },
...
export function Sofa8AdminFunnel() {
  const top = SOFA8_ADMIN_FUNNEL[0].value;
  ...
  return (
    <Card sx={{ height: 1 }}>
      ...
      <Stack spacing={2} sx={{ p: 3 }}>
        {SOFA8_ADMIN_FUNNEL.map((step, index) => {
          const ratio = (step.value / top) * 100;
          const previous = SOFA8_ADMIN_FUNNEL[index - 1];
...
export function Sofa8AdminQuickLinks() {
  return (
    <Card sx={{ p: 3 }}>
      ...
      <Grid container spacing={2}>
        {SOFA8_ADMIN_TOOLS.map((tool) => (
          <Grid key={tool.slug} xs={12} sm={6} md={3}>
            <ButtonBase
              component={RouterLink}
              href={`${SOFA8_ADMIN_ROOT}/tools/${tool.slug}`}
              sx={{
                width: 1,
                height: 1,
                p: 2,
