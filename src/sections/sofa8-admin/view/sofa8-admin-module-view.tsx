...
import { type AdminRow, useSofa8AdminRows } from '../sofa8-admin-store';
import { Sofa8AdminLayout, SOFA8_ADMIN_THEME } from './sofa8-admin-layout';
import { SOFA8_ADMIN_ROOT, SOFA8_ADMIN_GROUPS, findSofa8AdminModule } from '../sofa8-admin-data';

// ----------------------------------------------------------------------

const { ACCENT, SURFACE } = SOFA8_ADMIN_THEME;
...
export function Sofa8AdminModuleView() {
  ...
  const found = useMemo(() => findSofa8AdminModule(groupSlug, moduleSlug), [groupSlug, moduleSlug]);

  const group = found?.group;
  const module = found?.module;

  const { rows, createRow, updateRow, deleteRow, deleteRows, resetRows } = useSofa8AdminRows(
    group?.slug ?? '',
    module?.slug ?? ''
  );
  ...
  if (!found || !group || !module) {
    const first = SOFA8_ADMIN_GROUPS[0];
        ...
        <Grid container spacing={3}>
          ...
          <Grid xs={12}>
            <Card>
              ...
              <TableContainer sx={{ borderTop: `1px solid ${alpha('#0B5B78', 0.16)}` }}>
                <Table size="medium">
                  <TableHead>
                    <TableRow>
              ...
              >
                ...
                <Chip
                  ...
                  sx={{ mr: 2, mb: { xs: 2, sm: 0 }, bgcolor: alpha(ACCENT, 0.14), color: '#0B5B78' }}
                />
              </Stack>
            </Card>
          </Grid>
