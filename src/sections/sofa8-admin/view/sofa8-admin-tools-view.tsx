...
import {
  ...
  SOFA8_ADMIN_TOOLS,
  SOFA8_NOTIFICATIONS,
  SOFA8_SETTINGS_FIELDS,
  SOFA8_SETTINGS_TOGGLES,
  findSofa8AdminTool,
} from '../sofa8-admin-tools';
import { Sofa8AdminCharts } from './sofa8-admin-charts';
import { Sofa8AdminFunnel, Sofa8AdminRevenueChart } from './sofa8-admin-insights';
import { Sofa8AdminLayout } from './sofa8-admin-layout';
import { SOFA8_ADMIN_ROOT } from '../sofa8-admin-data';

// ----------------------------------------------------------------------

const ACCENT = '#E5B876';
const SURFACE = '#0B2E3F';
...
export function Sofa8AdminToolsView() {
  const { tool: toolSlug } = useParams();
  const tool = findSofa8AdminTool(toolSlug ?? 'reports');
  ...
  const [settings, setSettings] = useState<SettingsValues>(() =>
    Object.fromEntries(SOFA8_SETTINGS_FIELDS.map((field) => [field.key, field.value]))
  );
  const [toggles, setToggles] = useState<ToggleValues>(() =>
    Object.fromEntries(SOFA8_SETTINGS_TOGGLES.map((item) => [item.key, item.on]))
  );

  const notifications = useMemo(
    () =>
      SOFA8_NOTIFICATIONS.filter((item) => {
  ...
  if (!tool) {
    return <Navigate to={`${SOFA8_ADMIN_ROOT}/tools/reports`} replace />;
  }
    ...
    <Stack spacing={3}>
      <Card>
        ...
        <Grid container spacing={2.5} sx={{ p: 3 }}>
          {SOFA8_SETTINGS_FIELDS.map((field) => (
      ...
      <Card>
        ...
        <Grid container spacing={1} sx={{ p: 2.5 }}>
          {SOFA8_SETTINGS_TOGGLES.map((item) => (
      ...
      <Grid xs={12} md={5}>
        <Card sx={{ p: 3, height: 1 }}>
          <Stack alignItems="center" spacing={1.5} sx={{ textAlign: 'center' }}>
            ...
            <Typography variant="h6">{SOFA8_PROFILE.name}</Typography>
            <Chip label={SOFA8_PROFILE.role} color="warning" variant="soft" />
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>{SOFA8_PROFILE.email}</Typography>
          </Stack>
          <Divider sx={{ my: 3 }} />
          <Stack spacing={1.5}>
            {[
              ['Số điện thoại', SOFA8_PROFILE.phone],
              ['Bộ phận', SOFA8_PROFILE.team],
              ['Tham gia từ', SOFA8_PROFILE.joined],
            ].map(([label, value]) => (
        ...
        <Stack spacing={3}>
          <Card>
            ...
            <Stack direction="row" flexWrap="wrap" useFlexGap gap={1} sx={{ p: 3 }}>
              {SOFA8_PROFILE.permissions.map((permission) => (
          ...
          <Card>
            ...
            <Divider />
            <Stack divider={<Divider />}>
              {SOFA8_PROFILE.sessions.map((session) => (
  ...
  return (
    <>
      <Helmet>
        <title>{`${tool.name} | Quản trị Sofa8`}</title>
      ...
      >
        <Stack spacing={3}>
          <Stack direction="row" spacing={1} sx={{ overflowX: 'auto', pb: 0.5 }}>
            {SOFA8_ADMIN_TOOLS.map((item) => (
              <Button
                key={item.slug}
                href={`${SOFA8_ADMIN_ROOT}/tools/${item.slug}`}
