...
import { type AdminRow, useSofa4AdminRows } from '../sofa4-admin-store';
import { Sofa4AdminLayout, SOFA4_ADMIN_THEME } from './sofa4-admin-layout';
import { SOFA4_ADMIN_ROOT, SOFA4_ADMIN_GROUPS, findSofa4AdminModule } from '../sofa4-admin-data';

// ----------------------------------------------------------------------

const { ACCENT, SURFACE } = SOFA4_ADMIN_THEME;
...
export function Sofa4AdminModuleView() {
  const { group: groupSlug, module: moduleSlug } = useParams();

  const found = useMemo(() => findSofa4AdminModule(groupSlug, moduleSlug), [groupSlug, moduleSlug]);

  const group = found?.group;
  const module = found?.module;

  const { rows, createRow, updateRow, deleteRow, deleteRows, resetRows } = useSofa4AdminRows(
    group?.slug ?? '',
    module?.slug ?? ''
  );
  ...
  if (!found || !group || !module) {
    const first = SOFA4_ADMIN_GROUPS[0];
    return <Navigate to={`${SOFA4_ADMIN_ROOT}/${first.slug}/${first.modules[0].slug}`} replace />;
  }
  ...
  return (
    <>
      <Helmet>
        <title>{`${module.name} | ${group.name} - Quản trị Sofa4`}</title>
