...
import { SOFA4_ADMIN_ROOT, SOFA4_ADMIN_GROUPS } from '../sofa4-admin-data';
...
export function Sofa4AdminLayout({
  children,
  activeGroup,
  activeModule,
  title,
  ...
  const [openGroups, setOpenGroups] = useState<string[]>(
    activeGroup ? [activeGroup] : [SOFA4_ADMIN_GROUPS[0].slug]
  );
    ...
    <Stack sx={{ width: NAV_WIDTH, height: 1, bgcolor: BG, color: TEXT }}>
                  ...
                  {group.modules.map((module) => {
                    ...
                    return (
                      <ButtonBase
                        key={module.slug}
                        component={RouterLink}
                        href={`${SOFA4_ADMIN_ROOT}/${group.slug}/${module.slug}`}
                        sx={{
                          gap: 1.25,
                          px: 1.5,
                          py: 0.85,
      ...
      <Stack direction="row" alignItems="center" spacing={1.5} sx={{ p: 2 }}>
        ...
        <Tooltip title="Về trang bán hàng">
          <IconButton component={RouterLink} href="/sofa4" sx={{ color: alpha(TEXT, 0.7) }}>
...
export const SOFA4_ADMIN_THEME = { BG, SURFACE, ACCENT, TEXT };
