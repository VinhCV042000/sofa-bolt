// SOFA4 ADMIN — cấu hình khu vực quản trị (dữ liệu demo)
// ----------------------------------------------------------------------

export const SOFA4_ADMIN_ROOT = '/sofa4/admin';

export type Sofa4AdminColumn = { key: string; label: string; type?: 'text' | 'status' | 'money' | 'number' };

export type Sofa4AdminModule = {
  slug: string;
  name: string;
  description: string;
  icon: string;
  stats: { label: string; value: string; trend?: string }[];
  columns: Sofa4AdminColumn[];
  rows: Record<string, string | number>[];
  actions?: string[];
};

export type Sofa4AdminGroup = {
  slug: string;
  name: string;
  icon: string;
  modules: Sofa4AdminModule[];
};
...
const STATUS_COL: Sofa4AdminColumn = { key: 'status', label: 'Trạng thái', type: 'status' };
...
const CMS_GROUP: Sofa4AdminGroup = {
  slug: 'cms',
  name: 'CMS',
  icon: 'solar:documents-bold-duotone',
  modules: [
    cmsPage('home', 'Trang chủ', '/sofa4'),
    cmsPage('about', 'Trang giới thiệu', '/sofa4/about'),
    cmsPage('contact', 'Trang liên hệ', '/sofa4/contact'),
    cmsPage('policy', 'Trang chính sách', '/sofa4/policy'),
    cmsPage('terms', 'Trang điều khoản', '/sofa4/policy/terms'),
    cmsPage('faq', 'Trang FAQ', '/sofa4/faq'),
    mod(
      'blog',
      'Trang blog',
    ...
    mod(
      ...
      [
        { page: '/sofa4', title: 'SOFA4 – Sofa thủ công cao cấp', length: 54, status: 'Tốt' },
        { page: '/sofa4/about', title: 'Về chúng tôi | SOFA4', length: 42, status: 'Tốt' },
        { page: '/sofa4/faq', title: '', length: 0, status: 'Thiếu meta' },
        { page: '/sofa4/policy', title: 'Chính sách mua hàng SOFA4', length: 48, status: 'Tốt' },
      ],
      ['Quét lại']
    ),
    mod(
      ...
      [
        { title: 'Hướng dẫn đo kích thước', slug: '/sofa4/huong-dan-do', status: 'Đã xuất bản', updated: '08/08/2026' },
        { title: 'Cam kết chất liệu', slug: '/sofa4/cam-ket', status: 'Đã xuất bản', updated: '01/08/2026' },
        { title: 'Tuyển đại lý 2026', slug: '/sofa4/tuyen-dai-ly', status: 'Bản nháp', updated: '13/08/2026' },
      ],
      ['Tạo trang']
    ),
  ],
...
const PRODUCT_GROUP: Sofa4AdminGroup = {
  slug: 'catalog',
  name: 'Sản phẩm',
...
const WAREHOUSE_GROUP: Sofa4AdminGroup = {
  slug: 'warehouse',
  name: 'Kho hàng',
...
const ORDER_GROUP: Sofa4AdminGroup = {
  slug: 'orders',
  name: 'Đơn hàng',
...
const CRM_GROUP: Sofa4AdminGroup = {
  slug: 'crm',
  name: 'CRM',
...
const MARKETING_GROUP: Sofa4AdminGroup = {
  slug: 'marketing',
  name: 'Marketing',
...
const ANALYTICS_GROUP: Sofa4AdminGroup = {
  slug: 'analytics',
  name: 'Analytics',
...
const seoPage = (slug: string, name: string, sample: string) =>
  mod(
    ...
    [
      { url: sample, title: `${name} – SOFA4`, keyword: 'sofa cao cấp', position: 4, status: 'Tốt' },
...
const SEO_GROUP: Sofa4AdminGroup = {
  slug: 'seo',
  name: 'SEO',
  icon: 'solar:graph-new-bold-duotone',
  modules: [
    seoPage('category', 'Trang danh mục', '/sofa4/products/category'),
    seoPage('product', 'Trang sản phẩm', '/sofa4/products'),
    seoPage('collection', 'Trang bộ sưu tập', '/sofa4/collections'),
    seoPage('project', 'Trang dự án', '/sofa4/projects'),
    seoPage('showroom', 'Trang showroom', '/sofa4/showrooms'),
    seoPage('blog', 'Trang blog', '/sofa4/blog'),
    seoPage('brand', 'Trang thương hiệu', '/sofa4/about'),
    mod(
      'sitemap',
      'Sitemap',
    ...
    mod(
      ...
      [
        ...
        { agent: '*', rule: 'Disallow', path: '/sofa4/admin', status: 'Hoạt động' },
        { agent: '*', rule: 'Disallow', path: '/sofa4/cart', status: 'Hoạt động' },
...
const ACCESS_GROUP: Sofa4AdminGroup = {
  slug: 'access',
  name: 'Phân quyền',
  ...
  modules: [
    mod(
      ...
      [
        { name: 'Ngọc Anh', email: 'ngocanh@sofa4.vn', role: 'Quản trị hệ thống', last: '15/08/2026 09:12', status: 'Hoạt động' },
        { name: 'Minh Quân', email: 'quan@sofa4.vn', role: 'Quản lý bán hàng', last: '14/08/2026 17:40', status: 'Hoạt động' },
        { name: 'Thu Hà', email: 'ha@sofa4.vn', role: 'CSKH', last: '15/08/2026 08:02', status: 'Hoạt động' },
        { name: 'Gia Bảo', email: 'bao@sofa4.vn', role: 'Biên tập nội dung', last: '02/07/2026 10:20', status: 'Bị khoá' },
      ],
      ['Mời người dùng']
    ),
    mod(
...
export const SOFA4_ADMIN_GROUPS: Sofa4AdminGroup[] = [
  CMS_GROUP,
  PRODUCT_GROUP,
  WAREHOUSE_GROUP,
  ORDER_GROUP,
...
export function findSofa4AdminModule(groupSlug?: string, moduleSlug?: string) {
  const group = SOFA4_ADMIN_GROUPS.find((g) => g.slug === groupSlug);
  if (!group) return null;
  const found = group.modules.find((m) => m.slug === moduleSlug) ?? group.modules[0];
  return { group, module: found };
}

export const SOFA4_ADMIN_KPIS = [
...
export const SOFA4_ADMIN_ACTIVITIES = [
