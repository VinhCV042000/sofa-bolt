...
import { SOFA10_COLORS, SOFA10_FAQS } from './sofa10-data';

// ----------------------------------------------------------------------

export function Sofa10Faqs({ sx, ...other }: BoxProps) {
  return (
    <Box
      component="section"
      id="faqs"
          ...
          {SOFA10_FAQS.map((faq, index) => (
            <Accordion
              ...
              sx={{
                ...
                '&:before': { display: 'none' },
                '& + &': { mt: 0.5 },
                borderBottom: `1px solid ${varAlpha(SOFA10_COLORS.stoneLight, 0.3)}`,
                borderRadius: 0,
                boxShadow: 'none',
              }}
            >
              <AccordionSummary
                expandIcon={<Iconify icon="eva:plus-outline" sx={{ color: SOFA10_COLORS.wood }} />}
              ...
              <AccordionDetails sx={{ pb: 3 }}>
                <Typography variant="body2" sx={{ color: 'text.secondary', lineHeight: 2, fontWeight: 'fontWeightLight' }}>
                  {faq.answer}
                </Typography>
              </AccordionDetails>
            </Accordion>
