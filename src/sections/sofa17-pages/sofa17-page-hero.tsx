...
import { SOFA17_COLORS } from '../sofa17/sofa17-data';

interface Sofa17PageHeroProps {
...
export function Sofa17PageHero({ title, subtitle, image, overline, sx, ...other }: Sofa17PageHeroProps & Omit<BoxProps, 'title'>) {
          ...
          {overline && (
            <Box component={m.div} variants={varFade({ distance: 24 }).inUp}>
              <Stack direction="row" alignItems="center" spacing={1.5}>
                ...
                <Typography variant="overline" sx={{ color: SOFA17_COLORS.gold, letterSpacing: 4, fontWeight: 'fontWeightMedium', textTransform: 'uppercase' }}>{overline}</Typography>
              </Stack>
            </Box>
          )}
          <Box component={m.div} variants={varFade({ distance: 24 }).inUp}>
            <Typography component="h1" sx={{ color: 'common.white', fontFamily: theme.typography.fontSecondaryFamily, fontSize: { xs: 32, md: 52 }, lineHeight: 1.1, fontWeight: 'fontWeightLight', letterSpacing: -0.5, '& span': { ...textGradient(`90deg, ${SOFA17_COLORS.goldLight} 0%, ${SOFA17_COLORS.gold} 50%, ${SOFA17_COLORS.goldDark} 100%`), fontStyle: 'italic' } }}>{title}</Typography>
          </Box>
          {subtitle && (
            <Box component={m.div} variants={varFade({ distance: 24 }).inUp}>
...
export function Sofa17Section({ children, bg = 'black', py = { xs: 8, md: 12 } }: { children: React.ReactNode; bg?: 'black' | 'charcoal' | 'ivory' | 'pearl'; py?: { xs: number; md: number } }) {
  const bgcolor = bg === 'charcoal' ? SOFA17_COLORS.charcoal : bg === 'ivory' ? SOFA17_COLORS.ivory : bg === 'pearl' ? SOFA17_COLORS.pearl : SOFA17_COLORS.black;
  const color = bg === 'ivory' || bg === 'pearl' ? SOFA17_COLORS.black : SOFA17_COLORS.ivory;
  return (
