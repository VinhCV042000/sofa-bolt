...
export function Sofa17LinkGrid({ items, cols = 4 }: { items: Sofa17LinkItem[]; cols?: number }) {
          ...
          <Grid key={item.title} xs={12} sm={6} md={12 / cols}>
            <Stack
              ...
              sx={{
                ...
                height: 1,
                textDecoration: 'none',
                bgcolor: SOFA17_COLORS.charcoal,
                border: `1px solid ${varAlpha(SOFA17_COLORS.gold, 0.25)}`,
                transition: 'all 0.25s',
                ...(interactive && { '&:hover': { borderColor: SOFA17_COLORS.gold, transform: 'translateY(-4px)' } }),
              }}
            >
              {item.icon && (
                <Box
                  sx={{
                    ...
                    color: SOFA17_COLORS.gold,
                    border: `1px solid ${varAlpha(SOFA17_COLORS.gold, 0.4)}`,
                  }}
                >
...
export function Sofa17Panel({ title, children }: { title?: string; children: React.ReactNode }) {
  const theme = useTheme();
  return (
    <Stack spacing={3} sx={{ p: { xs: 3, md: 4 }, bgcolor: SOFA17_COLORS.charcoal, border: `1px solid ${varAlpha(SOFA17_COLORS.gold, 0.25)}` }}>
      {title && (
...
export function Sofa17SectionTitle({ children }: { children: React.ReactNode }) {
  const theme = useTheme();
  return (
    <Typography
      variant="h4"
...
export function sofa17Price(value: number) {
