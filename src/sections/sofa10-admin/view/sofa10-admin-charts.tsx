...
import {
  ...
  SOFA10_ADMIN_TOP_PRODUCTS,
  SOFA10_ADMIN_ORDER_SERIES,
  SOFA10_ADMIN_CHANNEL_CHART,
  SOFA10_ADMIN_REVENUE_SERIES,
} from '../sofa10-admin-config';

const PALETTE = ['#7C8A6B', '#C4A882', '#8B7355', '#A3B18A', '#3E4C3C'];

export function Sofa10AdminCharts() {
  ...
  const channelOptions = useChart({
    colors: PALETTE,
    labels: SOFA10_ADMIN_CHANNEL_CHART.labels,
    stroke: { width: 0 },
  ...
  const topProductOptions = useChart({
    colors: [PALETTE[0]],
    xaxis: { categories: SOFA10_ADMIN_TOP_PRODUCTS.labels },
    ...
    <Grid container spacing={3}>
      <Grid xs={12} md={8}>
        <Card>
          ...
          <Chart
            type="line"
            series={SOFA10_ADMIN_REVENUE_SERIES}
      ...
      <Grid xs={12} md={4}>
        <Card sx={{ height: 1 }}>
          <CardHeader title="Kênh bán hàng" subheader="Tỷ trọng theo kênh (%)" />
          <Chart
            type="donut"
            series={SOFA10_ADMIN_CHANNEL_CHART.series}
      ...
      <Grid xs={12} md={6}>
        <Card>
          ...
          <Chart
            type="bar"
            series={SOFA10_ADMIN_ORDER_SERIES}
      ...
      <Grid xs={12} md={6}>
        <Card>
          ...
          <Chart
            type="bar"
            series={[{ name: 'Đã bán', data: SOFA10_ADMIN_TOP_PRODUCTS.series }]}
      ...
      <Grid xs={12}>
        <Card sx={{ p: 3 }}>
          <Stack direction="row" justifyContent="space-between" flexWrap="wrap" spacing={2}>
            {[
              { label: 'Giá trị đơn trung bình', value: '14,6tr' },
              { label: 'Tỷ lệ giữ khách', value: '62%' },
              { label: 'Đơn hoàn / đổi trả', value: '0,9%' },
              { label: 'Tồn kho khả dụng', value: '2.920' },
            ].map((item) => (
