...
import {
  SOFA2_ADMIN_ROOT,
  SOFA2_ADMIN_CHART_MONTHS,
  SOFA2_ADMIN_REVENUE_SERIES,
} from '../sofa2-admin-data';
import { SOFA2_ADMIN_TOOLS, SOFA2_ADMIN_FUNNEL } from '../sofa2-admin-tools';
...
export function Sofa2AdminRevenueChart() {
  const actual = SOFA2_ADMIN_REVENUE_SERIES[0].data;
  const target = SOFA2_ADMIN_REVENUE_SERIES[1].data;
  ...
  const options = useChart({
    colors: [ACCENT, GREY, alpha(SURFACE, 0.5)],
    stroke: { width: [0, 3, 2], dashArray: [0, 0, 6] },
    xaxis: { categories: SOFA2_ADMIN_CHART_MONTHS },
...
export function Sofa2AdminFunnel() {
  const top = SOFA2_ADMIN_FUNNEL[0].value;
  ...
  return (
    <Card sx={{ height: 1 }}>
      ...
      <Stack spacing={2} sx={{ p: 3 }}>
        {SOFA2_ADMIN_FUNNEL.map((step, index) => {
          const ratio = (step.value / top) * 100;
          const previous = SOFA2_ADMIN_FUNNEL[index - 1];
              ...
              >
                <Box
                  sx={{
                    ...
                    fontSize: 12,
                    fontWeight: 700,
                    background: `linear-gradient(90deg, ${ACCENT}, ${GREY})`,
...
export function Sofa2AdminQuickLinks() {
  return (
    <Card sx={{ p: 3 }}>
        ...
        {SOFA2_ADMIN_TOOLS.map((tool) => (
          <Grid key={tool.slug} xs={12} sm={6} md={3}>
            <ButtonBase
              ...
              href={`${SOFA2_ADMIN_ROOT}/tools/${tool.slug}`}
              sx={{
                width: 1,
                height: 1,
                p: 2,
                ...
                border: `1px solid ${alpha(SURFACE, 0.14)}`,
                transition: 'all .2s',
                '&:hover': { borderColor: ACCENT, bgcolor: alpha(ACCENT, 0.04) },
              }}
            >
              <Box sx={{ color: ACCENT }}>
