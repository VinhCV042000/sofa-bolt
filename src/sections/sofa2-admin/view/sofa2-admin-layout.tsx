...
import { SOFA2_ADMIN_TOOLS } from '../sofa2-admin-tools';
...
export function Sofa2AdminLayout({
  ...
  const brandName = 'LUXE Sofa';
  const siteLink = '/sofa2';
  ...
  const nav = (
    <Stack sx={{ width: NAV_WIDTH, height: 1, bgcolor: BG, color: TEXT }}>
      <Stack
        ...
        alignItems="center"
        spacing={1.5}
        sx={{ px: 2.5, py: 3, borderBottom: `1px solid ${alpha(ACCENT, 0.12)}` }}
      >
        <Box
          sx={{
            width: 40,
        ...
        <Box>
          <Typography variant="subtitle1" sx={{ lineHeight: 1.2, fontWeight: 800, letterSpacing: 1 }}>
            {brandName}
          </Typography>
          <Typography variant="caption" sx={{ color: alpha(TEXT, 0.5), letterSpacing: 0.5 }}>
            Trung tâm quản trị
          </Typography>
        </Box>
      </Stack>
      ...
      <Box sx={{ flex: 1, overflowY: 'auto', px: 1.5, py: 2 }}>
        <ButtonBase
          ...
          sx={{
            ...
            color: !activeGroup ? ACCENT : alpha(TEXT, 0.7),
            bgcolor: !activeGroup ? alpha(ACCENT, 0.1) : 'transparent',
            ...
            <Box key={group.slug} sx={{ mb: 0.5 }}>
              <ButtonBase
                ...
                sx={{
                  ...
                  borderRadius: 1.25,
                  justifyContent: 'flex-start',
                  color: isActiveGroup ? ACCENT : alpha(TEXT, 0.75),
                    ...
                    return (
                      <ButtonBase
                        ...
                        sx={{
                          ...
                          borderRadius: 1,
                          justifyContent: 'flex-start',
                          color: selected ? ACCENT : alpha(TEXT, 0.55),
                          bgcolor: selected ? alpha(ACCENT, 0.08) : 'transparent',
                          '&:hover': { bgcolor: alpha(ACCENT, 0.06), color: TEXT },
                        }}
                      >
                        <Box
                          sx={{
        ...
        <Typography
          variant="overline"
          sx={{ display: 'block', px: 1.5, pt: 2, pb: 1, color: alpha(TEXT, 0.4) }}
        >
          Tiện ích
        </Typography>

        {SOFA2_ADMIN_TOOLS.map((tool) => (
          <ButtonBase
            key={tool.slug}
            component={RouterLink}
      ...
      <Stack direction="row" alignItems="center" spacing={1.5} sx={{ p: 2 }}>
        <Avatar sx={{ bgcolor: alpha(ACCENT, 0.16), color: ACCENT, width: 36, height: 36 }}>MA</Avatar>
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Typography variant="subtitle2" noWrap>
            Minh Anh
          </Typography>
          <Typography variant="caption" sx={{ color: alpha(TEXT, 0.5) }} noWrap>
            Quản trị viên
          </Typography>
        </Box>
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        <Box
          ...
          sx={{
            ...
            borderBottom: `1px solid ${alpha(SURFACE, 0.12)}`,
          }}
        >
          <Container maxWidth={false} sx={{ maxWidth: 1440 }}>
            <Stack direction="row" alignItems="center" spacing={2} sx={{ py: 2 }}>
              ...
              <Box sx={{ flex: 1, minWidth: 0 }}>
                ...
                <Typography variant="h5" sx={{ color: BG, fontWeight: 800 }} noWrap>
                  {title}
                </Typography>
              </Box>
              <Chip
                ...
                label="Dữ liệu demo"
                sx={{ bgcolor: alpha(BG, 0.08), color: SURFACE, fontWeight: 600 }}
              />
            </Stack>
          </Container>
        </Box>
...
export const SOFA2_ADMIN_THEME = { BG, SURFACE, ACCENT, ACCENT_DEEP, TEXT };
