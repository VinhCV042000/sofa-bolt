...
import {
  ...
  SOFA2_ADMIN_TOP_PRODUCTS,
  SOFA2_ADMIN_ORDER_SERIES,
  SOFA2_ADMIN_CHANNEL_CHART,
  SOFA2_ADMIN_REVENUE_SERIES,
} from '../sofa2-admin-data';

// ----------------------------------------------------------------------

const PALETTE = ['#1A1A1A', '#455A64', '#607D8B', '#90A4AE', '#CFD8DC'];

export function Sofa2AdminCharts() {
  ...
  const channelOptions = useChart({
    colors: PALETTE,
    labels: SOFA2_ADMIN_CHANNEL_CHART.labels,
    stroke: { width: 0 },
  ...
  const topProductOptions = useChart({
    colors: [PALETTE[0]],
    xaxis: { categories: SOFA2_ADMIN_TOP_PRODUCTS.labels },
    ...
    <Grid container spacing={3}>
      <Grid xs={12} md={8}>
        <Card>
          ...
          <Chart
            type="line"
            series={SOFA2_ADMIN_REVENUE_SERIES}
      ...
      <Grid xs={12} md={4}>
        <Card sx={{ height: 1 }}>
          ...
          <Chart
            type="donut"
            series={SOFA2_ADMIN_CHANNEL_CHART.series}
      ...
      <Grid xs={12} md={6}>
        <Card>
          ...
          <Chart
            type="bar"
            series={SOFA2_ADMIN_ORDER_SERIES}
      ...
      <Grid xs={12} md={6}>
        <Card>
          ...
          <Chart
            type="bar"
            series={[{ name: 'Đã bán', data: SOFA2_ADMIN_TOP_PRODUCTS.series }]}
        ...
        <Card sx={{ p: 3 }}>
          <Stack direction="row" justifyContent="space-between" flexWrap="wrap" spacing={2}>
            {[
              ...
              { label: 'Đơn hoàn / đổi trả', value: '1.3%' },
              { label: 'Tồn kho khả dụng', value: '3.248' },
            ].map((item) => (
