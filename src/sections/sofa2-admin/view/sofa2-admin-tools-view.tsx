...
import {
  ...
  SOFA2_ADMIN_TOOLS,
  SOFA2_NOTIFICATIONS,
  SOFA2_SETTINGS_FIELDS,
  SOFA2_SETTINGS_TOGGLES,
  findSofa2AdminTool,
} from '../sofa2-admin-tools';
import { Sofa2AdminCharts } from './sofa2-admin-charts';
import { Sofa2AdminFunnel, Sofa2AdminRevenueChart } from './sofa2-admin-insights';
import { Sofa2AdminLayout } from './sofa2-admin-layout';
import { SOFA2_ADMIN_ROOT } from '../sofa2-admin-data';
...
export function Sofa2AdminToolsView() {
  ...
  const [settings, setSettings] = useState<SettingsValues>(() =>
    Object.fromEntries(SOFA2_SETTINGS_FIELDS.map((field) => [field.key, field.value]))
  );
  const [toggles, setToggles] = useState<ToggleValues>(() =>
    Object.fromEntries(SOFA2_SETTINGS_TOGGLES.map((item) => [item.key, item.on]))
  );

  const notifications = useMemo(
    () =>
      SOFA2_NOTIFICATIONS.filter((item) => {
    ...
    <Stack spacing={3}>
      <Card>
        ...
        <Grid container spacing={2.5} sx={{ p: 3 }}>
          {SOFA2_SETTINGS_FIELDS.map((field) => (
      ...
      <Grid xs={12} md={5}>
        <Card sx={{ p: 3, height: 1 }}>
          <Stack alignItems="center" spacing={1.5} sx={{ textAlign: 'center' }}>
            ...
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>{SOFA2_PROFILE.email}</Typography>
          </Stack>
          <Divider sx={{ my: 3 }} />
          <Stack spacing={1.5}>
            {[
      ...
      >
        <Stack spacing={3}>
          <Stack direction="row" spacing={1} sx={{ overflowX: 'auto', pb: 0.5 }}>
            {SOFA2_ADMIN_TOOLS.map((item) => (
              <Button
                key={item.slug}
                href={`${SOFA2_ADMIN_ROOT}/tools/${item.slug}`}
