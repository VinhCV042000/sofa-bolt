...
import {
  SOFA19_PROFILE,
  SOFA19_ADMIN_TOOLS,
  SOFA19_NOTIFICATIONS,
  SOFA19_SETTINGS_FIELDS,
  SOFA19_SETTINGS_TOGGLES,
  findSofa19AdminTool,
} from '../sofa19-admin-tools';
import { Sofa19AdminCharts } from './sofa19-admin-charts';
import { Sofa19AdminFunnel, Sofa19AdminRevenueChart } from './sofa19-admin-insights';
import { Sofa19AdminLayout } from './sofa19-admin-layout';
import { SOFA19_ADMIN_ROOT } from '../sofa19-admin-data';
...
export function Sofa19AdminToolsView() {
  const { tool: toolSlug } = useParams();
  const tool = findSofa19AdminTool(toolSlug ?? 'reports');
  ...
  const [settings, setSettings] = useState<SettingsValues>(() =>
    Object.fromEntries(SOFA19_SETTINGS_FIELDS.map((field) => [field.key, field.value]))
  );
  const [toggles, setToggles] = useState<ToggleValues>(() =>
    Object.fromEntries(SOFA19_SETTINGS_TOGGLES.map((item) => [item.key, item.on]))
  );

  const notifications = useMemo(
    () =>
      SOFA19_NOTIFICATIONS.filter((item) => {
  ...
  if (!tool) {
    return <Navigate to={`${SOFA19_ADMIN_ROOT}/tools/reports`} replace />;
  }
    ...
    <Stack spacing={3}>
      <Card>
        ...
        <Grid container spacing={2.5} sx={{ p: 3 }}>
          {SOFA19_SETTINGS_FIELDS.map((field) => (
      ...
      <Card>
        ...
        <Grid container spacing={1} sx={{ p: 2.5 }}>
          {SOFA19_SETTINGS_TOGGLES.map((item) => (
      ...
      <Grid xs={12} md={5}>
        <Card sx={{ p: 3, height: 1 }}>
          <Stack alignItems="center" spacing={1.5} sx={{ textAlign: 'center' }}>
            ...
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>{SOFA19_PROFILE.email}</Typography>
          </Stack>
          <Divider sx={{ my: 3 }} />
          <Stack spacing={1.5}>
            {[
              ['Số điện thoại', SOFA19_PROFILE.phone],
              ['Bộ phận', SOFA19_PROFILE.team],
              ['Tham gia từ', SOFA19_PROFILE.joined],
            ].map(([label, value]) => (
        ...
        <Stack spacing={3}>
          <Card>
            ...
            <Stack direction="row" flexWrap="wrap" useFlexGap gap={1} sx={{ p: 3 }}>
              {SOFA19_PROFILE.permissions.map((permission) => (
          ...
          <Card>
            ...
            <Divider />
            <Stack divider={<Divider />}>
              {SOFA19_PROFILE.sessions.map((session) => (
  ...
  return (
    <>
      <Helmet>
        <title>{`${tool.name} | Quản trị Sofa19`}</title>
      ...
      >
        <Stack spacing={3}>
          <Stack direction="row" spacing={1} sx={{ overflowX: 'auto', pb: 0.5 }}>
            {SOFA19_ADMIN_TOOLS.map((item) => (
              <Button
                key={item.slug}
                href={`${SOFA19_ADMIN_ROOT}/tools/${item.slug}`}
