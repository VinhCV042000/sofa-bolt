...
import { type AdminRow, useSofa19AdminRows } from '../sofa19-admin-store';
import { Sofa19AdminLayout, SOFA19_ADMIN_THEME } from './sofa19-admin-layout';
import { SOFA19_ADMIN_ROOT, SOFA19_ADMIN_GROUPS, findSofa19AdminModule } from '../sofa19-admin-data';

// ----------------------------------------------------------------------

const { ACCENT, ACCENT_DEEP, SURFACE } = SOFA19_ADMIN_THEME;
...
export function Sofa19AdminModuleView() {
  const { group: groupSlug, module: moduleSlug } = useParams();

  const found = useMemo(() => findSofa19AdminModule(groupSlug, moduleSlug), [groupSlug, moduleSlug]);

  const group = found?.group;
  const module = found?.module;

  const { rows, createRow, updateRow, deleteRow, deleteRows, resetRows } = useSofa19AdminRows(
    group?.slug ?? '',
    module?.slug ?? ''
  );
  ...
  if (!found || !group || !module) {
    const first = SOFA19_ADMIN_GROUPS[0];
    return <Navigate to={`${SOFA19_ADMIN_ROOT}/${first.slug}/${first.modules[0].slug}`} replace />;
  }
  ...
  return (
    <>
      <Helmet>
        <title>{`${module.name} | ${group.name} - Quản trị Sofa19`}</title>
