...
export function Sofa19Story({ sx, ...other }: BoxProps) {
          ...
          <Grid xs={12} md={5}>
            ...
            >
              <Box
                component="img"
                alt="Heritage"
                src={SOFA19_IMAGES.story}
                sx={{ width: 1, height: { xs: 300, md: 440 }, objectFit: 'cover' }}
              />
              {/* Golden ornate frame */}
              <Box sx={{ position: 'absolute', top: -10, left: -10, right: -10, bottom: -10, border: `2px solid ${varAlpha(SOFA19_COLORS.golden, 0.3)}`, borderRadius: 3, zIndex: -1 }} />
              {/* Est badge */}
              <Box
                sx={{
                  position: 'absolute',
                  ...
                  <Grid key={item.title} xs={12} sm={6}>
                    <Box
                      ...
                      sx={{
                        p: 3.5,
                        height: 1,
                        bgcolor: varAlpha(SOFA19_COLORS.jungle, 0.3),
                        borderRadius: 3,
                        border: `1px solid ${varAlpha(SOFA19_COLORS.golden, 0.15)}`,
                        transition: (theme) => theme.transitions.create('box-shadow', { duration: 400 }),
                        '&:hover': { boxShadow: `0 12px 32px -12px ${varAlpha(SOFA19_COLORS.golden, 0.25)}` },
                      }}
                    >
                      <Stack spacing={2}>
                        <Box
                          sx={{
                            ...
                            bgcolor: varAlpha(SOFA19_COLORS.golden, 0.15),
                          }}
                        >
                          <Iconify icon={STORY_ICONS[index]} width={22} sx={{ color: SOFA19_COLORS.golden }} />
                        </Box>
                        <Stack spacing={0.75}>
                          <Typography variant="subtitle1" sx={{ fontWeight: 'fontWeightMedium', color: SOFA19_COLORS.cream, letterSpacing: 0.3 }}>
                            {item.title}
                          </Typography>
                          <Typography variant="body2" sx={{ color: varAlpha(SOFA19_COLORS.cream, 0.6), lineHeight: 1.8, fontWeight: 'fontWeightLight' }}>
                            {item.text}
                          </Typography>
                        </Stack>
                      </Stack>
