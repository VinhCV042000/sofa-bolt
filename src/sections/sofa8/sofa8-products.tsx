...
import { SOFA8_COLORS, SOFA8_PRODUCTS } from './sofa8-data';

// ----------------------------------------------------------------------

export function Sofa8Products({ sx, ...other }: BoxProps) {
  return (
    <Box component="section" id="products" sx={{ py: { xs: 8, md: 12 }, ...sx }} {...other}>
      <Container component={MotionViewport}>
        ...
        <Grid container spacing={3}>
          {SOFA8_PRODUCTS.map((product, index) => (
            <Grid key={product.name} xs={12} sm={6} md={4}>
              <Card
                ...
                sx={{
                  ...
                  border: (theme) => `1px solid ${varAlpha(SOFA8_COLORS.oceanLight, 0.2)}`,
              ...
              >
                <Box sx={{ position: 'relative', overflow: 'hidden' }}>
                  ...
                  <Label
                    sx={{
                      ...
                      position: 'absolute',
                      borderRadius: 20,
                      bgcolor: product.badge.includes('%') ? SOFA8_COLORS.coral : SOFA8_COLORS.ocean,
                      color: 'common.white',
                    }}
                  >
                    {product.badge}
                ...
                <Stack spacing={1.5} sx={{ p: 3, flex: 1 }}>
                  <Typography variant="overline" sx={{ color: SOFA8_COLORS.oceanLight, lineHeight: 1.2, letterSpacing: 1 }}>
                    {product.category}
                  </Typography>
                  <Typography variant="h6" sx={{ fontWeight: 'fontWeightBold', flex: 1, color: SOFA8_COLORS.navy }}>
                    {product.name}
                  </Typography>
                  ...
                  <Stack direction="row" alignItems="center" spacing={0.5}>
                    <Rating value={product.rating} readOnly size="small" precision={0.1} sx={{ color: SOFA8_COLORS.coral }} />
                    <Typography variant="caption" sx={{ color: 'text.secondary' }}>
                      ({product.reviews})
                    </Typography>
                  </Stack>
                    ...
                    {product.colors.map((color) => (
                      <Box
                        ...
                        sx={{
                          ...
                          borderRadius: '50%',
                          bgcolor: color,
                          border: `2px solid ${varAlpha(SOFA8_COLORS.oceanLight, 0.25)}`,
                        }}
                      />
                    ))}
                  </Stack>
                  <Stack direction="row" alignItems="baseline" spacing={1}>
                    <Typography variant="h6" sx={{ color: SOFA8_COLORS.ocean, fontWeight: 'fontWeightBold' }}>
                  ...
                  <Button
                    ...
                    sx={{
                      borderRadius: 24,
                      bgcolor: varAlpha(SOFA8_COLORS.ocean, 0.12),
                      color: SOFA8_COLORS.oceanDeep,
                      '&:hover': { bgcolor: varAlpha(SOFA8_COLORS.ocean, 0.22) },
                    }}
                  >
                    Thêm vào giỏ
                  </Button>
