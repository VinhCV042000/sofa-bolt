...
import { _sofa1B2BLeads } from 'src/_mock/_sofa1';
...
import {
  RenderCellSofa1B2BLead,
  RenderCellSofa1B2BLeadStatus,
  RenderCellSofa1B2BLeadCreatedAt,
} from '../sofa1-b2b-lead-table-row';

// ----------------------------------------------------------------------

const HIDE_COLUMNS_TOGGLABLE = ['actions'];

export function Sofa1B2BLeadListView() {
  const confirmRows = useBoolean();

  const [tableData, setTableData] = useState(_sofa1B2BLeads);
  ...
  const columns: GridColDef[] = [
    {
      ...
      minWidth: 260,
      hideable: false,
      renderCell: (params) => <RenderCellSofa1B2BLead params={params} />,
    },
    { field: 'contactName', headerName: 'Người liên hệ', width: 160 },
    { field: 'phone', headerName: 'Điện thoại', width: 140 },
    { field: 'projectType', headerName: 'Loại dự án', width: 140 },
    {
      field: 'status',
      headerName: 'Trạng thái',
      width: 150,
      renderCell: (params) => (
        <RenderCellSofa1B2BLeadStatus params={params} onChangeStatus={handleChangeStatus} />
      ),
    },
    {
      field: 'createdAt',
      headerName: 'Ngày tạo',
      width: 160,
      renderCell: (params) => <RenderCellSofa1B2BLeadCreatedAt params={params} />,
    },
    {
      type: 'actions',
      field: 'actions',
  ...
  return (
    <>
      <DashboardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
        <CustomBreadcrumbs
          heading="Khách hàng doanh nghiệp"
          links={[
      ...
      <ConfirmDialog
        ...
        content={
          <>
            Bạn có chắc muốn xóa <strong> {selectedRowIds.length} </strong> yêu cầu?
          </>
        }
        action={
          <Button
