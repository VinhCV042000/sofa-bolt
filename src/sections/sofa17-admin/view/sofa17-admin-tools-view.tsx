...
import {
  SOFA17_PROFILE,
  SOFA17_ADMIN_TOOLS,
  SOFA17_NOTIFICATIONS,
  SOFA17_SETTINGS_FIELDS,
  SOFA17_SETTINGS_TOGGLES,
  findSofa17AdminTool,
} from '../sofa17-admin-tools';
import { Sofa17AdminCharts } from './sofa17-admin-charts';
import { Sofa17AdminFunnel, Sofa17AdminRevenueChart } from './sofa17-admin-insights';
import { Sofa17AdminLayout } from './sofa17-admin-layout';
import { SOFA17_ADMIN_ROOT } from '../sofa17-admin-data';
...
export function Sofa17AdminToolsView() {
  const { tool: toolSlug } = useParams();
  const tool = findSofa17AdminTool(toolSlug ?? 'reports');
  ...
  const [settings, setSettings] = useState<SettingsValues>(() =>
    Object.fromEntries(SOFA17_SETTINGS_FIELDS.map((field) => [field.key, field.value]))
  );
  const [toggles, setToggles] = useState<ToggleValues>(() =>
    Object.fromEntries(SOFA17_SETTINGS_TOGGLES.map((item) => [item.key, item.on]))
  );

  const notifications = useMemo(
    () =>
      SOFA17_NOTIFICATIONS.filter((item) => {
  ...
  if (!tool) {
    return <Navigate to={`${SOFA17_ADMIN_ROOT}/tools/reports`} replace />;
  }
    ...
    <Stack spacing={3}>
      <Card>
        ...
        <Grid container spacing={2.5} sx={{ p: 3 }}>
          {SOFA17_SETTINGS_FIELDS.map((field) => (
      ...
      <Card>
        ...
        <Grid container spacing={1} sx={{ p: 2.5 }}>
          {SOFA17_SETTINGS_TOGGLES.map((item) => (
      ...
      <Grid xs={12} md={5}>
        <Card sx={{ p: 3, height: 1 }}>
          <Stack alignItems="center" spacing={1.5} sx={{ textAlign: 'center' }}>
            ...
            <Typography variant="h6">{SOFA17_PROFILE.name}</Typography>
            <Chip label={SOFA17_PROFILE.role} color="warning" variant="soft" />
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>{SOFA17_PROFILE.email}</Typography>
          </Stack>
          <Divider sx={{ my: 3 }} />
          <Stack spacing={1.5}>
            {[
              ['Số điện thoại', SOFA17_PROFILE.phone],
              ['Bộ phận', SOFA17_PROFILE.team],
              ['Tham gia từ', SOFA17_PROFILE.joined],
            ].map(([label, value]) => (
        ...
        <Stack spacing={3}>
          <Card>
            ...
            <Stack direction="row" flexWrap="wrap" useFlexGap gap={1} sx={{ p: 3 }}>
              {SOFA17_PROFILE.permissions.map((permission) => (
          ...
          <Card>
            ...
            <Divider />
            <Stack divider={<Divider />}>
              {SOFA17_PROFILE.sessions.map((session) => (
      ...
      >
        <Stack spacing={3}>
          <Stack direction="row" spacing={1} sx={{ overflowX: 'auto', pb: 0.5 }}>
            {SOFA17_ADMIN_TOOLS.map((item) => (
              <Button
                key={item.slug}
                href={`${SOFA17_ADMIN_ROOT}/tools/${item.slug}`}
