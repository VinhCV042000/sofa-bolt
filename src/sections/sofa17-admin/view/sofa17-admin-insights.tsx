...
import {
  SOFA17_ADMIN_ROOT,
  SOFA17_ADMIN_CHART_MONTHS,
  SOFA17_ADMIN_REVENUE_SERIES,
} from '../sofa17-admin-data';
import { SOFA17_ADMIN_TOOLS, SOFA17_ADMIN_FUNNEL } from '../sofa17-admin-tools';
...
export function Sofa17AdminRevenueChart() {
  const actual = SOFA17_ADMIN_REVENUE_SERIES[0].data;
  const target = SOFA17_ADMIN_REVENUE_SERIES[1].data;
  ...
  const options = useChart({
    ...
    xaxis: { categories: SOFA17_ADMIN_CHART_MONTHS },
...
export function Sofa17AdminFunnel() {
  const top = SOFA17_ADMIN_FUNNEL[0].value;
  ...
  return (
    <Card sx={{ height: 1 }}>
      ...
      <Stack spacing={2} sx={{ p: 3 }}>
        {SOFA17_ADMIN_FUNNEL.map((step, index) => {
          const ratio = (step.value / top) * 100;
          const previous = SOFA17_ADMIN_FUNNEL[index - 1];
...
export function Sofa17AdminQuickLinks() {
  return (
    <Card sx={{ p: 3 }}>
      ...
      <Grid container spacing={2}>
        {SOFA17_ADMIN_TOOLS.map((tool) => (
          <Grid key={tool.slug} xs={12} sm={6} md={3}>
            <ButtonBase
              component={RouterLink}
              href={`${SOFA17_ADMIN_ROOT}/tools/${tool.slug}`}
              sx={{
                width: 1,
                height: 1,
                p: 2,
