...
import { type AdminRow, useSofa17AdminRows } from '../sofa17-admin-store';
import { Sofa17AdminLayout, SOFA17_ADMIN_THEME } from './sofa17-admin-layout';
import { SOFA17_ADMIN_ROOT, SOFA17_ADMIN_GROUPS, findSofa17AdminModule } from '../sofa17-admin-data';

// ----------------------------------------------------------------------

const { ACCENT, SURFACE } = SOFA17_ADMIN_THEME;
...
export function Sofa17AdminModuleView() {
  ...
  const found = useMemo(() => findSofa17AdminModule(groupSlug, moduleSlug), [groupSlug, moduleSlug]);

  const group = found?.group;
  const module = found?.module;

  const { rows, createRow, updateRow, deleteRow, deleteRows, resetRows } = useSofa17AdminRows(
    group?.slug ?? '',
    module?.slug ?? ''
  );
  ...
  if (!found || !group || !module) {
    const first = SOFA17_ADMIN_GROUPS[0];
        ...
        <Grid container spacing={3}>
          ...
          <Grid xs={12}>
            <Card>
              ...
              <TableContainer sx={{ borderTop: `1px solid ${alpha('#9B2335', 0.16)}` }}>
                <Table size="medium">
                  <TableHead>
                    <TableRow>
              ...
              >
                ...
                <Chip
                  ...
                  sx={{ mr: 2, mb: { xs: 2, sm: 0 }, bgcolor: alpha(ACCENT, 0.14), color: '#9B2335' }}
                />
              </Stack>
            </Card>
          </Grid>
