// SOFA17 ADMIN — cấu hình khu vực quản trị (dữ liệu demo)
// ----------------------------------------------------------------------

export const SOFA17_ADMIN_ROOT = '/sofa17/admin';

export type Sofa17AdminColumn = { key: string; label: string; type?: 'text' | 'status' | 'money' | 'number' };

export type Sofa17AdminModule = {
  slug: string;
  name: string;
  description: string;
  icon: string;
  stats: { label: string; value: string; trend?: string }[];
  columns: Sofa17AdminColumn[];
  rows: Record<string, string | number>[];
  actions?: string[];
};

export type Sofa17AdminGroup = {
  slug: string;
  name: string;
  icon: string;
  modules: Sofa17AdminModule[];
};
...
const STATUS_COL: Sofa17AdminColumn = { key: 'status', label: 'Trạng thái', type: 'status' };
...
const CMS_GROUP: Sofa17AdminGroup = {
  slug: 'cms',
  name: 'CMS',
  icon: 'solar:documents-bold-duotone',
  modules: [
    cmsPage('home', 'Trang chủ', '/sofa17'),
    cmsPage('about', 'Trang giới thiệu', '/sofa17/about'),
    cmsPage('contact', 'Trang liên hệ', '/sofa17/contact'),
    cmsPage('policy', 'Trang chính sách', '/sofa17/policy'),
    cmsPage('terms', 'Trang điều khoản', '/sofa17/policy/terms'),
    cmsPage('faq', 'Trang FAQ', '/sofa17/faq'),
    mod(
      'blog',
      'Trang blog',
    ...
    mod(
      ...
      [
        { page: '/sofa17', title: 'SOFA17 – Sofa thủ công cao cấp', length: 54, status: 'Tốt' },
        { page: '/sofa17/about', title: 'Về chúng tôi | SOFA17', length: 42, status: 'Tốt' },
        { page: '/sofa17/faq', title: '', length: 0, status: 'Thiếu meta' },
        { page: '/sofa17/policy', title: 'Chính sách mua hàng SOFA17', length: 48, status: 'Tốt' },
      ],
      ['Quét lại']
    ),
    mod(
      ...
      [
        { title: 'Hướng dẫn đo kích thước', slug: '/sofa17/huong-dan-do', status: 'Đã xuất bản', updated: '08/08/2026' },
        { title: 'Cam kết chất liệu', slug: '/sofa17/cam-ket', status: 'Đã xuất bản', updated: '01/08/2026' },
        { title: 'Tuyển đại lý 2026', slug: '/sofa17/tuyen-dai-ly', status: 'Bản nháp', updated: '13/08/2026' },
      ],
      ['Tạo trang']
    ),
  ],
...
const PRODUCT_GROUP: Sofa17AdminGroup = {
  slug: 'catalog',
  name: 'Sản phẩm',
...
const WAREHOUSE_GROUP: Sofa17AdminGroup = {
  slug: 'warehouse',
  name: 'Kho hàng',
...
const ORDER_GROUP: Sofa17AdminGroup = {
  slug: 'orders',
  name: 'Đơn hàng',
...
const CRM_GROUP: Sofa17AdminGroup = {
  slug: 'crm',
  name: 'CRM',
...
const MARKETING_GROUP: Sofa17AdminGroup = {
  slug: 'marketing',
  name: 'Marketing',
...
const ANALYTICS_GROUP: Sofa17AdminGroup = {
  slug: 'analytics',
  name: 'Analytics',
...
const seoPage = (slug: string, name: string, sample: string) =>
  mod(
    ...
    [
      { url: sample, title: `${name} – SOFA17`, keyword: 'sofa cao cấp', position: 4, status: 'Tốt' },
...
const SEO_GROUP: Sofa17AdminGroup = {
  slug: 'seo',
  name: 'SEO',
  icon: 'solar:graph-new-bold-duotone',
  modules: [
    seoPage('category', 'Trang danh mục', '/sofa17/products/category'),
    seoPage('product', 'Trang sản phẩm', '/sofa17/products'),
    seoPage('collection', 'Trang bộ sưu tập', '/sofa17/collections'),
    seoPage('project', 'Trang dự án', '/sofa17/projects'),
    seoPage('showroom', 'Trang showroom', '/sofa17/showrooms'),
    seoPage('blog', 'Trang blog', '/sofa17/blog'),
    seoPage('brand', 'Trang thương hiệu', '/sofa17/about'),
    mod(
      'sitemap',
      'Sitemap',
    ...
    mod(
      ...
      [
        ...
        { agent: '*', rule: 'Disallow', path: '/sofa17/admin', status: 'Hoạt động' },
        { agent: '*', rule: 'Disallow', path: '/sofa17/cart', status: 'Hoạt động' },
...
const ACCESS_GROUP: Sofa17AdminGroup = {
  slug: 'access',
  name: 'Phân quyền',
  ...
  modules: [
    mod(
      ...
      [
        { name: 'Ngọc Anh', email: 'ngocanh@sofa17.vn', role: 'Quản trị hệ thống', last: '15/08/2026 09:12', status: 'Hoạt động' },
        { name: 'Minh Quân', email: 'quan@sofa17.vn', role: 'Quản lý bán hàng', last: '14/08/2026 17:40', status: 'Hoạt động' },
        { name: 'Thu Hà', email: 'ha@sofa17.vn', role: 'CSKH', last: '15/08/2026 08:02', status: 'Hoạt động' },
        { name: 'Gia Bảo', email: 'bao@sofa17.vn', role: 'Biên tập nội dung', last: '02/07/2026 10:20', status: 'Bị khoá' },
      ],
      ['Mời người dùng']
    ),
    mod(
...
export const SOFA17_ADMIN_GROUPS: Sofa17AdminGroup[] = [
  CMS_GROUP,
  PRODUCT_GROUP,
  WAREHOUSE_GROUP,
  ORDER_GROUP,
...
export function findSofa17AdminModule(groupSlug?: string, moduleSlug?: string) {
  const group = SOFA17_ADMIN_GROUPS.find((g) => g.slug === groupSlug);
  if (!group) return null;
  const found = group.modules.find((m) => m.slug === moduleSlug) ?? group.modules[0];
  return { group, module: found };
}

export const SOFA17_ADMIN_KPIS = [
...
export const SOFA17_ADMIN_ACTIVITIES = [
...
export const SOFA17_ADMIN_CHART_MONTHS = [
  'T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'T8', 'T9', 'T10', 'T11', 'T12',
];

export const SOFA17_ADMIN_REVENUE_SERIES = [
