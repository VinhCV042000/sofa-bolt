...
import {
  ...
  SOFA7_ADMIN_TOOLS,
  SOFA7_NOTIFICATIONS,
  SOFA7_SETTINGS_FIELDS,
  SOFA7_SETTINGS_TOGGLES,
  findSofa7AdminTool,
} from '../sofa7-admin-tools';
import { Sofa7AdminCharts } from './sofa7-admin-charts';
import { Sofa7AdminFunnel, Sofa7AdminRevenueChart } from './sofa7-admin-insights';
import { Sofa7AdminLayout } from './sofa7-admin-layout';
import { SOFA7_ADMIN_ROOT } from '../sofa7-admin-data';
...
export function Sofa7AdminToolsView() {
  const { tool: toolSlug } = useParams();
  const tool = findSofa7AdminTool(toolSlug ?? 'reports');
  ...
  const [settings, setSettings] = useState<SettingsValues>(() =>
    Object.fromEntries(SOFA7_SETTINGS_FIELDS.map((field) => [field.key, field.value]))
  );
  const [toggles, setToggles] = useState<ToggleValues>(() =>
    Object.fromEntries(SOFA7_SETTINGS_TOGGLES.map((item) => [item.key, item.on]))
  );

  const notifications = useMemo(
    () =>
      SOFA7_NOTIFICATIONS.filter((item) => {
  ...
  if (!tool) {
    return <Navigate to={`${SOFA7_ADMIN_ROOT}/tools/reports`} replace />;
  }
    ...
    <Stack spacing={3}>
      <Card>
        ...
        <Grid container spacing={2.5} sx={{ p: 3 }}>
          {SOFA7_SETTINGS_FIELDS.map((field) => (
      ...
      <Card>
        ...
        <Grid container spacing={1} sx={{ p: 2.5 }}>
          {SOFA7_SETTINGS_TOGGLES.map((item) => (
      ...
      <Grid xs={12} md={5}>
        <Card sx={{ p: 3, height: 1 }}>
          <Stack alignItems="center" spacing={1.5} sx={{ textAlign: 'center' }}>
            ...
            <Typography variant="h6">{SOFA7_PROFILE.name}</Typography>
            <Chip label={SOFA7_PROFILE.role} color="warning" variant="soft" />
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>{SOFA7_PROFILE.email}</Typography>
          </Stack>
          <Divider sx={{ my: 3 }} />
          <Stack spacing={1.5}>
            {[
              ['Số điện thoại', SOFA7_PROFILE.phone],
              ['Bộ phận', SOFA7_PROFILE.team],
              ['Tham gia từ', SOFA7_PROFILE.joined],
            ].map(([label, value]) => (
        ...
        <Stack spacing={3}>
          <Card>
            ...
            <Stack direction="row" flexWrap="wrap" useFlexGap gap={1} sx={{ p: 3 }}>
              {SOFA7_PROFILE.permissions.map((permission) => (
          ...
          <Card>
            ...
            <Divider />
            <Stack divider={<Divider />}>
              {SOFA7_PROFILE.sessions.map((session) => (
  ...
  return (
    <>
      <Helmet>
        <title>{`${tool.name} | Quản trị Sofa7`}</title>
      ...
      >
        <Stack spacing={3}>
          <Stack direction="row" spacing={1} sx={{ overflowX: 'auto', pb: 0.5 }}>
            {SOFA7_ADMIN_TOOLS.map((item) => (
              <Button
                key={item.slug}
                href={`${SOFA7_ADMIN_ROOT}/tools/${item.slug}`}
