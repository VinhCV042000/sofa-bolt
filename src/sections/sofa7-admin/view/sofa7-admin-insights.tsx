...
import {
  SOFA7_ADMIN_ROOT,
  SOFA7_ADMIN_CHART_MONTHS,
  SOFA7_ADMIN_REVENUE_SERIES,
} from '../sofa7-admin-data';
import { SOFA7_ADMIN_TOOLS, SOFA7_ADMIN_FUNNEL } from '../sofa7-admin-tools';
...
export function Sofa7AdminRevenueChart() {
  const actual = SOFA7_ADMIN_REVENUE_SERIES[0].data;
  const target = SOFA7_ADMIN_REVENUE_SERIES[1].data;
  ...
  const options = useChart({
    ...
    xaxis: { categories: SOFA7_ADMIN_CHART_MONTHS },
...
export function Sofa7AdminFunnel() {
  const top = SOFA7_ADMIN_FUNNEL[0].value;
  ...
  return (
    <Card sx={{ height: 1 }}>
      ...
      <Stack spacing={2} sx={{ p: 3 }}>
        {SOFA7_ADMIN_FUNNEL.map((step, index) => {
          const ratio = (step.value / top) * 100;
          const previous = SOFA7_ADMIN_FUNNEL[index - 1];
...
export function Sofa7AdminQuickLinks() {
  return (
    <Card sx={{ p: 3 }}>
      ...
      <Grid container spacing={2}>
        {SOFA7_ADMIN_TOOLS.map((tool) => (
          <Grid key={tool.slug} xs={12} sm={6} md={3}>
            <ButtonBase
              component={RouterLink}
              href={`${SOFA7_ADMIN_ROOT}/tools/${tool.slug}`}
              sx={{
                width: 1,
                height: 1,
                p: 2,
