...
import { type AdminRow, useSofa7AdminRows } from '../sofa7-admin-store';
import { Sofa7AdminLayout, SOFA7_ADMIN_THEME } from './sofa7-admin-layout';
import { SOFA7_ADMIN_ROOT, SOFA7_ADMIN_GROUPS, findSofa7AdminModule } from '../sofa7-admin-data';

// ----------------------------------------------------------------------

const { ACCENT, SURFACE } = SOFA7_ADMIN_THEME;
...
export function Sofa7AdminModuleView() {
  ...
  const found = useMemo(() => findSofa7AdminModule(groupSlug, moduleSlug), [groupSlug, moduleSlug]);

  const group = found?.group;
  const module = found?.module;

  const { rows, createRow, updateRow, deleteRow, deleteRows, resetRows } = useSofa7AdminRows(
    group?.slug ?? '',
    module?.slug ?? ''
  );
  ...
  if (!found || !group || !module) {
    const first = SOFA7_ADMIN_GROUPS[0];
