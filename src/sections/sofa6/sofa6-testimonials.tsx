...
import { SOFA6_COLORS, SOFA6_TESTIMONIALS } from './sofa6-data';

// ----------------------------------------------------------------------

export function Sofa6Testimonials({ sx, ...other }: BoxProps) {
            ...
            {SOFA6_TESTIMONIALS.map((item) => (
              ...
              >
                <Iconify
                  ...
                  width={32}
                  sx={{ color: SOFA6_COLORS.sage, opacity: 0.2, position: 'absolute', top: 16, right: 16 }}
                />
                <Rating value={item.rating} readOnly size="small" sx={{ color: SOFA6_COLORS.terracotta }} />
                <Typography variant="body2" sx={{ color: 'text.secondary', flex: 1, lineHeight: 1.7 }}>
                  &ldquo;{item.content}&rdquo;
                </Typography>
                <Stack direction="row" spacing={2} alignItems="center">
                  <Avatar src={item.avatar} alt={item.name} sx={{ border: `2px solid ${SOFA6_COLORS.sageLight}` }} />
                  <Stack spacing={0.25}>
                    <Typography variant="subtitle2" sx={{ color: SOFA6_COLORS.ink }}>
                      {item.name}
                    </Typography>
                    <Typography variant="caption" sx={{ color: SOFA6_COLORS.clay }}>
                      {item.role}
                    </Typography>
                  </Stack>
                </Stack>
