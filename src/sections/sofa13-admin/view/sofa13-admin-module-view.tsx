...
import { type AdminRow, useSofa13AdminRows } from '../sofa13-admin-store';
import { Sofa13AdminLayout, SOFA13_ADMIN_THEME } from './sofa13-admin-layout';
import { SOFA13_ADMIN_ROOT, SOFA13_ADMIN_GROUPS, findSofa13AdminModule } from '../sofa13-admin-data';

// ----------------------------------------------------------------------

const { ACCENT, SURFACE } = SOFA13_ADMIN_THEME;
...
export function Sofa13AdminModuleView() {
  const { group: groupSlug, module: moduleSlug } = useParams();
  const adminRoot = SOFA13_ADMIN_ROOT;

  const found = useMemo(() => findSofa13AdminModule(groupSlug, moduleSlug), [groupSlug, moduleSlug]);

  const group = found?.group;
  const module = found?.module;

  const { rows, createRow, updateRow, deleteRow, deleteRows, resetRows } = useSofa13AdminRows(
    group?.slug ?? '',
    module?.slug ?? ''
  );
  ...
  if (!found || !group || !module) {
    const first = SOFA13_ADMIN_GROUPS[0];
