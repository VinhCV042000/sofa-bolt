...
import {
  SOFA13_PROFILE,
  SOFA13_ADMIN_TOOLS,
  SOFA13_NOTIFICATIONS,
  SOFA13_SETTINGS_FIELDS,
  SOFA13_SETTINGS_TOGGLES,
  findSofa13AdminTool,
} from '../sofa13-admin-tools';
import { Sofa13AdminCharts } from './sofa13-admin-charts';
import { Sofa13AdminFunnel, Sofa13AdminRevenueChart } from './sofa13-admin-insights';
import { Sofa13AdminLayout } from './sofa13-admin-layout';
import { SOFA13_ADMIN_ROOT } from '../sofa13-admin-data';
...
export function Sofa13AdminToolsView() {
  const { tool: toolSlug } = useParams();
  const tool = findSofa13AdminTool(toolSlug ?? 'reports');
  const [toast, setToast] = useState('');
  const [period, setPeriod] = useState('12 months');
  const [notificationSearch, setNotificationSearch] = useState('');
  const [notificationLevel, setNotificationLevel] = useState('all');
  const [settings, setSettings] = useState<SettingsValues>(() =>
    Object.fromEntries(SOFA13_SETTINGS_FIELDS.map((field) => [field.key, field.value]))
  );
  const [toggles, setToggles] = useState<ToggleValues>(() =>
    Object.fromEntries(SOFA13_SETTINGS_TOGGLES.map((item) => [item.key, item.on]))
  );

  const notifications = useMemo(
    () =>
      SOFA13_NOTIFICATIONS.filter((item) => {
  ...
  if (!tool) {
    return <Navigate to={`${SOFA13_ADMIN_ROOT}/tools/reports`} replace />;
  }
    ...
    <Stack spacing={3}>
      <Card>
        ...
        <Grid container spacing={2.5} sx={{ p: 3 }}>
          {SOFA13_SETTINGS_FIELDS.map((field) => (
      ...
      <Card>
        ...
        <Grid container spacing={1} sx={{ p: 2.5 }}>
          {SOFA13_SETTINGS_TOGGLES.map((item) => (
      ...
      <Grid xs={12} md={5}>
        <Card sx={{ p: 3, height: 1 }}>
          <Stack alignItems="center" spacing={1.5} sx={{ textAlign: 'center' }}>
            ...
            <Typography variant="h6">{SOFA13_PROFILE.name}</Typography>
            <Chip label={SOFA13_PROFILE.role} color="warning" variant="soft" />
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>{SOFA13_PROFILE.email}</Typography>
          </Stack>
          <Divider sx={{ my: 3 }} />
          <Stack spacing={1.5}>
            {[
              ['Phone', SOFA13_PROFILE.phone],
              ['Team', SOFA13_PROFILE.team],
              ['Joined', SOFA13_PROFILE.joined],
            ].map(([label, value]) => (
        ...
        <Stack spacing={3}>
          <Card>
            ...
            <Stack direction="row" flexWrap="wrap" useFlexGap gap={1} sx={{ p: 3 }}>
              {SOFA13_PROFILE.permissions.map((permission) => (
          ...
          <Card>
            ...
            <Divider />
            <Stack divider={<Divider />}>
              {SOFA13_PROFILE.sessions.map((session) => (
      ...
      >
        <Stack spacing={3}>
          <Stack direction="row" spacing={1} sx={{ overflowX: 'auto', pb: 0.5 }}>
            {SOFA13_ADMIN_TOOLS.map((item) => (
              <Button
                key={item.slug}
                href={`${SOFA13_ADMIN_ROOT}/tools/${item.slug}`}
