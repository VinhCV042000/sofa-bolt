...
import {
  SOFA13_ADMIN_ROOT,
  SOFA13_ADMIN_CHART_MONTHS,
  SOFA13_ADMIN_REVENUE_SERIES,
} from '../sofa13-admin-data';
import { SOFA13_ADMIN_TOOLS, SOFA13_ADMIN_FUNNEL } from '../sofa13-admin-tools';
...
export function Sofa13AdminRevenueChart() {
  const actual = SOFA13_ADMIN_REVENUE_SERIES[0].data;
  const target = SOFA13_ADMIN_REVENUE_SERIES[1].data;
  ...
  const options = useChart({
    ...
    xaxis: { categories: SOFA13_ADMIN_CHART_MONTHS },
...
export function Sofa13AdminFunnel() {
  const top = SOFA13_ADMIN_FUNNEL[0].value;
  ...
  return (
    <Card sx={{ height: 1 }}>
      ...
      <Stack spacing={2} sx={{ p: 3 }}>
        {SOFA13_ADMIN_FUNNEL.map((step, index) => {
          const ratio = (step.value / top) * 100;
          const previous = SOFA13_ADMIN_FUNNEL[index - 1];
...
export function Sofa13AdminQuickLinks() {
  return (
    <Card sx={{ p: 3 }}>
      ...
      <Grid container spacing={2}>
        {SOFA13_ADMIN_TOOLS.map((tool) => (
          <Grid key={tool.slug} xs={12} sm={6} md={3}>
            <ButtonBase
              component={RouterLink}
              href={`${SOFA13_ADMIN_ROOT}/tools/${tool.slug}`}
              sx={{
                width: 1,
                height: 1,
                p: 2,
