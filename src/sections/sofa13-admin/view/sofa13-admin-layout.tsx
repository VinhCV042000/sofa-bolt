...
import { SOFA13_ADMIN_ROOT, SOFA13_ADMIN_GROUPS } from '../sofa13-admin-data';
import { SOFA13_ADMIN_TOOLS } from '../sofa13-admin-tools';
...
export function Sofa13AdminLayout({
  children,
  activeGroup,
  activeModule,
  title,
  ...
  const [openGroups, setOpenGroups] = useState<string[]>(
    activeGroup ? [activeGroup] : [SOFA13_ADMIN_GROUPS[0].slug]
  );
  ...
  const nav = (
    <Stack sx={{ width: NAV_WIDTH, height: 1, bgcolor: BG, color: TEXT }}>
      <Stack
        ...
        alignItems="center"
        spacing={1.5}
        sx={{ px: 2.5, py: 3, borderBottom: `1px solid ${alpha(ACCENT, 0.2)}` }}
      >
        <Box
          sx={{
            width: 40,
      ...
      <Stack direction="row" alignItems="center" spacing={1.5} sx={{ p: 2 }}>
        <Avatar sx={{ bgcolor: alpha(ACCENT, 0.24), color: ACCENT, width: 36, height: 36 }}>EV</Avatar>
        ...
        <Tooltip title="Back to storefront">
          <IconButton
            component={RouterLink}
            href="/sofa13"
...
export const SOFA13_ADMIN_THEME = { BG, SURFACE, ACCENT, TEXT };
