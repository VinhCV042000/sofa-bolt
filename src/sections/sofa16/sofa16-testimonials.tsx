...
export function Sofa16Testimonials({ sx, ...other }: BoxProps) {
          ...
          <Carousel carousel={carousel}>
            {content.testimonials.items.map((item, index) => (
              ...
              >
                ...
                <Rating value={5} readOnly size="small" sx={{ color: SOFA16_COLORS.terracotta }} />
                <Typography variant="body2" sx={{ color: SOFA16_COLORS.inkSoft, flex: 1, lineHeight: 2, fontWeight: 'fontWeightLight', fontStyle: 'italic' }}>
                  {item.quote}
                </Typography>
                <Stack direction="row" spacing={2} alignItems="center">
                  <Avatar src={AVATARS[index]} alt={item.name} sx={{ border: `2px solid ${varAlpha(SOFA16_COLORS.terracotta, 0.2)}` }} />
                  <Stack spacing={0.25}>
                    <Typography variant="subtitle2" sx={{ color: SOFA16_COLORS.ink, fontWeight: 'fontWeightMedium', letterSpacing: 0.3 }}>
                      {item.name}
                    </Typography>
                    <Typography variant="caption" sx={{ color: SOFA16_COLORS.olive, fontWeight: 'fontWeightLight' }}>
                      {item.role}
                    </Typography>
                  </Stack>
                </Stack>
