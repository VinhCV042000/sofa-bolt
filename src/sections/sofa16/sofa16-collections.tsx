...
const COLLECTION_IMAGES = [SOFA16_IMAGES.cat1, SOFA16_IMAGES.cat2, SOFA16_IMAGES.cat3, SOFA16_IMAGES.cat4];

export function Sofa16Collections({ sx, ...other }: BoxProps) {
    ...
    >
      <Container component={MotionViewport}>
        <Stack spacing={3} sx={{ mb: { xs: 6, md: 10 }, maxWidth: 580 }}>
          ...
          <Box component={m.div} variants={varFade({ distance: 24 }).inUp}>
            <Typography variant="h2" sx={{ color: SOFA16_COLORS.ink, fontWeight: 'fontWeightLight', letterSpacing: -0.5, lineHeight: 1.2 }}>
              {content.collections.title.split(',')[0]},
              <Box component="span" sx={{ color: SOFA16_COLORS.terracotta, fontStyle: 'italic' }}>
                {content.collections.title.split(',')[1]}
              </Box>
            </Typography>
          </Box>
          <Box component={m.div} variants={varFade({ distance: 24 }).inUp}>
            <Typography sx={{ color: SOFA16_COLORS.inkSoft, maxWidth: 480, lineHeight: 2, fontWeight: 'fontWeightLight' }}>
              ...
              >
                ...
                <Box
                  sx={{
                    inset: 0,
                    position: 'absolute',
                    background: `linear-gradient(to top, ${varAlpha(SOFA16_COLORS.ink, 0.85)}, ${varAlpha(SOFA16_COLORS.terracotta, 0.15)} 55%, transparent)`,
                  }}
                />
                <Stack
                  spacing={1.5}
                ...
                >
                  <Typography
                    variant="overline"
                    sx={{ color: SOFA16_COLORS.terracottaLight, letterSpacing: 2, textTransform: 'uppercase', fontWeight: 'fontWeightMedium' }}
                  >
                    {col.tagline}
                  </Typography>
                  <Typography variant="h5" sx={{ fontWeight: 'fontWeightLight', letterSpacing: 0.5, fontStyle: 'italic' }}>
                    {col.title}
                  </Typography>
                  ...
                  <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mt: 1 }}>
                    ...
                    <Iconify icon="eva:arrow-forward-outline" width={18} sx={{ color: SOFA16_COLORS.terracottaLight }} />
                  </Stack>
                </Stack>
              </Box>
            </Grid>
