...
import { type AdminRow, useSofa18AdminRows } from '../sofa18-admin-store';
import { Sofa18AdminLayout, SOFA18_ADMIN_THEME } from './sofa18-admin-layout';
import { SOFA18_ADMIN_ROOT, SOFA18_ADMIN_GROUPS, findSofa18AdminModule } from '../sofa18-admin-data';

// ----------------------------------------------------------------------

const { ACCENT, ACCENT_DEEP, SURFACE } = SOFA18_ADMIN_THEME;
...
export function Sofa18AdminModuleView() {
  const { group: groupSlug, module: moduleSlug } = useParams();

  const found = useMemo(() => findSofa18AdminModule(groupSlug, moduleSlug), [groupSlug, moduleSlug]);

  const group = found?.group;
  const module = found?.module;

  const { rows, createRow, updateRow, deleteRow, deleteRows, resetRows } = useSofa18AdminRows(
    group?.slug ?? '',
    module?.slug ?? ''
  );
  ...
  if (!found || !group || !module) {
    const first = SOFA18_ADMIN_GROUPS[0];
    return <Navigate to={`${SOFA18_ADMIN_ROOT}/${first.slug}/${first.modules[0].slug}`} replace />;
  }
  ...
  return (
    <>
      <Helmet>
        <title>{`${module.name} | ${group.name} - Quản trị Sofa18`}</title>
