...
import { SOFA18_ADMIN_ROOT, SOFA18_ADMIN_GROUPS } from '../sofa18-admin-data';
import { SOFA18_ADMIN_TOOLS } from '../sofa18-admin-tools';
...
export function Sofa18AdminLayout({
  children,
  activeGroup,
  activeModule,
  title,
  ...
  const adminRoot = SOFA18_ADMIN_ROOT;
  const brandName = 'Sofa18 Wabi-Sabi';
  const siteLink = '/sofa18';
  const isDesktop = useMediaQuery('(min-width:1200px)');
  const [mobileOpen, setMobileOpen] = useState(false);
  const [openGroups, setOpenGroups] = useState<string[]>(
    activeGroup ? [activeGroup] : [SOFA18_ADMIN_GROUPS[0].slug]
  );
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        <Box
          ...
          sx={{
            ...
            borderBottom: `1px solid ${alpha(ACCENT_DEEP, 0.16)}`,
          }}
        >
          <Container maxWidth={false} sx={{ maxWidth: 1440 }}>
            <Stack direction="row" alignItems="center" spacing={2} sx={{ py: 2 }}>
              ...
              <Chip
                ...
                label="Dữ liệu demo"
                sx={{ bgcolor: alpha(ACCENT, 0.16), color: ACCENT_DEEP, fontWeight: 600 }}
              />
            </Stack>
          </Container>
        </Box>
...
export const SOFA18_ADMIN_THEME = { BG, SURFACE, ACCENT, ACCENT_DEEP, TEXT };
