...
import {
  SOFA18_PROFILE,
  SOFA18_ADMIN_TOOLS,
  SOFA18_NOTIFICATIONS,
  SOFA18_SETTINGS_FIELDS,
  SOFA18_SETTINGS_TOGGLES,
  findSofa18AdminTool,
} from '../sofa18-admin-tools';
import { Sofa18AdminCharts } from './sofa18-admin-charts';
import { Sofa18AdminFunnel, Sofa18AdminRevenueChart } from './sofa18-admin-insights';
import { Sofa18AdminLayout } from './sofa18-admin-layout';
import { SOFA18_ADMIN_ROOT } from '../sofa18-admin-data';
...
export function Sofa18AdminToolsView() {
  const { tool: toolSlug } = useParams();
  const tool = findSofa18AdminTool(toolSlug ?? 'reports');
  ...
  const [settings, setSettings] = useState<SettingsValues>(() =>
    Object.fromEntries(SOFA18_SETTINGS_FIELDS.map((field) => [field.key, field.value]))
  );
  const [toggles, setToggles] = useState<ToggleValues>(() =>
    Object.fromEntries(SOFA18_SETTINGS_TOGGLES.map((item) => [item.key, item.on]))
  );

  const notifications = useMemo(
    () =>
      SOFA18_NOTIFICATIONS.filter((item) => {
  ...
  if (!tool) {
    return <Navigate to={`${SOFA18_ADMIN_ROOT}/tools/reports`} replace />;
  }
    ...
    <Stack spacing={3}>
      <Card>
        ...
        <Grid container spacing={2.5} sx={{ p: 3 }}>
          {SOFA18_SETTINGS_FIELDS.map((field) => (
      ...
      <Card>
        ...
        <Grid container spacing={1} sx={{ p: 2.5 }}>
          {SOFA18_SETTINGS_TOGGLES.map((item) => (
      ...
      <Grid xs={12} md={5}>
        <Card sx={{ p: 3, height: 1 }}>
          <Stack alignItems="center" spacing={1.5} sx={{ textAlign: 'center' }}>
            ...
            <Typography variant="h6">{SOFA18_PROFILE.name}</Typography>
            <Chip label={SOFA18_PROFILE.role} color="warning" variant="soft" />
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>{SOFA18_PROFILE.email}</Typography>
          </Stack>
          <Divider sx={{ my: 3 }} />
          <Stack spacing={1.5}>
            {[
              ['Số điện thoại', SOFA18_PROFILE.phone],
              ['Bộ phận', SOFA18_PROFILE.team],
              ['Tham gia từ', SOFA18_PROFILE.joined],
            ].map(([label, value]) => (
        ...
        <Stack spacing={3}>
          <Card>
            ...
            <Stack direction="row" flexWrap="wrap" useFlexGap gap={1} sx={{ p: 3 }}>
              {SOFA18_PROFILE.permissions.map((permission) => (
          ...
          <Card>
            ...
            <Divider />
            <Stack divider={<Divider />}>
              {SOFA18_PROFILE.sessions.map((session) => (
  ...
  return (
    <>
      <Helmet>
        <title>{`${tool.name} | Quản trị Sofa18`}</title>
      ...
      >
        <Stack spacing={3}>
          <Stack direction="row" spacing={1} sx={{ overflowX: 'auto', pb: 0.5 }}>
            {SOFA18_ADMIN_TOOLS.map((item) => (
              <Button
                key={item.slug}
                href={`${SOFA18_ADMIN_ROOT}/tools/${item.slug}`}
