// SOFA16 ADMIN — cấu hình khu vực quản trị (dữ liệu demo)
// ----------------------------------------------------------------------

export const SOFA16_ADMIN_ROOT = '/sofa16/admin';

export type Sofa16AdminColumn = { key: string; label: string; type?: 'text' | 'status' | 'money' | 'number' };

export type Sofa16AdminModule = {
  slug: string;
  name: string;
  description: string;
  icon: string;
  stats: { label: string; value: string; trend?: string }[];
  columns: Sofa16AdminColumn[];
  rows: Record<string, string | number>[];
  actions?: string[];
};

export type Sofa16AdminGroup = {
  slug: string;
  name: string;
  icon: string;
  modules: Sofa16AdminModule[];
};
...
const STATUS_COL: Sofa16AdminColumn = { key: 'status', label: 'Trạng thái', type: 'status' };
...
const CMS_GROUP: Sofa16AdminGroup = {
  slug: 'cms',
  name: 'CMS',
  icon: 'solar:documents-bold-duotone',
  modules: [
    cmsPage('home', 'Trang chủ', '/sofa16'),
    cmsPage('about', 'Trang giới thiệu', '/sofa16/about'),
    cmsPage('contact', 'Trang liên hệ', '/sofa16/contact'),
    cmsPage('policy', 'Trang chính sách', '/sofa16/policy'),
    cmsPage('terms', 'Trang điều khoản', '/sofa16/policy/terms'),
    cmsPage('faq', 'Trang FAQ', '/sofa16/faq'),
    mod(
      'blog',
      'Trang blog',
    ...
    mod(
      ...
      [
        { page: '/sofa16', title: 'SOFA16 – Sofa thủ công cao cấp', length: 54, status: 'Tốt' },
        { page: '/sofa16/about', title: 'Về chúng tôi | SOFA16', length: 42, status: 'Tốt' },
        { page: '/sofa16/faq', title: '', length: 0, status: 'Thiếu meta' },
        { page: '/sofa16/policy', title: 'Chính sách mua hàng SOFA16', length: 48, status: 'Tốt' },
      ],
      ['Quét lại']
    ),
    mod(
      ...
      [
        { title: 'Hướng dẫn đo kích thước', slug: '/sofa16/huong-dan-do', status: 'Đã xuất bản', updated: '08/08/2026' },
        { title: 'Cam kết chất liệu', slug: '/sofa16/cam-ket', status: 'Đã xuất bản', updated: '01/08/2026' },
        { title: 'Tuyển đại lý 2026', slug: '/sofa16/tuyen-dai-ly', status: 'Bản nháp', updated: '13/08/2026' },
      ],
      ['Tạo trang']
    ),
  ],
...
const PRODUCT_GROUP: Sofa16AdminGroup = {
  slug: 'catalog',
  name: 'Sản phẩm',
...
const WAREHOUSE_GROUP: Sofa16AdminGroup = {
  slug: 'warehouse',
  name: 'Kho hàng',
...
const ORDER_GROUP: Sofa16AdminGroup = {
  slug: 'orders',
  name: 'Đơn hàng',
...
const CRM_GROUP: Sofa16AdminGroup = {
  slug: 'crm',
  name: 'CRM',
...
const MARKETING_GROUP: Sofa16AdminGroup = {
  slug: 'marketing',
  name: 'Marketing',
...
const ANALYTICS_GROUP: Sofa16AdminGroup = {
  slug: 'analytics',
  name: 'Analytics',
...
const seoPage = (slug: string, name: string, sample: string) =>
  mod(
    ...
    [
      { url: sample, title: `${name} – SOFA16`, keyword: 'sofa cao cấp', position: 4, status: 'Tốt' },
...
const SEO_GROUP: Sofa16AdminGroup = {
  slug: 'seo',
  name: 'SEO',
  icon: 'solar:graph-new-bold-duotone',
  modules: [
    seoPage('category', 'Trang danh mục', '/sofa16/products/category'),
    seoPage('product', 'Trang sản phẩm', '/sofa16/products'),
    seoPage('collection', 'Trang bộ sưu tập', '/sofa16/collections'),
    seoPage('project', 'Trang dự án', '/sofa16/projects'),
    seoPage('showroom', 'Trang showroom', '/sofa16/showrooms'),
    seoPage('blog', 'Trang blog', '/sofa16/blog'),
    seoPage('brand', 'Trang thương hiệu', '/sofa16/about'),
    mod(
      'sitemap',
      'Sitemap',
    ...
    mod(
      ...
      [
        ...
        { agent: '*', rule: 'Disallow', path: '/sofa16/admin', status: 'Hoạt động' },
        { agent: '*', rule: 'Disallow', path: '/sofa16/cart', status: 'Hoạt động' },
...
const ACCESS_GROUP: Sofa16AdminGroup = {
  slug: 'access',
  name: 'Phân quyền',
  ...
  modules: [
    mod(
      ...
      [
        { name: 'Ngọc Anh', email: 'ngocanh@sofa16.vn', role: 'Quản trị hệ thống', last: '15/08/2026 09:12', status: 'Hoạt động' },
        { name: 'Minh Quân', email: 'quan@sofa16.vn', role: 'Quản lý bán hàng', last: '14/08/2026 17:40', status: 'Hoạt động' },
        { name: 'Thu Hà', email: 'ha@sofa16.vn', role: 'CSKH', last: '15/08/2026 08:02', status: 'Hoạt động' },
        { name: 'Gia Bảo', email: 'bao@sofa16.vn', role: 'Biên tập nội dung', last: '02/07/2026 10:20', status: 'Bị khoá' },
      ],
      ['Mời người dùng']
    ),
    mod(
...
export const SOFA16_ADMIN_GROUPS: Sofa16AdminGroup[] = [
  CMS_GROUP,
  PRODUCT_GROUP,
  WAREHOUSE_GROUP,
  ORDER_GROUP,
...
export function findSofa16AdminModule(groupSlug?: string, moduleSlug?: string) {
  const group = SOFA16_ADMIN_GROUPS.find((g) => g.slug === groupSlug);
  if (!group) return null;
  const found = group.modules.find((m) => m.slug === moduleSlug) ?? group.modules[0];
  return { group, module: found };
}

export const SOFA16_ADMIN_KPIS = [
...
export const SOFA16_ADMIN_ACTIVITIES = [
...
export const SOFA16_ADMIN_CHART_MONTHS = [
  'T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'T8', 'T9', 'T10', 'T11', 'T12',
];

export const SOFA16_ADMIN_REVENUE_SERIES = [
