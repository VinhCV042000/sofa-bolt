...
import { type AdminRow, useSofa16AdminRows } from '../sofa16-admin-store';
import { Sofa16AdminLayout, SOFA16_ADMIN_THEME } from './sofa16-admin-layout';
import { SOFA16_ADMIN_ROOT, SOFA16_ADMIN_GROUPS, findSofa16AdminModule } from '../sofa16-admin-data';

// ----------------------------------------------------------------------

const { ACCENT, SURFACE } = SOFA16_ADMIN_THEME;
...
export function Sofa16AdminModuleView() {
  ...
  const found = useMemo(() => findSofa16AdminModule(groupSlug, moduleSlug), [groupSlug, moduleSlug]);

  const group = found?.group;
  const module = found?.module;

  const { rows, createRow, updateRow, deleteRow, deleteRows, resetRows } = useSofa16AdminRows(
    group?.slug ?? '',
    module?.slug ?? ''
  );
  ...
  if (!found || !group || !module) {
    const first = SOFA16_ADMIN_GROUPS[0];
