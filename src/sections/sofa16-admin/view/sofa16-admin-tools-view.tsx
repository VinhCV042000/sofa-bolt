...
import {
  SOFA16_PROFILE,
  SOFA16_ADMIN_TOOLS,
  SOFA16_NOTIFICATIONS,
  SOFA16_SETTINGS_FIELDS,
  SOFA16_SETTINGS_TOGGLES,
  findSofa16AdminTool,
} from '../sofa16-admin-tools';
import { Sofa16AdminCharts } from './sofa16-admin-charts';
import { Sofa16AdminFunnel, Sofa16AdminRevenueChart } from './sofa16-admin-insights';
import { Sofa16AdminLayout } from './sofa16-admin-layout';
import { SOFA16_ADMIN_ROOT } from '../sofa16-admin-data';
...
export function Sofa16AdminToolsView() {
  const { tool: toolSlug } = useParams();
  const tool = findSofa16AdminTool(toolSlug ?? 'reports');
  ...
  const [settings, setSettings] = useState<SettingsValues>(() =>
    Object.fromEntries(SOFA16_SETTINGS_FIELDS.map((field) => [field.key, field.value]))
  );
  const [toggles, setToggles] = useState<ToggleValues>(() =>
    Object.fromEntries(SOFA16_SETTINGS_TOGGLES.map((item) => [item.key, item.on]))
  );

  const notifications = useMemo(
    () =>
      SOFA16_NOTIFICATIONS.filter((item) => {
  ...
  if (!tool) {
    return <Navigate to={`${SOFA16_ADMIN_ROOT}/tools/reports`} replace />;
  }
    ...
    <Stack spacing={3}>
      <Card>
        ...
        <Grid container spacing={2.5} sx={{ p: 3 }}>
          {SOFA16_SETTINGS_FIELDS.map((field) => (
      ...
      <Card>
        ...
        <Grid container spacing={1} sx={{ p: 2.5 }}>
          {SOFA16_SETTINGS_TOGGLES.map((item) => (
      ...
      <Grid xs={12} md={5}>
        <Card sx={{ p: 3, height: 1 }}>
          <Stack alignItems="center" spacing={1.5} sx={{ textAlign: 'center' }}>
            ...
            <Typography variant="h6">{SOFA16_PROFILE.name}</Typography>
            <Chip label={SOFA16_PROFILE.role} color="warning" variant="soft" />
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>{SOFA16_PROFILE.email}</Typography>
          </Stack>
          <Divider sx={{ my: 3 }} />
          <Stack spacing={1.5}>
            {[
              ['Số điện thoại', SOFA16_PROFILE.phone],
              ['Bộ phận', SOFA16_PROFILE.team],
              ['Tham gia từ', SOFA16_PROFILE.joined],
            ].map(([label, value]) => (
        ...
        <Stack spacing={3}>
          <Card>
            ...
            <Stack direction="row" flexWrap="wrap" useFlexGap gap={1} sx={{ p: 3 }}>
              {SOFA16_PROFILE.permissions.map((permission) => (
          ...
          <Card>
            ...
            <Divider />
            <Stack divider={<Divider />}>
              {SOFA16_PROFILE.sessions.map((session) => (
      ...
      >
        <Stack spacing={3}>
          <Stack direction="row" spacing={1} sx={{ overflowX: 'auto', pb: 0.5 }}>
            {SOFA16_ADMIN_TOOLS.map((item) => (
              <Button
                key={item.slug}
                href={`${SOFA16_ADMIN_ROOT}/tools/${item.slug}`}
