...
import {
  SOFA16_ADMIN_ROOT,
  SOFA16_ADMIN_CHART_MONTHS,
  SOFA16_ADMIN_REVENUE_SERIES,
} from '../sofa16-admin-data';
import { SOFA16_ADMIN_TOOLS, SOFA16_ADMIN_FUNNEL } from '../sofa16-admin-tools';
...
export function Sofa16AdminRevenueChart() {
  const actual = SOFA16_ADMIN_REVENUE_SERIES[0].data;
  const target = SOFA16_ADMIN_REVENUE_SERIES[1].data;
  ...
  const options = useChart({
    ...
    xaxis: { categories: SOFA16_ADMIN_CHART_MONTHS },
...
export function Sofa16AdminFunnel() {
  const top = SOFA16_ADMIN_FUNNEL[0].value;
  ...
  return (
    <Card sx={{ height: 1 }}>
      ...
      <Stack spacing={2} sx={{ p: 3 }}>
        {SOFA16_ADMIN_FUNNEL.map((step, index) => {
          const ratio = (step.value / top) * 100;
          const previous = SOFA16_ADMIN_FUNNEL[index - 1];
...
export function Sofa16AdminQuickLinks() {
  return (
    <Card sx={{ p: 3 }}>
      ...
      <Grid container spacing={2}>
        {SOFA16_ADMIN_TOOLS.map((tool) => (
          <Grid key={tool.slug} xs={12} sm={6} md={3}>
            <ButtonBase
              component={RouterLink}
              href={`${SOFA16_ADMIN_ROOT}/tools/${tool.slug}`}
              sx={{
                width: 1,
                height: 1,
                p: 2,
